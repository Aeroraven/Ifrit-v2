...
#undef getX
	...
	IFRIT_KERNEL void geometryShadingKernel(
		...
		if constexpr (CU_OPT_ALIGNED_INDEX_BUFFER) {
			...
			posPtr[threadIdx.x * 3 + 0] = dPosBufferAligned + indices.x;
			posPtr[threadIdx.x * 3 + 1] = dPosBufferAligned + indices.y;
			posPtr[threadIdx.x * 3 + 2] = dPosBufferAligned + indices.z;
			varyingPtr[threadIdx.x * 3 + 0] = dVaryingBuffer + indices.x * csVaryingCounts;
			varyingPtr[threadIdx.x * 3 + 1] = dVaryingBuffer + indices.y * csVaryingCounts;
			varyingPtr[threadIdx.x * 3 + 2] = dVaryingBuffer + indices.z * csVaryingCounts;
		}
		else {
	...
	IFRIT_KERNEL void geometryClippingKernel(
		...
		if constexpr (geometryShaderEnabled) {
			...
			if constexpr (CU_OPT_ALIGNED_INDEX_BUFFER) {
				dIndexBuffer = static_cast<int*>(__builtin_assume_aligned(dIndexBuffer, 16));
				iint4 indices = reinterpret_cast<iint4*>(dIndexBuffer)[indexStart >> 2];
	...
	IFRIT_KERNEL void unifiedRasterEngineStageIIKernel(
		...
		for (int sI = 0; sI < dAssembledTriangleCounterM2; sI += CU_SINGLE_TIME_TRIANGLE_FIRST_BINNER) {
			...
			if (dGeometryShader == nullptr) {
				Impl::pixelShadingKernel<0> CU_KARG2(dim3(numTileX, numTileY, 1), dim3(CU_EXPERIMENTAL_SUBTILE_WIDTH, CU_TILE_WIDTH, dispZ)) (
					dFragmentShader, dIndexBuffer, dVaryingBuffer, dColorBuffer, dDepthBuffer);
			}
			else {
				Impl::pixelShadingKernel<1> CU_KARG2(dim3(numTileX, numTileY, 1), dim3(CU_EXPERIMENTAL_SUBTILE_WIDTH, CU_TILE_WIDTH, dispZ)) (
					dFragmentShader, dIndexBuffer, dVaryingBuffer, dColorBuffer, dDepthBuffer);
			}
	...
	IFRIT_KERNEL void unifiedRasterEngineKernel(
		...
		for (int i = 0; i < totalIndices; i += CU_SINGLE_TIME_TRIANGLE * CU_TRIANGLE_STRIDE) {
			...
			if (dGeometryShader == nullptr) {
				...
				Impl::geometryClippingKernel<0> CU_KARG2(geometryExecutionBlocks, CU_GEOMETRY_PROCESSING_THREADS)(
				...
				Impl::geometryShadingKernel CU_KARG2(geometryShaderBlocks, CU_GEOMETRY_SHADER_THREADS) (i, indexCount, dGeometryShader, (float4*)dPositionBuffer,
					dIndexBuffer, dVaryingBuffer);
				Impl::geometryClippingKernelEntryWithGS CU_KARG2(1,1)();
			}
	...
	IFRIT_HOST void unifiedRasterEngineProfileEntry(
		...
		for (int i = 0; i < totalIndices; i += CU_SINGLE_TIME_TRIANGLE * CU_TRIANGLE_STRIDE) {
			...
			for (int sI = 0; sI < dAssembledTriangleCounterM2Host; sI += CU_SINGLE_TIME_TRIANGLE_FIRST_BINNER) {
				...
				int dispZ = (CU_TILE_WIDTH / CU_EXPERIMENTAL_SUBTILE_WIDTH) + (CU_TILE_WIDTH % CU_EXPERIMENTAL_SUBTILE_WIDTH != 0);
				if (dGeometryShader == nullptr) {
					Impl::pixelShadingKernel<0> CU_KARG4(dim3(numTileX, numTileY, 1), dim3(CU_EXPERIMENTAL_SUBTILE_WIDTH, CU_TILE_WIDTH, dispZ), 0, compStream) (
						dFragmentShader, dIndexBuffer, dVaryingBuffer, dColorBuffer, dDepthBuffer
						);
				}
				else {
					Impl::pixelShadingKernel<1> CU_KARG4(dim3(numTileX, numTileY, 1), dim3(CU_EXPERIMENTAL_SUBTILE_WIDTH, CU_TILE_WIDTH, dispZ), 0, compStream) (
						dFragmentShader, dIndexBuffer, dVaryingBuffer, dColorBuffer, dDepthBuffer
						);
				}
