...
// //Sponza float4x4 view = (lookAt({ 0,0.75,1.50 }, { 0,0.75,0.0 }, { 0,1,0
// }));
// //yomiya float4x4 view = (lookAt({ 0,0.1,0.25 }, { 0,0.1,0.0 }, { 0,1,0 }));
...
float4x4 proj = (perspective(60 * 3.14159 / 180, 1920.0 / 1080.0, 1.1, 3000));
float4x4 model;
...
public:
  IFRIT_DUAL virtual void execute(const void *const *input, ifloat4 *outPos,
    ...
    float4x4 proj = (perspective(60 * 3.14159 / 180, 1920.0 / 1080.0, 0.01,
  ...
  if (presentEngine == PE_CONSOLE) {
    ...
    windowProvider.loop([&](int *coreTime) {
      ...
      std::chrono::high_resolution_clock::time_point end =
          std::chrono::high_resolution_clock::now();
      *coreTime = (int)std::chrono::duration_cast<std::chrono::milliseconds>(
                      end - start)
                      .count();
      // printf("PassDone %d\n", *coreTime);
      backend.updateTexture(*image);
...
int mainGpu() {
  ...
  imageLoader.loadRGBA(IFRIT_ASSET_PATH "/fox_diffuse.png", &texFox, &texFoxH,
                       &texFoxW);
  ...
  stringstream ss;
  ss << "Ifrit-v2 CUDA (Resolution: " << DEMO_RESOLUTION << "x"
     << DEMO_RESOLUTION << ")";
