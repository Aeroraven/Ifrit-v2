...
void TileRasterWorker::tiledProcessing(TileRasterRenderer *renderer, bool clearDepth) IFRIT_AP_NOTHROW {
  ...
  for (int i = size_cast<int>(interpolatedVaryingsAddr.size()) - 1; i >= 0; i--) {
...
void TileRasterWorker::geometryProcessing(TileRasterRenderer *renderer) IFRIT_AP_NOTHROW {
  ...
  auto directTriangleSetup = [&]() {
    ...
    gather32Fp32_stride4_256((f32 *)candV1, tv1X, tv1Y, tv1Z, tv1W);
    __m256 tv1WInv = approxReciprocalNR_256(tv1W);
    tv1X = _mm256_mul_ps(tv1X, tv1WInv);
    tv1Y = _mm256_mul_ps(tv1Y, tv1WInv);
    tv1Z = _mm256_mul_ps(tv1Z, tv1WInv);

    gather32Fp32_stride4_256((f32 *)candV2, tv2X, tv2Y, tv2Z, tv2W);
    __m256 tv2WInv = approxReciprocalNR_256(tv2W);
    tv2X = _mm256_mul_ps(tv2X, tv2WInv);
    tv2Y = _mm256_mul_ps(tv2Y, tv2WInv);
    tv2Z = _mm256_mul_ps(tv2Z, tv2WInv);

    gather32Fp32_stride4_256((f32 *)candV3, tv3X, tv3Y, tv3Z, tv3W);
    __m256 tv3WInv = approxReciprocalNR_256(tv3W);
    ...
    __m256 aV3ySubV1yNeg = flipFp32Sign_256(aV3ySubV1y); // _mm256_sub_ps(_mm256_setzero_ps(), aV3ySubV1y);
    __m256 aEdgeCoef = _mm256_fmadd_ps(aV3ySubV1yNeg, aV2xSubV1xS, aEdgeCoefMul);

    __m256 aAr = approxReciprocalNR_256(aEdgeCoef); //_mm256_rcp_ps(aEdgeCoef);
    ...
    __m256 rF3Z = flipFp32Sign_256(_mm256_mul_ps(rF3Z_o, aAr));
    ...
    __m256 rF1Z = flipFp32Sign_256(_mm256_mul_ps(rF1Z_o, aAr));
    ...
    __m256 rF2Z = flipFp32Sign_256(_mm256_mul_ps(rF2Z_o, aAr));

    __m256 rE1X = _mm256_mul_ps(aCsFh, aV2ySubV1y);
    __m256 rE1Y = _mm256_mul_ps(aCsFw, aV2xSubV1x);
    __m256 rE1Z_o = _mm256_fmadd_ps(tv2X, tv1Y, flipFp32Sign_256(aS3));
    ...
    __m256 rE2Z_o = _mm256_fmadd_ps(tv3X, tv2Y, flipFp32Sign_256(aS1));
    ...
    __m256 rE3Z_o = _mm256_fmadd_ps(tv1X, tv3Y, flipFp32Sign_256(aS2));
      ...
      for (int x = leftBlock; x < rightBlock; x += 4) {
        ...
        for (int y = topBlock; y < bottomBlock; y += 2) {
          ...
          for (int i = 0; i < 8; i++) {
            ...
            if (criteriaTA[i] == -3) {
              npropBlock.tile = i32Pack(dwX[i], dwY[i]);
      ...
      for (int x = leftBlock; x < rightBlock; x += 2) {
        for (int y = topBlock; y < bottomBlock; y += 2) {
          ...
          for (int i = 0; i < 4; i++) {
            ...
            if (criteriaTA[i] == -3) {
              npropBlock.tile = i32Pack(dwX, dwY);
              coverQueue.push_back(npropBlock);
            } else {
              const auto subtilesXPerTile = context->numSubtilesPerTileX;
              ...
              for (int dx = subTileMinX; dx < subTileMaxX; dx += 2) {
                for (int dy = subTileMinY; dy < subTileMaxY; dy += 2) {
                  ...
                  if (_mm_testc_si128(accept128, _mm_set1_epi32(-1))) {
                    ...
                    if (accept128Mask) {
                      auto pixelId = dx + dy * TILE_RASTER_TEMP_CONST_MAXW;
                      npropPixel.tile = i32Pack(pixelId, accept128Mask & 0x0000FFFF);
                      coverQueue.push_back(npropPixel);
                    }
                  }
                }
          ...
          if (criteriaTA == 3) {
            ...
            for (int dx = subTileMinX; dx < subTileMaxX; dx += 2) {
              for (int dy = subTileMinY; dy < subTileMaxY; dy += 2) {
                ...
                if (mask) {
                  auto pixelId = dx + dy * TILE_RASTER_TEMP_CONST_MAXW;
                  npropPixel.tile = i32Pack(pixelId, mask & 0x0000FFFF);
                  coverQueue.push_back(npropPixel);
                }
              }
            }
...
void TileRasterWorker::fragmentProcessingSingleTile(TileRasterRenderer *renderer, bool clearedDepth,
  ...
  auto proposalProcessFunc = [&]<bool tpAlphaBlendEnable, IfritCompareOp tpDepthFunc, bool tpOnlyTaggingPass>(
    ...
    if (proposal.level == TileRasterLevel::PIXEL) {
      ...
      auto propX = i32UnpackFromi64First(proposalTile);
      auto propY = i32UnpackFromi64Second(proposalTile);
