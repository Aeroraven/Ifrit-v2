...
BufferManagerImpl::BufferManagerImpl(
    std::shared_ptr<TrivialBufferManager> wrapperObject) {
...
void BufferManagerImpl::destroyBuffer(const IfritBuffer &buffer) {
  ...
  if (maintained && buffer.id >= 0 &&
      !buffer.manager.owner_before(wrapperObject) &&
...
void BufferManagerImpl::mapBufferMemory(const IfritBuffer &buffer,
                                        void **ppData) {
...
void BufferManagerImpl::bufferData(const IfritBuffer &buffer, const void *src,
...
void BufferManagerImpl::bufferDataUnsafe(const IfritBuffer &buffer,
                                         const void *src, size_t offset,
                                         size_t size) IFRIT_AP_NOTHROW {
  memcpy(bufferMetadata[buffer.id].data.get() + offset, src, size);
}
} // namespace Ifrit::GraphicsBackend::SoftGraphics::BufferManager::Impl

namespace Ifrit::GraphicsBackend::SoftGraphics::BufferManager {
...
IfritBuffer
TrivialBufferManager::createBuffer(const IfritBufferCreateInfo &pCI) {
  if (!initialized) {
...
void TrivialBufferManager::mapBufferMemory(const IfritBuffer &buffer,
                                           void **ppData) {
  if (!initialized) {
    ifritError("Buffer manager not initialized");
  }
...
void TrivialBufferManager::bufferData(const IfritBuffer &buffer,
                                      const void *src, size_t offset,
                                      size_t size) {
  if (!initialized) {
    ifritError("Buffer manager not initialized");
