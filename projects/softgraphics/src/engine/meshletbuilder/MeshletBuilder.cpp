...
namespace Ifrit::Graphics::SoftGraphics::MeshletBuilder::Impl
{
    ...
    void initializeContext(MbContext* ctx, const VertexBuffer& vbuf, const std::vector<int>& ibuf, int posAttrId)
    {
        ...
        for (int i = 0; i < totVerts; i++)
        {
            ctx->remainActiveAdj[i] = Ifrit::SizeCast<int>(ctx->adjMaps[i].size());
        }
    }
    ...
    int meshletExpansion(const MbContext& ctx, const MbCurrentMeshlet& meshlet)
    {
        ...
        Vector3f clusterNormal   = { meshlet.sumNormal.x * meshlet.invSumNormal,
              meshlet.sumNormal.y * meshlet.invSumNormal, meshlet.sumNormal.z * meshlet.invSumNormal };
        ...
        auto     scoreFunc = [&](int triId, float cweight) {
            ...
            auto  spread =
                clusterNormal.x * tri.normal.x + clusterNormal.y * tri.normal.y + clusterNormal.z * tri.normal.z;
    ...
    void addTriangleToMeshlet(int triId, MbContext* ctx, MbCurrentMeshlet* meshlet)
    {
        ...
        auto normalLen = sqrt(meshlet->sumNormal.x * meshlet->sumNormal.x + meshlet->sumNormal.y * meshlet->sumNormal.y
...
namespace Ifrit::Graphics::SoftGraphics::MeshletBuilder
{
        ...
        Meshlet& outData, std::vector<int>& outVertexOffset, std::vector<int>& outIndexOffset, bool autoIncre)
    {
        ...
        for (auto& m : meshlets)
        {
            ...
            totalIndices += Ifrit::SizeCast<int>(m->ibufs.size());
        }
        outVertexOffset.push_back(totalVerts);
        outIndexOffset.push_back(totalIndices);
