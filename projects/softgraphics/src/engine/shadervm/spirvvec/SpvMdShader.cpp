...
namespace Ifrit::Graphics::SoftGraphics::ShaderVM::SpirvVec
{
    ...
    SpvVecRuntimeBackend::SpvVecRuntimeBackend(const ShaderRuntimeBuilder& runtime, std::vector<char> irByteCode)
    {
    ...
    void SpvVecRuntimeBackend::updateSymbolTable(bool isCopy)
    {
        ...
        for (int i = 0; i < SpVcQuadSize; i++)
        {
            ...
            this->symbolTables.outputs[i].resize(svmir->binds.outputVarSymbols[i].size());
            this->symbolTables.inputBytes[i].resize(svmir->binds.inputVarSymbols[i].size());
            this->symbolTables.outputBytes[i].resize(svmir->binds.outputVarSymbols[i].size());
        }
        for (int T = 0; T < SpVcQuadSize; T++)
        {
            for (int i = 0; i < svmir->binds.inputVarSymbols[T].size(); i++)
            {
                this->symbolTables.inputs[T][i]     = this->runtime->lookupSymbol(svmir->binds.inputVarSymbols[T][i]);
                this->symbolTables.inputBytes[T][i] = svmir->binds.inputSize[T][i];
            }
            for (int i = 0; i < svmir->binds.outputVarSymbols[T].size(); i++)
            {
                this->symbolTables.outputs[T][i]     = this->runtime->lookupSymbol(svmir->binds.outputVarSymbols[T][i]);
        ...
        for (int i = 0; i < svmir->binds.uniformVarSz.size(); i++)
        {
            ...
            this->symbolTables.uniform[loc] = { this->runtime->lookupSymbol(svmir->binds.uniformVarSymbols[i]),
                svmir->binds.uniformVarSz[i] };
        }
        this->symbolTables.entry = this->runtime->lookupSymbol(svmir->binds.mainFunction);
    }

    SpvVecFragmentShader::SpvVecFragmentShader(const ShaderRuntimeBuilder& runtime, std::vector<char> irByteCode)
        : SpvVecRuntimeBackend(runtime, irByteCode)
    {
        isThreadSafe         = false;
        forcedQuadInvocation = true;
    }
    SpvVecFragmentShader::SpvVecFragmentShader(const SpvVecFragmentShader& p) : SpvVecRuntimeBackend(p)
    {
        isThreadSafe                 = false;
        forcedQuadInvocation         = true;
    ...
    IFRIT_HOST void SpvVecFragmentShader::updateUniformData(int binding, int set, const void* pData)
    {
        auto& uniformData = symbolTables.uniform[{ binding, set }];
        memcpy(uniformData.first, pData, uniformData.second);
    }

    void SpvVecFragmentShader::execute(const void* varyings, void* colorOutput, float* fragmentDepth)
    {
        ifritError("Shader execution should be organized in quads");
    }
    IFRIT_HOST void SpvVecFragmentShader::executeInQuad(
        const void** varyings, void** colorOutput, float** fragmentDepth)
    {
    ...
    IFRIT_HOST std::vector<std::pair<int, int>> SpvVecFragmentShader::getUniformList()
    {
