...
#define EMIT_ERROR_CORE(...) ifritError(getParsingProgress(), __VA_ARGS__);
    ...
    std::string readString(const std::vector<uint32_t>& data, int& pos)
    {
        ...
        int   strlength = SizeCast<int>(strlen(str));
        pos += (strlength + 3) / 4;
...
#define MASK_CONT(v, x) v.continueMask->llvmVarName[(x)].arg
        ...
        CONV_PASS(OpBranchConditional)
        {
            ...
            if (CONTAIN_TAG(SPVC_BLOCK_LOOP_HEADER))
            { // Indicates a Loop break section
                // Write %1 to MASK_BR
                FORALL_CHAN([&](int x) {
        ...
        CONV_PASS(OpFNegate)
        {
            convPassMathUnaryGeneral(pc, params, ctx, irg, "fneg");
        }

        CONV_PASS(OpFAdd)
        {
            convPassMathBinaryGeneral(pc, params, ctx, irg, "fadd");
        }
        CONV_PASS(OpFSub)
        {
            convPassMathBinaryGeneral(pc, params, ctx, irg, "fsub");
        }
        CONV_PASS(OpFMul)
        {
            convPassMathBinaryGeneral(pc, params, ctx, irg, "fmul");
        }
        CONV_PASS(OpFDiv)
        {
            convPassMathBinaryGeneral(pc, params, ctx, irg, "fdiv");
        }

        CONV_PASS(OpIAdd)
        {
            convPassMathBinaryInteger(pc, params, ctx, irg, "add");
        }
        CONV_PASS(OpISub)
        {
            convPassMathBinaryInteger(pc, params, ctx, irg, "sub");
        }
        CONV_PASS(OpIMul)
        {
            convPassMathBinaryInteger(pc, params, ctx, irg, "mul");
        }
