...
namespace Ifrit::Graphics::SoftGraphics::ShaderVM::Spirv
{
    ...
    void SpvRuntimeBackend::updateSymbolTable(bool isCopy)
    {
        ...
        cSISize = SizeCast<int>(svmir->shaderMaps.inputVarSymbols.size());
        cSOSize = SizeCast<int>(svmir->shaderMaps.outputVarSymbols.size());
        for (int i = 0; i < svmir->shaderMaps.uniformSize.size(); i++)
        {
            std::pair<int, int> loc         = svmir->shaderMaps.uniformVarLoc[i];
            this->symbolTables.uniform[loc] = { this->runtime->lookupSymbol(svmir->shaderMaps.uniformVarSymbols[i]),
    ...
    SpvVertexShader::SpvVertexShader(const ShaderRuntimeBuilder& runtime, std::vector<char> irByteCode)
    ...
    SpvFragmentShader::SpvFragmentShader(const ShaderRuntimeBuilder& runtime, std::vector<char> irByteCode)
        : SpvRuntimeBackend(runtime, irByteCode)
    {
        isThreadSafe = false;
    }
    ...
    void SpvVertexShader::execute(const void* const* input, Vector4f* outPos, Vector4f* const* outVaryings)
    {
        // TODO: Input & Output
    ...
    IFRIT_DUAL void SpvRaygenShader::execute(const Vector3i& inputInvocation, const Vector3i& dimension, void* context)
    {
    ...
    SpvMissShader::SpvMissShader(const ShaderRuntimeBuilder& runtime, std::vector<char> irByteCode)
        : SpvRuntimeBackend(runtime, irByteCode)
    {
        isThreadSafe = false;
    }
    ...
    SpvClosestHitShader::SpvClosestHitShader(const ShaderRuntimeBuilder& runtime, std::vector<char> irByteCode)
        : SpvRuntimeBackend(runtime, irByteCode)
    {
        isThreadSafe = false;
    }
