...
IFRIT_APIDECL SinglePassHiZPass::SinglePassHiZPass(IApplication *app) {
  ...
  m_app = app;
  m_singlePassHiZPass =
      createComputePass(rhi, "CommonPass/SinglePassHzb.comp.glsl", 1, 6);
}

IFRIT_APIDECL bool
SinglePassHiZPass::checkResourceToRebuild(PerFrameData::SinglePassHiZData &data,
  ...
  if (!cond && (data.m_hizTexture->getWidth() != rWidth ||
                data.m_hizTexture->getHeight() != rHeight)) {
    cond = true;
  }
  if (!cond)
    return false;
...
IFRIT_APIDECL void SinglePassHiZPass::prepareHiZResources(
    PerFrameData::SinglePassHiZData &data, GPUTexture *depthTexture,
    GPUSampler *sampler, uint32_t rtWidth, uint32_t rtHeight) {
  auto rhi = m_app->getRhiLayer();
  auto maxMip = int(std::floor(std::log2(std::max(rtWidth, rtHeight))) + 1);
  data.m_hizTexture = rhi->createMipMapTexture(
      rtWidth, rtHeight, maxMip, RhiImageFormat::RHI_FORMAT_R32_SFLOAT,
      kbImUsage_UAV);

  data.m_hizRefs.resize(0);
  data.m_hizRefs.push_back(0);
  for (int i = 0; i < maxMip; i++) {
    auto bindlessId = rhi->registerUAVImage(data.m_hizTexture.get(),
                                            {static_cast<u32>(i), 0, 1, 1});
    data.m_hizRefs.push_back(bindlessId->getActiveId());
  }
  data.m_hizRefBuffer = rhi->createBufferDevice(u32Size * data.m_hizRefs.size(),
                                                kbBufUsage_SSBO_CopyDest);
  data.m_hizAtomics = rhi->createBufferDevice(u32Size * data.m_hizRefs.size(),
                                              kbBufUsage_SSBO_CopyDest);
  ...
  tq->runSyncCommand([&](const GPUCmdBuffer *cmd) {
    staged->cmdCopyToDevice(cmd, data.m_hizRefs.data(),
                            u32Size * data.m_hizRefs.size(), 0);
  });
  ...
  data.m_hizRefBufferId = rhi->registerStorageBuffer(data.m_hizRefBuffer.get());
...
IFRIT_APIDECL void
SinglePassHiZPass::runHiZPass(const PerFrameData::SinglePassHiZData &data,
