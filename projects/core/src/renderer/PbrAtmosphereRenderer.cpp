...
template <uint32_t N>
inline consteval ifloat3
interpolateSpectrum(const std::array<float, N> waveLengths,
                    const std::array<float, N> values, float scale) {
...
IFRIT_APIDECL void
PbrAtmosphereRenderer::preparePerframeData(PerFrameData &perframeData) {
  // todo: implement
  ...
  constexpr auto rayleighScattering =
      Util::PbrAtmoConstants::getRayleighScattering();
  ...
  constexpr auto absorptionExtinction =
      Util::PbrAtmoConstants::getAbsorptionExtinction();
  ...
  constexpr auto solarIrradianceFloat =
      Math::ConstFunc::convertArray<double, float, solarIrradiance.size()>(
          solarIrradiance);
  constexpr auto waveLengthToSample =
      Math::ConstFunc::uniformSampleIncl<float, solarIrradiance.size()>(
          Util::PbrAtmoConstants::kLambdaMin,
          Util::PbrAtmoConstants::kLambdaMax);
  ...
  data->m_atmosphereParams.rayleighScattering = toVec3(interpolateSpectrum(
      waveLengthToSample, rayleighScattering, Util::PbrAtmoConstants::km));

  data->m_atmosphereParams.mieDensity.layers[0] = mieLayer1;
  data->m_atmosphereParams.mieDensity.layers[1] = mieLayer1;
  data->m_atmosphereParams.mieScattering = toVec3(interpolateSpectrum(
      waveLengthToSample, mieScattering, Util::PbrAtmoConstants::km));

  data->m_atmosphereParams.mieExtinction = toVec3(interpolateSpectrum(
  ...
  data->m_atmosphereParams.absorptionExtinction = toVec3(interpolateSpectrum(
  ...
  data->m_transmittance =
      createPbrAtmoTex2D(AtmospherePASConfig::TRANSMITTANCE_TEXTURE_WIDTH,
                         AtmospherePASConfig::TRANSMITTANCE_TEXTURE_HEIGHT);
  data->m_deltaIrradiance =
      createPbrAtmoTex2D(AtmospherePASConfig::IRRADIANCE_TEXTURE_WIDTH,
                         AtmospherePASConfig::IRRADIANCE_TEXTURE_HEIGHT);
  data->m_irradiance =
      createPbrAtmoTex2D(AtmospherePASConfig::IRRADIANCE_TEXTURE_WIDTH,
                         AtmospherePASConfig::IRRADIANCE_TEXTURE_HEIGHT);
  data->m_scattering =
      createPbrAtmoTex3D(AtmospherePASConfig::SCATTERING_TEXTURE_WIDTH,
                         AtmospherePASConfig::SCATTERING_TEXTURE_HEIGHT,
  ...
  // bindless ids
  data->m_transmittanceId =
      rhi->registerUAVImage(data->m_transmittance.get(), {0, 0, 1, 1});
  data->m_deltaIrradianceId =
      rhi->registerUAVImage(data->m_deltaIrradiance.get(), {0, 0, 1, 1});
  data->m_irradianceId =
      rhi->registerUAVImage(data->m_irradiance.get(), {0, 0, 1, 1});
  data->m_scatteringId =
      rhi->registerUAVImage(data->m_scattering.get(), {0, 0, 1, 1});
  data->m_optionalSingleMieScatteringId = rhi->registerUAVImage(
      data->m_optionalSingleMieScattering.get(), {0, 0, 1, 1});
  data->m_deltaRayleighScatteringId = rhi->registerUAVImage(
      data->m_deltaRayleighScattering.get(), {0, 0, 1, 1});
  data->m_deltaMieScatteringId =
      rhi->registerUAVImage(data->m_deltaMieScattering.get(), {0, 0, 1, 1});
  data->m_deltaScatteringDensityId =
      rhi->registerUAVImage(data->m_deltaScatteringDensity.get(), {0, 0, 1, 1});
  data->m_deltaMultipleScatteringId = data->m_deltaRayleighScatteringId;

  // bindless ids for combined image sampler
  data->m_transmittanceCombSamplerId = rhi->registerCombinedImageSampler(
      data->m_transmittance.get(), data->m_sampler.get());
  data->m_deltaIrradianceCombSamplerId = rhi->registerCombinedImageSampler(
      data->m_deltaIrradiance.get(), data->m_sampler.get());
  data->m_irradianceCombSamplerId = rhi->registerCombinedImageSampler(
      data->m_irradiance.get(), data->m_sampler.get());
  data->m_scatteringCombSamplerId = rhi->registerCombinedImageSampler(
      data->m_scattering.get(), data->m_sampler.get());
  data->m_optionalSingleMieScatteringCombSamplerId =
      rhi->registerCombinedImageSampler(
          data->m_optionalSingleMieScattering.get(), data->m_sampler.get());
  data->m_deltaRayleighScatteringCombSamplerId =
      rhi->registerCombinedImageSampler(data->m_deltaRayleighScattering.get(),
                                        data->m_sampler.get());
  data->m_deltaMieScatteringCombSamplerId = rhi->registerCombinedImageSampler(
      data->m_deltaMieScattering.get(), data->m_sampler.get());
  data->m_deltaScatteringDensityCombSamplerId =
      rhi->registerCombinedImageSampler(data->m_deltaScatteringDensity.get(),
                                        data->m_sampler.get());
  data->m_deltaMultipleScatteringCombSamplerId =
      data->m_deltaRayleighScatteringCombSamplerId;
...
PbrAtmosphereRenderer::createShaderFromFile(
  ...
  return rhi->createShader(path, shaderCodeVec, entry, stage,
...
IFRIT_APIDECL void PbrAtmosphereRenderer::setupTransmittancePrecomputePass() {
  auto rhi = m_app->getRhiLayer();
  auto shader =
      createShaderFromFile("PAS.ComputeTransmittance.comp.glsl", "main",
...
IFRIT_APIDECL void PbrAtmosphereRenderer::setupIrradiancePrecomputePass() {
  auto rhi = m_app->getRhiLayer();
  auto shader =
      createShaderFromFile("PAS.ComputeIrradiance.comp.glsl", "main",
                           GraphicsBackend::Rhi::RhiShaderStage::Compute);
...
IFRIT_APIDECL void PbrAtmosphereRenderer::setupSingleScatteringPass() {
  auto rhi = m_app->getRhiLayer();
  auto shader =
      createShaderFromFile("PAS.ComputeSingleScattering.comp.glsl", "main",
                           GraphicsBackend::Rhi::RhiShaderStage::Compute);
  ...
  m_singleScatteringPass->setPushConstSize(sizeof(uint32_t) * 6 +
                                           sizeof(float4x4));
}
...
IFRIT_APIDECL void PbrAtmosphereRenderer::setupScatteringDensityPass() {
  auto rhi = m_app->getRhiLayer();
  auto shader =
      createShaderFromFile("PAS.ComputeScatteringDensity.comp.glsl", "main",
                           GraphicsBackend::Rhi::RhiShaderStage::Compute);
...
IFRIT_APIDECL void PbrAtmosphereRenderer::setupIndirectIrradiancePass() {
  auto rhi = m_app->getRhiLayer();
  auto shader =
      createShaderFromFile("PAS.ComputeIndirectIrradiance.comp.glsl", "main",
                           GraphicsBackend::Rhi::RhiShaderStage::Compute);
  ...
  m_indirectIrradiancePass->setPushConstSize(sizeof(uint32_t) * 7 +
                                             sizeof(float4x4));
}
...
IFRIT_APIDECL void PbrAtmosphereRenderer::setupMultipleScatteringPass() {
  auto rhi = m_app->getRhiLayer();
  auto shader =
      createShaderFromFile("PAS.ComputeMultipleScattering.comp.glsl", "main",
                           GraphicsBackend::Rhi::RhiShaderStage::Compute);
  ...
  m_multipleScatteringPass->setPushConstSize(sizeof(uint32_t) * 5 +
                                             sizeof(float4x4));
}

IFRIT_APIDECL std::unique_ptr<PbrAtmosphereRenderer::GPUCommandSubmission>
PbrAtmosphereRenderer::renderInternal(
  ...
  auto data = reinterpret_cast<PbrAtmospherePerframe *>(
      perframe.m_atmosphereData.get());
  ...
  m_transmittancePrecomputePass->setRecordFunction(
      [&](RhiRenderPassContext *ctx) {
        ctx->m_cmd->setPushConst(m_transmittancePrecomputePass, 0,
                                 sizeof(pcTransmittance), &pcTransmittance);
        constexpr auto wgX =
            divRoundUp(AtmospherePASConfig::TRANSMITTANCE_TEXTURE_WIDTH,
                       AtmospherePASConfig::cPasTransmittanceTGX);
        constexpr auto wgY =
            divRoundUp(AtmospherePASConfig::TRANSMITTANCE_TEXTURE_HEIGHT,
                       AtmospherePASConfig::cPasTransmittanceTGY);
  ...
  auto transmittanceBarrier =
      createUAVImageBarrier(data->m_transmittance.get());
  ...
  pcIrradiance.transmittanceRef =
      data->m_transmittanceCombSamplerId->getActiveId();
  ...
  m_irradiancePrecomputePass->setRecordFunction([&](RhiRenderPassContext *ctx) {
    ctx->m_cmd->setPushConst(m_irradiancePrecomputePass, 0,
                             sizeof(pcIrradiance), &pcIrradiance);
    constexpr auto wgX =
        divRoundUp(AtmospherePASConfig::IRRADIANCE_TEXTURE_WIDTH,
                   AtmospherePASConfig::cPasIrradianceTGX);
    constexpr auto wgY =
        divRoundUp(AtmospherePASConfig::IRRADIANCE_TEXTURE_HEIGHT,
                   AtmospherePASConfig::cPasIrradianceTGY);
    ctx->m_cmd->dispatch(wgX, wgY, 1);
  });
  ...
  pSingleScattering.deltaRayleigh =
      data->m_deltaRayleighScatteringId->getActiveId();
  ...
  pSingleScattering.singleMieScattering =
      data->m_optionalSingleMieScatteringId->getActiveId();
  pSingleScattering.transmittanceSampler =
      data->m_transmittanceCombSamplerId->getActiveId();
  m_singleScatteringPass->setRecordFunction([&](RhiRenderPassContext *ctx) {
    ctx->m_cmd->setPushConst(m_singleScatteringPass, 0,
                             sizeof(pSingleScattering), &pSingleScattering);
    constexpr auto wgX =
        divRoundUp(AtmospherePASConfig::SCATTERING_TEXTURE_WIDTH,
                   AtmospherePASConfig::cPasSingleScatteringTGX);
    constexpr auto wgY =
        divRoundUp(AtmospherePASConfig::SCATTERING_TEXTURE_HEIGHT,
                   AtmospherePASConfig::cPasSingleScatteringTGY);
    constexpr auto wgZ =
        divRoundUp(AtmospherePASConfig::SCATTERING_TEXTURE_DEPTH,
                   AtmospherePASConfig::cPasSingleScatteringTGZ);
    ctx->m_cmd->dispatch(wgX, wgY, wgZ);
  });

  // Step4. For higher order scattering
  auto singleRayleighScatterBarrier =
      createUAVImageBarrier(data->m_deltaRayleighScattering.get());
  auto singleMieScatterBarrier =
      createUAVImageBarrier(data->m_deltaMieScattering.get());
  ...
  auto deltaIrradianceBarrier =
      createUAVImageBarrier(data->m_deltaIrradiance.get());
  auto multipleScatteringBarrier =
      createUAVImageBarrier(data->m_deltaMultipleScattering.get());
  auto scatterDensityBarrier =
      createUAVImageBarrier(data->m_deltaScatteringDensity.get());
  ...
  auto recordCmdOrder = [&](uint32_t order) {
    ...
    pScatteringDensity.transmittanceSampler =
        data->m_transmittanceCombSamplerId->getActiveId();
    pScatteringDensity.singleRayleighScatterSampler =
        data->m_deltaRayleighScatteringCombSamplerId->getActiveId();
    pScatteringDensity.singleMieScatterSampler =
        data->m_deltaMieScatteringCombSamplerId->getActiveId();
    pScatteringDensity.multipleScatteringSampler =
        data->m_deltaMultipleScatteringCombSamplerId->getActiveId();
    pScatteringDensity.irradianceSampler =
        data->m_deltaIrradianceCombSamplerId->getActiveId();
    pScatteringDensity.scatterDensity =
        data->m_deltaScatteringDensityId->getActiveId();
    ...
    pIndirectIrradiance.deltaIrradiance =
        data->m_deltaIrradianceId->getActiveId();
    pIndirectIrradiance.irradiance = data->m_irradianceId->getActiveId();
    pIndirectIrradiance.singleRayleighScatteringSamp =
        data->m_deltaRayleighScatteringCombSamplerId->getActiveId();
    pIndirectIrradiance.singleMieScatteringSamp =
        data->m_deltaMieScatteringCombSamplerId->getActiveId();
    pIndirectIrradiance.multipleScatteringSamp =
        data->m_deltaMultipleScatteringCombSamplerId->getActiveId();
    ...
    pMultipleScattering.deltaMultipleScattering =
        data->m_deltaMultipleScatteringId->getActiveId();
    pMultipleScattering.scattering = data->m_scatteringId->getActiveId();
    pMultipleScattering.transmittanceSamp =
        data->m_transmittanceCombSamplerId->getActiveId();
    pMultipleScattering.scatteringDensitySamp =
        data->m_deltaScatteringDensityCombSamplerId->getActiveId();

    m_scatteringDensity->setRecordFunction([&](RhiRenderPassContext *ctx) {
      ctx->m_cmd->setPushConst(m_scatteringDensity, 0,
                               sizeof(pScatteringDensity), &pScatteringDensity);
      constexpr auto wgX =
          divRoundUp(AtmospherePASConfig::SCATTERING_TEXTURE_WIDTH,
                     AtmospherePASConfig::cPasScatteringDensityTGX);
      constexpr auto wgY =
          divRoundUp(AtmospherePASConfig::SCATTERING_TEXTURE_HEIGHT,
                     AtmospherePASConfig::cPasScatteringDensityTGY);
      constexpr auto wgZ =
          divRoundUp(AtmospherePASConfig::SCATTERING_TEXTURE_DEPTH,
                     AtmospherePASConfig::cPasScatteringDensityTGZ);
      ctx->m_cmd->dispatch(wgX, wgY, wgZ);
    });

    m_indirectIrradiancePass->setRecordFunction([&](RhiRenderPassContext *ctx) {
      ctx->m_cmd->setPushConst(m_indirectIrradiancePass, 0,
                               sizeof(pIndirectIrradiance),
                               &pIndirectIrradiance);
      constexpr auto wgX =
          divRoundUp(AtmospherePASConfig::IRRADIANCE_TEXTURE_WIDTH,
                     AtmospherePASConfig::cPasIndirectIrradianceTGX);
      constexpr auto wgY =
          divRoundUp(AtmospherePASConfig::IRRADIANCE_TEXTURE_HEIGHT,
                     AtmospherePASConfig::cPasIndirectIrradianceTGY);
      ctx->m_cmd->dispatch(wgX, wgY, 1);
    });

    m_multipleScatteringPass->setRecordFunction([&](RhiRenderPassContext *ctx) {
      ctx->m_cmd->setPushConst(m_multipleScatteringPass, 0,
                               sizeof(pMultipleScattering),
                               &pMultipleScattering);
      constexpr auto wgX =
          divRoundUp(AtmospherePASConfig::SCATTERING_TEXTURE_WIDTH,
                     AtmospherePASConfig::cPasMultipleScatteringTGX);
      constexpr auto wgY =
          divRoundUp(AtmospherePASConfig::SCATTERING_TEXTURE_HEIGHT,
                     AtmospherePASConfig::cPasMultipleScatteringTGY);
      constexpr auto wgZ =
          divRoundUp(AtmospherePASConfig::SCATTERING_TEXTURE_DEPTH,
                     AtmospherePASConfig::cPasMultipleScatteringTGZ);
      ctx->m_cmd->dispatch(wgX, wgY, wgZ);
    });
  };
  ...
  auto runCmdOrder = [&](const RhiCommandBuffer *cmd, uint32_t order) {
    ...
    cmd->resourceBarrier({singleRayleighScatterBarrier, singleMieScatterBarrier,
                          scatteringBarrier, irradianceBarrier,
                          deltaIrradianceBarrier, multipleScatteringBarrier});
    m_scatteringDensity->run(cmd, 0);
    cmd->resourceBarrier(
        {scatterDensityBarrier, irradianceBarrier, deltaIrradianceBarrier});
  ...
  // Final, run
  auto cq = m_app->getRhiLayer()->getQueue(
...
IFRIT_APIDECL PbrAtmosphereResourceDesc
PbrAtmosphereRenderer::getResourceDesc(PerFrameData &perframe) {
  ...
  desc.texMieScattering =
      data->m_optionalSingleMieScatteringCombSamplerId->getActiveId();
