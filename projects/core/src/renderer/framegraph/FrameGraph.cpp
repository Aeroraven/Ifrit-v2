...
IFRIT_APIDECL PassNodeId
FrameGraph::addPass(const std::string &name, FrameGraphPassType type,
...
IFRIT_APIDECL void FrameGraph::setImportedResource(ResourceNodeId id,
                                                   FgBuffer *buffer) {
...
IFRIT_APIDECL void
FrameGraph::setImportedResource(ResourceNodeId id, FgTexture *texture,
...
IFRIT_APIDECL void
FrameGraph::setExecutionFunction(PassNodeId id, std::function<void()> func) {
  m_passes[id].passFunction = func;
}

IFRIT_APIDECL CompiledFrameGraph
FrameGraphCompiler::compile(const FrameGraph &graph) {
  ...
  while (!rootPasses.empty()) {
    ...
    for (auto j = 0; auto &resId : graph.m_passes[passId0].outputResources) {
      void *resPtr = nullptr;
      if (graph.m_resources[resId].type ==
          FrameGraphResourceType::ResourceBuffer) {
        resPtr = graph.m_resources[resId].importedBuffer;
      } else if (graph.m_resources[resId].type ==
                 FrameGraphResourceType::ResourceTexture) {
      ...
      auto srcState = getOutputResouceState(
          pass.type, resType, graph.m_resources[resId].importedTexture,
...
GraphicsBackend::Rhi::RhiResourceBarrier
toRhiResBarrier(const CompiledFrameGraph::ResourceBarriers &barrier,
  ...
  if (barrier.enableTransitionBarrier) {
    ...
    resBarrier.m_transition.m_type =
        res.type == FrameGraphResourceType::ResourceBuffer
...
IFRIT_APIDECL void FrameGraphExecutor::executeInSingleCmd(
  ...
  for (auto &passId : compiledGraph.m_passTopoOrder) {
    ...
    for (auto i = 0; auto &resId : pass.outputResources) {
      ...
      if (compiledGraph.m_outputAliasedResourcesBarriers[passId][i]
              .enableTransitionBarrier) {
        ...
        //                .dstState));
        bool valid = false;
        auto resBarrier = toRhiResBarrier(
            compiledGraph.m_outputAliasedResourcesBarriers[passId][i],
    ...
    for (auto i = 0; auto &resId : pass.outputResources) {
      ...
      resState[resId] =
          compiledGraph.m_passResourceBarriers[passId][i].dstState;
      if (compiledGraph.m_passResourceBarriers[passId][i]
              .enableTransitionBarrier &&
          compiledGraph.m_passResourceBarriers[passId][i].enableUAVBarrier) {
        throw std::runtime_error(
            "Buggy design! Resource barrier can't be both transition and UAV");
      }
      bool valid = false;
      auto resBarrier =
          toRhiResBarrier(compiledGraph.m_passResourceBarriers[passId][i],
                          compiledGraph.m_graph->m_resources[resId], valid);
      if (valid)
