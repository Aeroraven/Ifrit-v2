...
#pragma once
#include "ifrit/core/renderer/ayanami/AyanamiGlobalDF.h"
...
namespace Ifrit::Core::Ayanami
{
        ...
        : m_rhi(rhi)
    {
        ...
        for (u32 i = 0; i < config.m_globalDFClipmapLevels; i++)
        {
            ...
            m_TestClipMaps[i]->m_clipmapTexture = rhi->CreateTexture3D("Ayanami_GlobalDF", resolution, resolution, resolution,
                Graphics::Rhi::RhiImageFormat::RhiImgFmt_R32_SFLOAT,
                Graphics::Rhi::RhiImageUsage::RHI_IMAGE_USAGE_STORAGE_BIT | Graphics::Rhi::RhiImageUsage::RHI_IMAGE_USAGE_SAMPLED_BIT, true);

            m_TestClipMaps[i]->m_clipmapSampler = rhi->CreateTrivialBilinearSampler(false);
            m_TestClipMaps[i]->m_clipmapSRV     = rhi->RegisterCombinedImageSampler(m_TestClipMaps[i]->m_clipmapTexture.get(),
                    m_TestClipMaps[i]->m_clipmapSampler.get());
        }
    }

    IFRIT_APIDECL void AyanamiGlobalDF::AddClipmapUpdate(const Graphics::Rhi::RhiCommandList* cmdList, u32 clipmapLevel, u32 perFrameDataId,
        u32 numMeshes, u32 meshDFListId)
    {
        if (m_updateClipmapPass == nullptr)
        {
            m_updateClipmapPass = RenderingUtil::CreateComputePass(m_rhi, "Ayanami/Ayanami.TrivialGlobalDFComposite.comp.glsl", 0, 16);
        }
        ...
        m_updateClipmapPass->SetRecordFunction([&](Graphics::Rhi::RhiRenderPassContext* ctx) {
            ...
            pc.m_GlobalDFWorldRangeMin = Vector4f(clipmap->m_worldBoundMin.x, clipmap->m_worldBoundMin.y, clipmap->m_worldBoundMin.z, 0);
            pc.m_GlobalDFWorldRangeMax = Vector4f(clipmap->m_worldBoundMax.x, clipmap->m_worldBoundMax.y, clipmap->m_worldBoundMax.z, 0);
        ...
        u32 perFrameDataId, u32 outTextureId, Vector2u outTextureSize)
    {
        ...
        {
            m_raymarchPass = RenderingUtil::CreateComputePass(m_rhi, "Ayanami/Ayanami.GlobalDFRayMarch.comp.glsl", 0, 17);
        }
        ...
        m_raymarchPass->SetRecordFunction([&](Graphics::Rhi::RhiRenderPassContext* ctx) {
            ...
            pc.m_GlobalDFBoxMax = Vector4f(clipmap->m_worldBoundMax.x, clipmap->m_worldBoundMax.y, clipmap->m_worldBoundMax.z, 0);
            pc.m_GlobalDFBoxMin = Vector4f(clipmap->m_worldBoundMin.x, clipmap->m_worldBoundMin.y, clipmap->m_worldBoundMin.z, 0);
