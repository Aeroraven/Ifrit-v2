...
IFRIT_APIDECL std::shared_ptr<MeshData> GLTFMesh::loadMesh() {
  ...
  auto texcoordData = reinterpret_cast<float *>(texcoordBuffer.data.data() + texcoordBufferView.byteOffset +
...
IFRIT_APIDECL void GLTFAsset::loadGLTF(AssetManager *m_manager) {
  ...
  std::function<void(int, float4x4)> traverseNode = [&](int nodeId, float4x4 parentTransform) {
    ...
    auto childTransform =
        Math::getTransformMat({scaleX, scaleY, scaleZ}, {translationX, translationY, translationZ}, {rotX, rotY, rotZ});
