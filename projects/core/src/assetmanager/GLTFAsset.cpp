...
namespace Ifrit::Core
{
        ...
        : m_asset(asset), m_meshId(meshId), m_primitiveId(primitiveId), m_nodeId(nodeId)
    {
        ...
        auto     localTransform    = Math::GetTransformMatrix({ scaleX, scaleY, scaleZ }, { posX, posY, posZ }, { rotX, rotY, rotZ });
    ...
    IFRIT_APIDECL Ref<MeshData> GLTFMesh::LoadMesh()
    {
        ...
        iAssertion(accessorPos.componentType == TINYGLTF_COMPONENT_TYPE_FLOAT, "GLTFMesh: component type not supported");
        iAssertion(accessorPos.type == TINYGLTF_TYPE_VEC3, "GLTFMesh: type not supported");
        iAssertion(accessorNormal.componentType == TINYGLTF_COMPONENT_TYPE_FLOAT, "GLTFMesh: component type not supported");
        iAssertion(accessorNormal.type == TINYGLTF_TYPE_VEC3, "GLTFMesh: type not supported");
        iAssertion(accessorTexcoord.componentType == TINYGLTF_COMPONENT_TYPE_FLOAT, "GLTFMesh: component type not supported");
        iAssertion(accessorTexcoord.type == TINYGLTF_TYPE_VEC2, "GLTFMesh: type not supported");
        iAssertion(accessorTangent.componentType == TINYGLTF_COMPONENT_TYPE_FLOAT, "GLTFMesh: component type not supported");
        ...
        auto  posData = reinterpret_cast<float*>(posBuffer.data.data() + posBufferView.byteOffset + accessorPos.byteOffset);
        auto  normalData =
            reinterpret_cast<float*>(normalBuffer.data.data() + normalBufferView.byteOffset + accessorNormal.byteOffset);
        auto texcoordData = reinterpret_cast<float*>(texcoordBuffer.data.data() + texcoordBufferView.byteOffset + accessorTexcoord.byteOffset);
        auto tangentData =
        ...
        auto indicesDataUshort = reinterpret_cast<u16*>(indicesBuffer.data.data() + indicesBufferView.byteOffset + accessorIndices.byteOffset);
    ...
    IFRIT_APIDECL void GLTFAsset::LoadGLTF(AssetManager* m_manager)
    {
        ...
        Fn<void(int, Matrix4x4f)> traverseNode = [&](int nodeId, Matrix4x4f parentTransform) {
            ...
            auto childTransform =
                Math::GetTransformMatrix({ scaleX, scaleY, scaleZ }, { translationX, translationY, translationZ }, { rotX, rotY, rotZ });
    ...
    IFRIT_APIDECL std::vector<String> GLTFAssetImporter::GetSupportedExtensionNames()
    {
        return { ".gltf", ".glb" };
    }
