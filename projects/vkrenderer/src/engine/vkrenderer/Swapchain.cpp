...
namespace Ifrit::Engine::VkRenderer{
    IFRIT_APIDECL void Swapchain::init(){
        ...
        vkrVulkanAssert(vkCreateWin32SurfaceKHR(m_context->getInstance(), &surfaceCI, nullptr, &m_surface), "Failed to create window surface");
#else
        ...
        for(int i = 0; i < queueData.m_queueFamilies.size(); i++){
            ...
            vkGetPhysicalDeviceSurfaceSupportKHR(m_context->getPhysicalDevice(), i, m_surface,&presentSupport);
            {
                ...
                vkEnumerateDeviceExtensionProperties(m_context->getPhysicalDevice(), nullptr, &extensionCount, availableExtensions.data());
        ...
        vkGetPhysicalDeviceSurfaceCapabilitiesKHR(m_context->getPhysicalDevice(), m_surface, &m_supportDetails.capabilities);
        {
            ...
            vkGetPhysicalDeviceSurfaceFormatsKHR(m_context->getPhysicalDevice(), m_surface, &formatCount, m_supportDetails.formats.data());
        }
        {
            ...
            vkGetPhysicalDeviceSurfacePresentModesKHR(m_context->getPhysicalDevice(), m_surface, &presentModeCount, m_supportDetails.presentModes.data());
        }
        ...
        vkrVulkanAssert(vkCreateSwapchainKHR(m_context->getDevice(), &swapchainCI, nullptr, &m_swapchain), "Failed to create swapchain");
        ...
        for(int i = 0; i < imageCount; i++){
            ...
            vkrVulkanAssert(vkCreateImageView(m_context->getDevice(), &imageViewCI, nullptr, &m_imageViews[i]), "Failed to create image view");
        }
    ...
    IFRIT_APIDECL Swapchain::Swapchain(EngineContext* context): m_context(context){
        init();
    }
