...
IFRIT_APIDECL void PipelineBarrier::addMemoryBarrier(VkMemoryBarrier barrier) { m_memoryBarriers.push_back(barrier); }
...
IFRIT_APIDECL void CommandBuffer::pipelineBarrier(const PipelineBarrier &barrier) const {
  std::abort();
  vkCmdPipelineBarrier(m_commandBuffer, barrier.m_srcStage, barrier.m_dstStage, barrier.m_dependencyFlags,
                       size_cast<int>(barrier.m_memoryBarriers.size()), barrier.m_memoryBarriers.data(),
                       size_cast<int>(barrier.m_bufferMemoryBarriers.size()), barrier.m_bufferMemoryBarriers.data(),
                       size_cast<int>(barrier.m_imageMemoryBarriers.size()), barrier.m_imageMemoryBarriers.data());
}

IFRIT_APIDECL void CommandBuffer::setViewports(const std::vector<Rhi::RhiViewport> &viewport) const {
...
IFRIT_APIDECL void CommandBuffer::setScissors(const std::vector<Rhi::RhiScissor> &scissor) const {
...
IFRIT_APIDECL void CommandBuffer::draw(uint32_t vertexCount, uint32_t instanceCount, uint32_t firstVertex,
                                       uint32_t firstInstance) const {
  vkCmdDraw(m_commandBuffer, vertexCount, instanceCount, firstVertex, firstInstance);
}

IFRIT_APIDECL void CommandBuffer::drawIndexed(uint32_t indexCount, uint32_t instanceCount, uint32_t firstIndex,
                                              int32_t vertexOffset, uint32_t firstInstance) const {
  vkCmdDrawIndexed(m_commandBuffer, indexCount, instanceCount, firstIndex, vertexOffset, firstInstance);
}

IFRIT_APIDECL void CommandBuffer::dispatch(uint32_t groupCountX, uint32_t groupCountY, uint32_t groupCountZ) const {
  vkCmdDispatch(m_commandBuffer, groupCountX, groupCountY, groupCountZ);
}

IFRIT_APIDECL void CommandBuffer::drawMeshTasks(uint32_t groupCountX, uint32_t groupCountY,
                                                uint32_t groupCountZ) const {
  m_context->getExtensionFunction().p_vkCmdDrawMeshTasksEXT(m_commandBuffer, groupCountX, groupCountY, groupCountZ);
}

IFRIT_APIDECL void CommandBuffer::drawMeshTasksIndirect(const Rhi::RhiBuffer *buffer, uint32_t offset,
...
IFRIT_APIDECL void CommandBuffer::copyBuffer(const Rhi::RhiBuffer *srcBuffer, const Rhi::RhiBuffer *dstBuffer,
                                             uint32_t size, uint32_t srcOffset, uint32_t dstOffset) const {
...
// Rhi compatible
IFRIT_APIDECL void CommandBuffer::imageBarrier(Rhi::RhiTexture *texture, Rhi::RhiResourceState2 src,
  ...
  case Rhi::RhiResourceState2::Undefined:
    ...
    barrier.srcAccessMask = VK_ACCESS_HOST_WRITE_BIT | VK_ACCESS_HOST_READ_BIT | VK_ACCESS_TRANSFER_WRITE_BIT |
  ...
  case Rhi::RhiResourceState2::Common:
    ...
    barrier.srcAccessMask = VK_ACCESS_SHADER_WRITE_BIT | VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_MEMORY_READ_BIT |
  ...
  case Rhi::RhiResourceState2::ShaderRead:
    ...
    barrier.dstAccessMask = VK_ACCESS_SHADER_WRITE_BIT | VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_MEMORY_READ_BIT |
                            VK_ACCESS_MEMORY_WRITE_BIT | VK_ACCESS_TRANSFER_WRITE_BIT | VK_ACCESS_TRANSFER_READ_BIT;
    break;
  ...
  case Rhi::RhiResourceState2::Undefined:
    ...
    barrier.dstAccessMask = VK_ACCESS_HOST_WRITE_BIT | VK_ACCESS_HOST_READ_BIT | VK_ACCESS_TRANSFER_WRITE_BIT |
                            VK_ACCESS_TRANSFER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT | VK_ACCESS_SHADER_READ_BIT |
                            VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT | VK_ACCESS_COLOR_ATTACHMENT_READ_BIT |
                            VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT | VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_READ_BIT |
                            VK_ACCESS_MEMORY_READ_BIT | VK_ACCESS_MEMORY_WRITE_BIT;
...
IFRIT_APIDECL void CommandBuffer::attachBindlessReferenceGraphics(Rhi::RhiGraphicsPass *pass, uint32_t setId,
...
IFRIT_APIDECL void CommandBuffer::attachBindlessReferenceCompute(Rhi::RhiComputePass *pass, uint32_t setId,
                                                                 Rhi::RhiBindlessDescriptorRef *ref) const {
  auto bindless = checked_cast<DescriptorBindlessIndices>(ref);
...
IFRIT_APIDECL void CommandBuffer::attachVertexBufferView(const Rhi::RhiVertexBufferView &view) const {
  ...
  exfun.p_vkCmdSetVertexInputEXT(m_commandBuffer, size_cast<uint32_t>(vxDesc->m_bindings.size()),
                                 vxDesc->m_bindings.data(), size_cast<uint32_t>(vxDesc->m_attributes.size()),
                                 vxDesc->m_attributes.data());
}

IFRIT_APIDECL void CommandBuffer::attachVertexBuffers(uint32_t firstSlot,
                                                      const std::vector<Rhi::RhiBuffer *> &buffers) const {
  ...
  vkCmdBindVertexBuffers(m_commandBuffer, firstSlot, size_cast<uint32_t>(buffers.size()), vxbuffers.data(),
                         offsets.data());
}

IFRIT_APIDECL void CommandBuffer::drawInstanced(uint32_t vertexCount, uint32_t instanceCount, uint32_t firstVertex,
                                                uint32_t firstInstance) const {
  vkCmdDraw(m_commandBuffer, vertexCount, instanceCount, firstVertex, firstInstance);
}

IFRIT_APIDECL void CommandBuffer::bufferClear(const Rhi::RhiBuffer *buffer, uint32_t val) const {
  auto buf = checked_cast<SingleBuffer>(buffer)->getBuffer();
  vkCmdFillBuffer(m_commandBuffer, buf, 0, VK_WHOLE_SIZE, val);
}

IFRIT_APIDECL void CommandBuffer::dispatchIndirect(const Rhi::RhiBuffer *buffer, uint32_t offset) const {
  auto buf = checked_cast<SingleBuffer>(buffer)->getBuffer();
  vkCmdDispatchIndirect(m_commandBuffer, buf, offset);
}

IFRIT_APIDECL void CommandBuffer::setPushConst(Rhi::RhiComputePass *pass, uint32_t offset, uint32_t size,
                                               const void *data) const {
  auto computePass = checked_cast<ComputePass>(pass);
  vkCmdPushConstants(m_commandBuffer, computePass->getPipelineLayout(), VK_SHADER_STAGE_ALL, offset, size, data);
};
IFRIT_APIDECL void CommandBuffer::setPushConst(Rhi::RhiGraphicsPass *pass, uint32_t offset, uint32_t size,
                                               const void *data) const {
  auto graphicsPass = checked_cast<GraphicsPass>(pass);
  vkCmdPushConstants(m_commandBuffer, graphicsPass->getPipelineLayout(), VK_SHADER_STAGE_ALL, offset, size, data);
};

IFRIT_APIDECL void CommandBuffer::clearUAVImageFloat(const Rhi::RhiTexture *texture,
...
void _resourceStateToAccessMask(Rhi::RhiResourceState2 state, VkAccessFlags &dstAccessMask) {
  switch (state) {
  case Rhi::RhiResourceState2::Undefined:
...
void _resourceStateToImageLayout(Rhi::RhiResourceState2 state, VkImageLayout &dstLayout) {
  switch (state) {
  case Rhi::RhiResourceState2::Undefined:
...
IFRIT_APIDECL
void CommandBuffer::resourceBarrier(const std::vector<Rhi::RhiResourceBarrier> &barriers) const {
  ...
  for (int i = 0; i < barriers.size(); i++) {
    ...
    if (barrier.m_type == Rhi::RhiBarrierType::Transition) {
      ...
      if (resourceType == Rhi::RhiResourceType::Buffer) {
        ...
        bufferBarrier.buffer = checked_cast<SingleBuffer>(barrier.m_transition.m_buffer)->getBuffer();
        ...
        if (srcState != barrier.m_transition.m_texture->getState() && srcState != Rhi::RhiResourceState2::Undefined) {
        ...
        auto image = checked_cast<SingleDeviceImage>(barrier.m_transition.m_texture);
        auto aspect = image->getAspect();
        VkImageMemoryBarrier imageBarrier{};
        imageBarrier.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
        imageBarrier.image = checked_cast<SingleDeviceImage>(barrier.m_transition.m_texture)->getImage();
      ...
      if (resourceType == Rhi::RhiResourceType::Buffer) {
        VkBufferMemoryBarrier bufferBarrier{};
        bufferBarrier.sType = VK_STRUCTURE_TYPE_BUFFER_MEMORY_BARRIER;
        bufferBarrier.buffer = checked_cast<SingleBuffer>(barrier.m_uav.m_buffer)->getBuffer();
        bufferBarrier.offset = 0;
        bufferBarrier.size = VK_WHOLE_SIZE;
        ...
        if (barrier.m_uav.m_texture->getState() != Rhi::RhiResourceState2::Common) {
          iError("Texture state mismatch, expected:{} actual:{}", i32(Rhi::RhiResourceState2::Common),
                 i32(barrier.m_uav.m_texture->getState()));
          std::abort();
        }
        _setTextureState(barrier.m_uav.m_texture, Rhi::RhiResourceState2::Common);

        // WARNING: subresource unspecified
        VkImageMemoryBarrier imageBarrier{};
        imageBarrier.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
        imageBarrier.image = checked_cast<SingleDeviceImage>(barrier.m_uav.m_texture)->getImage();
  ...
  vkCmdPipelineBarrier(m_commandBuffer, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, 0, 0,
                       nullptr, size_cast<int>(bufferBarriers.size()), bufferBarriers.data(),
...
// Class: Queue
IFRIT_APIDECL Queue::Queue(EngineContext *ctx, VkQueue queue, uint32_t family, VkQueueFlags capability)
...
IFRIT_APIDECL TimelineSemaphoreWait Queue::submitCommand(const std::vector<TimelineSemaphoreWait> &waitSemaphores,
...
void Queue::runSyncCommand(std::function<void(const Rhi::RhiCommandBuffer *)> func) {
...
std::unique_ptr<Rhi::RhiTaskSubmission> Queue::runAsyncCommand(std::function<void(const Rhi::RhiCommandBuffer *)> func,
