...
namespace Ifrit::Demo::OglBenchmarking {
	int mainCpu() {
		...
		for (int i = 0; i < procNormal.size(); i++) {
			normal4[i] = ifloat4(procNormal[i].x, procNormal[i].y, procNormal[i].z, 0.5f);
		}
		...
		glBufferData(GL_ARRAY_BUFFER, pos4.size() * sizeof(ifloat4), pos4.data(), GL_STATIC_DRAW);
		...
		glBufferData(GL_ARRAY_BUFFER, normal4.size() * sizeof(ifloat4), normal4.data(), GL_STATIC_DRAW);
