...
public:
  ...
  IFRIT_APIDECL void bindMeshShader(MeshShader *meshShader,
                                    VaryingDescriptor &varyingDescriptor,
                                    iint3 localSize);
  IFRIT_APIDECL void bindTaskShader(TaskShader *taskShader,
                                    VaryingDescriptor &varyingDescriptor);

  IFRIT_APIDECL void createTexture(int slotId,
                                   const IfritImageCreateInfo &createInfo);
  IFRIT_APIDECL void createSampler(int slotId,
                                   const IfritSamplerT &samplerState);
  IFRIT_APIDECL void generateMipmap(int slotId, IfritFilter filter);
  IFRIT_APIDECL void blitImage(int srcSlotId, int dstSlotId,
                               const IfritImageBlit &region,
                               IfritFilter filter);
  IFRIT_APIDECL void
  copyHostBufferToImage(void *srcBuffer, int dstSlot,
                        const std::vector<IfritBufferImageCopy> &regions);

  IFRIT_APIDECL void createBuffer(int slotId, int bufSize);
  IFRIT_APIDECL void copyHostBufferToBuffer(const void *srcBuffer, int dstSlot,
                                            int size);
  ...
  IFRIT_APIDECL void setClearValues(const std::vector<ifloat4> &clearColors,
                                    float clearDepth);
