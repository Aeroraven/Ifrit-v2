...
namespace Ifrit::Engine::ShaderVM::Spirv {
	...
	void SpvVMReader::initializeContext(SpvVMContext* outContext) {
		int v = 1;
		outContext->endianParserNative = ((*(char*)(&v)) == 1) ? IFSP_LITTLE_ENDIAN : IFSP_BIG_ENDIAN;
	}
	void SpvVMReader::parseByteCode(const char* byteCode, size_t length, SpvVMContext* outContext) {
		...
		auto readWord = [&](const uint32_t*& x) {
			auto val = *x; x++;
			if (doBswap) val = Ifrit::Core::Utility::byteSwapU32(val);
			return val;
		};
		...
		pCur++;
		if (outContext->headerMagic == spv::MagicNumber) outContext->endianBytecode = outContext->endianParserNative;
		else (doBswap = true), outContext->endianBytecode = (outContext->endianParserNative == IFSP_LITTLE_ENDIAN) ? IFSP_BIG_ENDIAN : IFSP_LITTLE_ENDIAN;
