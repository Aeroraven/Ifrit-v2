...
inline int cvrsQueuePrimitiveId(int x) {
  constexpr int mask =
      ((1 << (sizeof(int) * 8 - TileRasterContext::workerReprBits)) - 1);
  return x & mask;
}
...
inline void getAcceptRejectCoords(vfloat3 edgeCoefs[3], int chosenCoordTR[3],
                                  int chosenCoordTA[3]) IFRIT_AP_NOTHROW {
  constexpr const int VLB = 0, VLT = 1, VRT = 2, VRB = 3;
  for (int i = 0; i < 3; i++) {
    bool normalRight = edgeCoefs[i].x < 0;
    bool normalDown = edgeCoefs[i].y < 0;
    chosenCoordTR[i] =
        normalRight ? (normalDown ? VRB : VRT) : (normalDown ? VLB : VLT);
    chosenCoordTA[i] =
        normalRight ? (normalDown ? VLT : VLB) : (normalDown ? VRT : VRB);
  }
}
...
void TileRasterWorker::run() IFRIT_AP_NOTHROW {
  while (true) {
    ...
    if (curStatus == TileRasterStage::COMPLETED ||
        curStatus == TileRasterStage::IDLE) {
...
void TileRasterWorker::drawCall(bool withClear) IFRIT_AP_NOTHROW {
  ...
  if (withClear) {
    ...
    ptrCtx->frameBuffer->getDepthAttachment()->clearImageMultithread(
        depth, workerId, TOTAL_THREADS);
    ptrCtx->frameBuffer->getColorAttachment(0)->clearImageZeroMultiThread(
        workerId, TOTAL_THREADS);
  }
  vertexProcessing(rawRenderer);
  rawRenderer->statusTransitionBarrier2(TileRasterStage::VERTEX_SHADING_SYNC,
                                        TileRasterStage::GEOMETRY_PROCESSING);
  ...
  auto &w2 = ptrCtx->coverQueue[workerId];
  ...
  rawRenderer->statusTransitionBarrier2(
      TileRasterStage::GEOMETRY_PROCESSING_SYNC,
      TileRasterStage::RASTERIZATION);
  tiledProcessing(rendererReference, withClear);
}
uint32_t
TileRasterWorker::triangleHomogeneousClip(const int primitiveId, vfloat4 v1,
                                          vfloat4 v2,
                                          vfloat4 v3) IFRIT_AP_NOTHROW {
  ...
  for (int i = 0; i < retCnt - 2; i++) {
    ...
    auto edgeFunctionSimdVec = [](const vfloat4 &a, const vfloat4 &b,
                                  const vfloat4 &c) {
    ...
    atriShade.f3 = {sV2V1y * csInvX, sV2V1x * csInvY,
                    (-tv1.x * sV2V1y - tv1.y * sV2V1x + s3) * ar,
                    tv3.z * tv3.w};
    atriShade.f1 = {sV3V2y * csInvX, sV3V2x * csInvY,
                    (-tv2.x * sV3V2y - tv2.y * sV3V2x + s1) * ar,
                    tv1.z * tv1.w};
    atriShade.f2 = {sV1V3y * csInvX, sV1V3x * csInvY,
                    (-tv3.x * sV1V3y - tv3.y * sV1V3x + s2) * ar,
                    tv2.z * tv2.w};
    atriRaster.e1 = {csFh * sV2V1y, csFw * sV2V1x,
                     csFhFw * (tv2.x * tv1.y - tv1.x * tv2.y + s3 - EPS)};
    atriRaster.e2 = {csFh * sV3V2y, csFw * sV3V2x,
                     csFhFw * (tv3.x * tv2.y - tv2.x * tv3.y + s1 - EPS)};
    atriRaster.e3 = {csFh * sV1V3y, csFw * sV1V3x,
                     csFhFw * (tv1.x * tv3.y - tv3.x * tv1.y + s2 - EPS)};
    ...
    atriShade.f1.w = tv1.z * tv1.w;
    atriShade.f2.w = tv2.z * tv2.w;
    atriShade.f3.w = tv3.z * tv3.w;
    ...
    vfloat4 bboxR =
        round(fma(bbox, smallTriangleCullVecP, vfloat4(-0.5f))); // -0.5f;
    ...
    context->assembledTrianglesShade[workerId].emplace_back(
        std::move(atriShade));
...
bool TileRasterWorker::triangleFrustumClip(Ifrit::Math::SIMD::vfloat4 v1,
                                           Ifrit::Math::SIMD::vfloat4 v2,
                                           Ifrit::Math::SIMD::vfloat4 v3,
                                           vfloat4 &bbox) IFRIT_AP_NOTHROW {
...
bool TileRasterWorker::triangleCulling(vfloat4 v1, vfloat4 v2,
                                       vfloat4 v3) IFRIT_AP_NOTHROW {
  float d1 = (v1.x * v2.y);
  float d2 = (v2.x * v3.y);
  float d3 = (v3.x * v1.y);
...
void TileRasterWorker::executeBinner(
    const int primitiveId, const AssembledTriangleProposalRasterStage &atp,
    vfloat4 bbox) IFRIT_AP_NOTHROW {
  constexpr const int VLB = 0, VLT = 1, VRT = 2, VRB = 3;
  ...
  constexpr float tileSizeX = TileRasterContext::tileWidth;
  constexpr float tileSizeY = TileRasterContext::tileWidth;
  ...
  for (int y = tileMiny; y <= tileMaxy; y++) {
    ...
    for (int x = tileMinx; x <= tileMaxx; x++) {
      ...
      tileCoords[VLT] = {dTx, dTy};
      tileCoords[VLB] = {dTx, dTy + tileSizeY};
      tileCoords[VRB] = {dTx + tileSizeX, dTy + tileSizeY};
      tileCoords[VRT] = {dTx + tileSizeX, dTy};
      ...
      vfloat3 tileCoordsX_TR = vfloat3(tileCoords[chosenCoordTR[0]].x,
                                       tileCoords[chosenCoordTR[1]].x,
                                       tileCoords[chosenCoordTR[2]].x);
      vfloat3 tileCoordsY_TR = vfloat3(tileCoords[chosenCoordTR[0]].y,
                                       tileCoords[chosenCoordTR[1]].y,
                                       tileCoords[chosenCoordTR[2]].y);
      vfloat3 tileCoordsX_TA = vfloat3(tileCoords[chosenCoordTA[0]].x,
                                       tileCoords[chosenCoordTA[1]].x,
                                       tileCoords[chosenCoordTA[2]].x);
      vfloat3 tileCoordsY_TA = vfloat3(tileCoords[chosenCoordTA[0]].y,
                                       tileCoords[chosenCoordTA[1]].y,
                                       tileCoords[chosenCoordTA[2]].y);
      vfloat3 criteriaTRLocal =
          fma(coefX, tileCoordsX_TR, fma(coefY, tileCoordsY_TR, coefZ));
      vfloat3 criteriaTALocal =
          fma(coefX, tileCoordsX_TA, fma(coefY, tileCoordsY_TA, coefZ));
      criteriaTR += cmpltElements(criteriaTRLocal, zeroVec);
      criteriaTA += cmpltElements(criteriaTALocal, zeroVec);
#else
      for (int i = 0; i < 3; i++) {
        float criteriaTRLocal =
            edgeCoefs[i].x * tileCoords[chosenCoordTR[i]].x +
            edgeCoefs[i].y * tileCoords[chosenCoordTR[i]].y + edgeCoefs[i].z;
        float criteriaTALocal =
            edgeCoefs[i].x * tileCoords[chosenCoordTA[i]].x +
      ...
      if (criteriaTR != 3)
        continue;
      if (criteriaTA == 3) {
        cvQueue[getTileID(x, y)].emplace_back(
            cvrsQueuePack(workerId, primitiveId));
      } else {
        rsQueue[getTileID(x, y)].emplace_back(
            cvrsQueuePack(workerId, primitiveId));
      }
    }
  }
}
void TileRasterWorker::tiledProcessing(TileRasterRenderer *renderer,
  ...
  auto reqDeterministic =
      context->optForceDeterministic || context->blendState.blendEnable;
  if (reqDeterministic) {
...
void TileRasterWorker::vertexProcessing(TileRasterRenderer *renderer)
    IFRIT_AP_NOTHROW {
  auto varyingCnts = context->varyingDescriptor->getVaryingCounts();
...
void TileRasterWorker::geometryProcessing(TileRasterRenderer *renderer)
    IFRIT_AP_NOTHROW {
  ...
  auto directTriangleSetup = [&]() {
    ...
    __m256 aV3ySubV1yNeg = FastUtil::flipFp32Sign_256(
        aV3ySubV1y); // _mm256_sub_ps(_mm256_setzero_ps(), aV3ySubV1y);
    __m256 aEdgeCoef =
        _mm256_fmadd_ps(aV3ySubV1yNeg, aV2xSubV1xS, aEdgeCoefMul);

    __m256 aAr =
        FastUtil::approxReciprocalNR_256(aEdgeCoef); //_mm256_rcp_ps(aEdgeCoef);
    ...
    __m256 rE1Z_o =
        _mm256_fmadd_ps(tv2X, tv1Y, FastUtil::flipFp32Sign_256(aS3));
    __m256 rE1Z_q = _mm256_mul_ps(tv1X, tv2Y);
    __m256 rE1Z =
        _mm256_sub_ps(_mm256_sub_ps(rE1Z_o, rE1Z_q), _mm256_set1_ps(EPS));
    rE1Z = _mm256_mul_ps(rE1Z, aCsFhFw);

    __m256 rE2X = _mm256_mul_ps(aCsFh, aV3ySubV2y);
    __m256 rE2Y = _mm256_mul_ps(aCsFw, aV3xSubV2x);
    __m256 rE2Z_o =
        _mm256_fmadd_ps(tv3X, tv2Y, FastUtil::flipFp32Sign_256(aS1));
    __m256 rE2Z_q = _mm256_mul_ps(tv2X, tv3Y);
    __m256 rE2Z =
        _mm256_sub_ps(_mm256_sub_ps(rE2Z_o, rE2Z_q), _mm256_set1_ps(EPS));
    rE2Z = _mm256_mul_ps(rE2Z, aCsFhFw);

    __m256 rE3X = _mm256_mul_ps(aCsFh, aV1ySubV3y);
    __m256 rE3Y = _mm256_mul_ps(aCsFw, aV1xSubV3x);
    __m256 rE3Z_o =
        _mm256_fmadd_ps(tv1X, tv3Y, FastUtil::flipFp32Sign_256(aS2));
    __m256 rE3Z_q = _mm256_mul_ps(tv3X, tv1Y);
    __m256 rE3Z =
        _mm256_sub_ps(_mm256_sub_ps(rE3Z_o, rE3Z_q), _mm256_set1_ps(EPS));
    ...
    __m256 fcullBMaxZMask =
        _mm256_cmp_ps(bboxMaxZ, _mm256_setzero_ps(), _CMP_LT_OQ);
    __m256 fcullBMinZMask =
        _mm256_cmp_ps(bboxMinZ, _mm256_set1_ps(1.0f), _CMP_GT_OQ);
    __m256 fcullBMinXMask =
        _mm256_cmp_ps(bboxMinX, _mm256_set1_ps(1.0f), _CMP_GT_OQ);
    __m256 fcullBMaxXMask =
        _mm256_cmp_ps(bboxMaxX, _mm256_set1_ps(-1.0f), _CMP_LT_OQ);
    __m256 fcullBMinYMask =
        _mm256_cmp_ps(bboxMinY, _mm256_set1_ps(1.0f), _CMP_GT_OQ);
    __m256 fcullBMaxYMask =
        _mm256_cmp_ps(bboxMaxY, _mm256_set1_ps(-1.0f), _CMP_LT_OQ);
    ...
    fcullMask =
        _mm256_xor_ps(fcullMask, _mm256_castsi256_ps(_mm256_set1_epi32(-1)));
    ...
    bboxMinXR =
        _mm256_round_ps(bboxMinXR, _MM_FROUND_TO_NEG_INF | _MM_FROUND_NO_EXC);
    bboxMinYR =
        _mm256_round_ps(bboxMinYR, _MM_FROUND_TO_NEG_INF | _MM_FROUND_NO_EXC);
    bboxMaxXR =
        _mm256_round_ps(bboxMaxXR, _MM_FROUND_TO_NEG_INF | _MM_FROUND_NO_EXC);
    bboxMaxYR =
        _mm256_round_ps(bboxMaxYR, _MM_FROUND_TO_NEG_INF | _MM_FROUND_NO_EXC);
    ...
    bboxMask =
        _mm256_xor_ps(bboxMask, _mm256_castsi256_ps(_mm256_set1_epi32(-1)));
    if (_mm256_testz_ps(bboxMask, bboxMask))
      return;

    bboxMinX =
        _mm256_fmadd_ps(bboxMinX, _mm256_set1_ps(0.5f), _mm256_set1_ps(0.5f));
    bboxMinY =
        _mm256_fmadd_ps(bboxMinY, _mm256_set1_ps(0.5f), _mm256_set1_ps(0.5f));
    bboxMaxX =
        _mm256_fmadd_ps(bboxMaxX, _mm256_set1_ps(0.5f), _mm256_set1_ps(0.5f));
    bboxMaxY =
        _mm256_fmadd_ps(bboxMaxY, _mm256_set1_ps(0.5f), _mm256_set1_ps(0.5f));
    bboxMinX = _mm256_mul_ps(bboxMinX, aFbwDivTw);
    bboxMinY = _mm256_mul_ps(bboxMinY, aFbwDivTh);
    bboxMaxX = _mm256_mul_ps(bboxMaxX, aFbwDivTw);
    bboxMaxY = _mm256_mul_ps(bboxMaxY, aFbwDivTh);
    ...
    _mm256_storeu_si256((__m256i *)overallMaskT,
                        _mm256_castps_si256(overallMask));
    ...
    for (int i = 0; i < cands; i++) {
      ...
      atriShade.bx = vfloat3(0, 0, 1);
      atriShade.by = vfloat3(1, 0, 0);
      atriShade.f1 = vfloat4(f1xT[i], f1yT[i], f1zT[i], vz1T[i]);
      atriShade.f2 = vfloat4(f2xT[i], f2yT[i], f2zT[i], vz2T[i]);
      atriShade.f3 = vfloat4(f3xT[i], f3yT[i], f3zT[i], vz3T[i]);
      atriShade.originalPrimitive = candPrim[i];
      atriRaster.e1 = vfloat3(e1xT[i], e1yT[i], e1zT[i]);
      atriRaster.e2 = vfloat3(e2xT[i], e2yT[i], e2zT[i]);
  ...
  while ((curChunk = renderer->fetchUnresolvedChunkGeometry()) >= 0) {
    ...
    auto lim = std::min(ixBufSize, (curChunk + 1) * context->gsChunkSize *
                                       context->vertexStride);
...
void TileRasterWorker::rasterizationSingleTile(TileRasterRenderer *renderer,
  ...
  for (int T = TOTAL_THREADS - 1; T >= 0; T--) {
    ...
    for (int j = proposalT.size() - 1; j >= 0; j--) {
      ...
      for (int x = leftBlock; x < rightBlock; x += 4) {
        ...
        __m256 subTileMinX256 =
            _mm256_fmadd_ps(x256f, xTileWidth256f, tileMinX256);
        ...
        for (int k = 0; k < 3; k++) {
          critLocalTRX[k] =
              _mm256_mul_ps(edgeCoefs256X[k], tileCoordsX256[chosenCoordTR[k]]);
          critLocalTRY[k] =
              _mm256_mul_ps(edgeCoefs256X[k], tileCoordsX256[chosenCoordTA[k]]);
        }
        tileCoordsX256[VRB] = subTileMaxX256;
        for (int y = topBlock; y < bottomBlock; y += 2) {
          __m256i y256 = _mm256_setr_epi32(y + 0, y + 0, y + 0, y + 0, y + 1,
                                           y + 1, y + 1, y + 1);
          __m256i criteriaTR256 = _mm256_setzero_si256();
          __m256i criteriaTA256 = _mm256_setzero_si256();
          __m256 y256f = _mm256_cvtepi32_ps(y256);
          __m256 subTileMinY256 = _mm256_fmadd_ps(
              y256f, xTileWidth256f,
              tileMinY256); //_mm256_fmadd_ps(y256f, wfy256, tileMinY256);
          ...
          for (int k = 0; k < 3; k++) {
            criteriaLocalTR256[k] = _mm256_fmadd_ps(
                edgeCoefs256Y[k], tileCoordsY256[chosenCoordTR[k]],
                critLocalTRX[k]);
            criteriaLocalTA256[k] = _mm256_fmadd_ps(
                edgeCoefs256Y[k], tileCoordsY256[chosenCoordTA[k]],
                critLocalTRY[k]);
            __m256i criteriaTRMask = _mm256_castps_si256(_mm256_cmp_ps(
                criteriaLocalTR256[k], edgeCoefs256Z[k], _CMP_LT_OS));
            __m256i criteriaTAMask = _mm256_castps_si256(_mm256_cmp_ps(
                criteriaLocalTA256[k], edgeCoefs256Z[k], _CMP_LT_OS));
          ...
          for (int i = 0; i < 8; i++) {
            ...
            if (criteriaTA[i] == -3) {
              npropBlock.tile = FastUtil::i32Pack(dwX[i], dwY[i]);
      ...
      for (int x = leftBlock; x < rightBlock; x += 2) {
        for (int y = topBlock; y < bottomBlock; y += 2) {
          ...
          __m128 subTileMinY128 =
              _mm_fmadd_ps(y128f, xTileWidth128f, tileMinY128);
          ...
          for (int k = 0; k < 3; k++) {
            criteriaLocalTR128[k] = _mm_fmadd_ps(
                edgeCoefs128X[k], tileCoordsX128[chosenCoordTR[k]],
                _mm_mul_ps(edgeCoefs128Y[k], tileCoordsY128[chosenCoordTR[k]]));
            criteriaLocalTA128[k] = _mm_fmadd_ps(
                edgeCoefs128X[k], tileCoordsX128[chosenCoordTA[k]],
                _mm_mul_ps(edgeCoefs128Y[k], tileCoordsY128[chosenCoordTA[k]]));

            __m128i criteriaTRMask = _mm_castps_si128(
                _mm_cmplt_ps(criteriaLocalTR128[k], edgeCoefs128Z[k]));
            __m128i criteriaTAMask = _mm_castps_si128(
                _mm_cmplt_ps(criteriaLocalTA128[k], edgeCoefs128Z[k]));
          ...
          for (int i = 0; i < 4; i++) {
            ...
            if (criteriaTA[i] == -3) {
              ...
              static_assert(TileRasterContext::subtileBlockWidth == 4,
                            "subtileBlockWidth != s4");
              int dx = subTileMinX;
              __m256i dx256i =
                  _mm256_add_epi32(_mm256_set1_epi32(dx), dx256Offset);
              ...
              int dy = subTileMinY;
              __m256i dy256i =
                  _mm256_add_epi32(_mm256_set1_epi32(dy), dy256Offset);
              ...
              for (int sk = 0; sk < 2; sk++) {
                ...
                for (int k = 0; k < 3; k++) {
                  criteria256[k] =
                      _mm256_fmadd_ps(edgeCoefs256Y[k], dy256, criteria256X[k]);
                  auto acceptMask = _mm256_castps_si256(_mm256_cmp_ps(
                      criteria256[k], edgeCoefs256Z[k], _CMP_LT_OS));
                  accept256 = _mm256_add_epi32(accept256, acceptMask);
                }
                accept256 =
                    _mm256_cmpeq_epi32(accept256, _mm256_set1_epi32(-3));
                auto accept256Mask8 = _mm256_movemask_epi8(accept256);

                if (accept256Mask8 == -1) {
                  npropPixel256.tile = FastUtil::i32Pack(dx, dy);
                  ...
                  if (cond1 == 0x0000FFFF) {
                    ...
                    npropPixel.tile = FastUtil::i32Pack(pixelId, cond1);
                    coverQueue.push_back(npropPixel);
                  }
                  auto cond2 = (accept256Mask8 & 0xFFFF0000);
                  if (cond2 == 0xFFFF0000) {
                    npropPixel128.tile = FastUtil::i32Pack(dx + 2, dy);
                    coverQueue.push_back(npropPixel128);
                  } else if (cond2 != 0) {
                    auto pixelId = dx + dy * TILE_RASTER_TEMP_CONST_MAXW + 2;
                    npropPixel.tile =
                        FastUtil::i32Pack(pixelId, (int)(cond2 >> 16));
                    coverQueue.push_back(npropPixel);
                  }
                }
              ...
              for (int dx = subTileMinX; dx < subTileMaxX; dx += 2) {
                for (int dy = subTileMinY; dy < subTileMaxY; dy += 2) {
                  ...
                  for (int k = 0; k < 3; k++) {
                    criteria128[k] =
                        _mm_add_ps(_mm_mul_ps(edgeCoefs128X[k], dx128),
                                   _mm_mul_ps(edgeCoefs128Y[k], dy128));
                    __m128i acceptMask = _mm_castps_si128(
                        _mm_cmplt_ps(criteria128[k], edgeCoefs128Z[k]));
                  ...
                  if (_mm_testc_si128(accept128, _mm_set1_epi32(-1))) {
                    ...
                    if (accept128Mask) {
                      auto pixelId = dx + dy * TILE_RASTER_TEMP_CONST_MAXW;
                      npropPixel.tile = FastUtil::i32Pack(
                          pixelId, accept128Mask & 0x0000FFFF);
                      coverQueue.push_back(npropPixel);
                    }
                  }
                }
      ...
      auto totalSubtiles =
          context->numSubtilesPerTileX * context->numSubtilesPerTileX;
      ...
      for (int x = leftBlock; x < rightBlock; x += 1) {
        for (int y = topBlock; y < bottomBlock; y += 1) {
          ...
          for (int k = 0; k < 3; k++) {
            float criteriaTRLocal =
                edgeCoefs[k].x * tileCoords[chosenCoordTR[k]].x +
                edgeCoefs[k].y * tileCoords[chosenCoordTR[k]].y;
            float criteriaTALocal =
                edgeCoefs[k].x * tileCoords[chosenCoordTA[k]].x +
              ...
              for (int dy = subTileMinY; dy < subTileMaxY; dy += 2) {
                ...
                for (int k = 0; k < 4; k++) {
                  ...
                  for (int i2 = 0; i2 < 3; i2++) {
                    float criteria =
                        edgeCoefs[i2].x * px + edgeCoefs[i2].y * py;
                ...
                if (mask) {
                  auto pixelId = dx + dy * TILE_RASTER_TEMP_CONST_MAXW;
                  npropPixel.tile =
                      FastUtil::i32Pack(pixelId, mask & 0x0000FFFF);
                  coverQueue.push_back(npropPixel);
                }
              }
            }
...
void TileRasterWorker::sortOrderProcessingSingleTile(
    TileRasterRenderer *renderer, int tileId) IFRIT_AP_NOTHROW {
  auto curTile = tileId;
  ...
  auto sortCompareOp = [&](const TileBinProposal &a, const TileBinProposal &b) {
    ...
    auto aw = cvrsQueueWorkerId(ax);
    auto ap = cvrsQueuePrimitiveId(ax);
    auto ao = context->assembledTrianglesShade[aw][ap].originalPrimitive;
    auto bx = b.clipTrianglePacked;
    auto bw = cvrsQueueWorkerId(bx);
    auto bp = cvrsQueuePrimitiveId(bx);
    auto bo = context->assembledTrianglesShade[bw][bp].originalPrimitive;
    return ao < bo;
  };
  // Std sort is suboptimal here. `coverQueueLocal` without tile level proposals
  // is already sorted. We can use a more efficient sorting algorithm here.
...
void TileRasterWorker::fragmentProcessingSingleTile(
    TileRasterRenderer *renderer, bool clearedDepth,
    int tileId) IFRIT_AP_NOTHROW {
  auto curTile = tileId;
  ...
  auto proposalProcessFuncTileOnly =
      [&]<bool tpAlphaBlendEnable, IfritCompareOp tpDepthFunc,
          bool tpOnlyTaggingPass>(
          AssembledTriangleProposalReference &proposal) {
        const auto &triProposal =
            atriShadeQueue[proposal.workerId][proposal.primId];
#ifdef IFRIT_USE_SIMD_128
#ifdef IFRIT_USE_SIMD_256
        if constexpr (tpOnlyTaggingPass) {
          pixelShadingSIMD256Grouped<tpAlphaBlendEnable, tpDepthFunc,
                                     tpOnlyTaggingPass>(
          ...
          for (int dx = curTileX1; dx < curTileX2; dx += 4) {
            for (int dy = curTileY1; dy < curTileY2; dy += 2) {
              pixelShadingSIMD256<tpAlphaBlendEnable, tpDepthFunc,
                                  tpOnlyTaggingPass>(triProposal, dx, dy,
                                                     pxArgs);
            }
          }
        }
        ...
        for (int dx = curTileX1; dx < curTileX2; dx += 2) {
          for (int dy = curTileY1; dy < curTileY2; dy += 2) {
            pixelShadingSIMD128<tpAlphaBlendEnable, tpDepthFunc,
  ...
  auto proposalProcessFunc = [&]<bool tpAlphaBlendEnable,
                                 IfritCompareOp tpDepthFunc,
                                 bool tpOnlyTaggingPass>(
                                 TileBinProposal &proposal) {
    auto propClipTriPacked = proposal.clipTrianglePacked;
    auto propClipTriWorkerId = cvrsQueueWorkerId(propClipTriPacked);
    auto propClipTriPrimId = cvrsQueuePrimitiveId(propClipTriPacked);
    const auto &triProposal =
        atriShadeQueue[propClipTriWorkerId][propClipTriPrimId];
    if (proposal.level == TileRasterLevel::PIXEL) {
      ...
      int tileXd = static_cast<int>(propX % TILE_RASTER_TEMP_CONST_MAXW);
      int tileYd = static_cast<int>(propX / TILE_RASTER_TEMP_CONST_MAXW);
      auto dcx = propY;
#ifdef IFRIT_USE_SIMD_128
      constexpr bool hasSimd128 = true;
#else
      constexpr bool hasSimd128 = false;
#endif
      if constexpr (true && tpOnlyTaggingPass && hasSimd128) {
        pixelShadingSingleQuad<tpAlphaBlendEnable, tpDepthFunc,
                               tpOnlyTaggingPass>(triProposal, dcx, tileXd,
                                                  tileYd, pxArgs);
      } else {
        if (dcx & 0xF)
          pixelShading<tpAlphaBlendEnable, tpDepthFunc, tpOnlyTaggingPass>(
      ...
      auto propY = FastUtil::i32UnpackFromi64Second(proposalTile);
      auto subTilePixelX =
          (curTileX * subtileXPerTile + propX) * context->subtileBlockWidth;
      auto subTilePixelY =
          (curTileY * subtileXPerTile + propY) * context->subtileBlockWidth;
      auto subTilePixelX2 =
          (curTileX * subtileXPerTile + propX + 1) * context->subtileBlockWidth;
      auto subTilePixelY2 =
          (curTileY * subtileXPerTile + propY + 1) * context->subtileBlockWidth;
      subTilePixelX2 = std::min(subTilePixelX2, (uint32_t)frameBufferWidth);
      subTilePixelY2 = std::min(subTilePixelY2, (uint32_t)frameBufferHeight);
  ...
  if (context->optForceDeterministic) {
    auto iterFunc = [&]<bool tpAlphaBlendEnable, IfritCompareOp tpDepthFunc,
                        bool tpOnlyTaggingPass>() {
      if (coverQueueLocal.size() == 0)
        return;
      // Cache depth
      auto &depthRef = (*context->frameBuffer->getDepthAttachment());
      if (clearedDepth) {
        constexpr auto inftyDepth = getInftyDepthValue();
        FastUtil::memsetDword(depthCache, inftyDepth,
                              tagbufferSizeX * tagbufferSizeX);
      } else {
...
#define IF_DECLPS_ITERFUNC_0(tpAlphaBlendEnable, tpDepthFunc,                  \
                             tpOnlyTaggingPass)                                \
  iterFunc.operator()<tpAlphaBlendEnable, tpDepthFunc, tpOnlyTaggingPass>();
#define IF_DECLPS_ITERFUNC_0_BRANCH(tpAlphaBlendEnable, tpDepthFunc,           \
                                    tpOnlyTaggingPass)                         \
  if (context->depthFunc == tpDepthFunc)                                       \
  IF_DECLPS_ITERFUNC_0(tpAlphaBlendEnable, tpDepthFunc, tpOnlyTaggingPass)
...
#define IF_DECLPS_ITERFUNC(tpAlphaBlendEnable, tpOnlyTaggingPass)              \
  ...
  IF_DECLPS_ITERFUNC_0_BRANCH(tpAlphaBlendEnable, IF_COMPARE_OP_GREATER,       \
                              tpOnlyTaggingPass);                              \
  IF_DECLPS_ITERFUNC_0_BRANCH(                                                 \
      tpAlphaBlendEnable, IF_COMPARE_OP_GREATER_OR_EQUAL, tpOnlyTaggingPass);  \
  ...
  IF_DECLPS_ITERFUNC_0_BRANCH(tpAlphaBlendEnable, IF_COMPARE_OP_NOT_EQUAL,     \
    ...
    auto iterFunc = [&]<bool tpAlphaBlendEnable, IfritCompareOp tpDepthFunc,
                        bool tpOnlyTaggingPass>() {
      auto curTileX =
          (unsigned)curTile % (unsigned)context->numTilesX * context->tileWidth;
      auto curTileY =
          (unsigned)curTile / (unsigned)context->numTilesX * context->tileWidth;
      auto &depthRef = (*context->frameBuffer->getDepthAttachment());
      if constexpr (tpOnlyTaggingPass) {
        ...
        if (directReturn && coverQueueLocalSize == 0)
          return;
        FastUtil::memsetDword(tagbuf.valid, -1,
                              tagbufferSizeX * tagbufferSizeX);
      }
      // Cache depth
      if (clearedDepth) {
      ...
      for (int i = TOTAL_THREADS - 1; i >= 0; i--) {
        ...
        for (int j = pq.size() - 1; j >= 0; j--) {
          ...
          auto propPrimId = cvrsQueuePrimitiveId(proposal);
          auto propWorkerId = cvrsQueueWorkerId(proposal);
          AssembledTriangleProposalReference proposalX = {
              static_cast<uint32_t>(propWorkerId), propPrimId};
          proposalProcessFuncTileOnly.
          operator()<tpAlphaBlendEnable, tpDepthFunc, tpOnlyTaggingPass>(
              proposalX);
        }
      }
      // TileLocal
      ...
      if constexpr (tpOnlyTaggingPass) {
        auto forceQuad = context->threadSafeFS[0]->forcedQuadInvocation;
        if (forceQuad) {
          pixelShadingFromTagBufferQuadInvo(curTileX, curTileY, pxArgs);
        } else {
...
void TileRasterWorker::pixelShadingFromTagBufferQuadInvo(
  ...
  const void *itp[2][4] = {
      {interpVaryingsQuad, interpVaryingsQuad + 1, interpVaryingsQuad + 2,
       interpVaryingsQuad + 3},
      {interpVaryingsQuad + 4, interpVaryingsQuad + 5, interpVaryingsQuad + 6,
       interpVaryingsQuad + 7},
  };
  void *co[2][4] = {
      {colorOutpQuad, colorOutpQuad + 1, colorOutpQuad + 2, colorOutpQuad + 3},
      {colorOutpQuad + 4, colorOutpQuad + 5, colorOutpQuad + 6,
       colorOutpQuad + 7},
  };
#ifdef IFRIT_USE_SIMD_256
  ...
  __m256i dxId256T =
      _mm256_setr_epi32(0, 1, 0 + tagbufferSizeX, 1 + tagbufferSizeX, 2, 3,
                        2 + tagbufferSizeX, 3 + tagbufferSizeX);
#endif
  ...
  for (int i = 0; i < tagbufferSizeX * tagbufferSizeX; i += (8 >> 1)) {
    ...
    if (i & tagbufferSizeX) {
      i += tagbufferSizeX;
      if (i >= tagbufferSizeX * tagbufferSizeX)
        break;
    }
    // Source Arrangement:
    ...
    __m256i dx256Tmp1 = _mm256_castps_si256(
        _mm256_and_ps(_mm256_castsi256_ps(dxId256),
                      _mm256_castsi256_ps(_mm256_set1_epi32(0xf))));
    ...
    __m256i validMask = _mm256_andnot_si256(_mm256_or_si256(dxFwMask, dyFhMask),
    ...
    __m256 baryVecY =
        _mm256_i32gather_ps(((float *)ptrAtpBary) + 1, gatherIdx, 4);
    __m256 baryVecZ =
        _mm256_i32gather_ps(((float *)ptrAtpBary) + 2, gatherIdx, 4);
    ...
    __m256 interpolatedDepth =
        reqDepth ? _mm256_i32gather_ps((float *)depthCache, dxId256, 4)
                 : _mm256_setzero_ps();
    __m256i tagBufferValid =
        _mm256_i32gather_epi32((int *)ptrValid, dxId256, 4);
    ...
    // If all invalid, skip
    if (_mm256_testz_si256(idxMask2, _mm256_set1_epi32(0xffffffff)))
      continue;
    ...
    __m256 atpF1Y =
        _mm256_i32gather_ps(((float *)ptrAtpF1F2) + 1, gatherIdx, 4);
    __m256 atpF2X =
        _mm256_i32gather_ps(((float *)ptrAtpF1F2) + 2, gatherIdx, 4);
    __m256 atpF2Y =
        _mm256_i32gather_ps(((float *)ptrAtpF1F2) + 3, gatherIdx, 4);
    ...
    for (int T = 0; T < 2; T++) {
      for (int j = T * 4; j < T * 4 + 4; j++) {
        ...
        float pos0_baryVecX = baryVecXsSelf - (quadIndiceSelf & 1) * f1xT[j] -
                              (quadIndiceSelf >> 0x1) * f1yT[j];
        float pos0_baryVecY = baryVecYsSelf - (quadIndiceSelf & 1) * f2xT[j] -
                              (quadIndiceSelf >> 0x1) * f2yT[j];
        float pos0_baryVecZ = baryVecZsSelf - (quadIndiceSelf & 1) * f3xT[j] -
                              (quadIndiceSelf >> 0x1) * f3yT[j];

        // Set baryVecs to the data of position 0 or 4
        __m128i quadXOffset = _mm_setr_epi32(0, 1, 0, 1);
        __m128i quadYOffset = _mm_setr_epi32(0, 0, 1, 1);

        __m128 quadXOffsetV1 =
            _mm_mul_ps(_mm_cvtepi32_ps(quadXOffset), _mm_set1_ps(f1xT[j]));
        __m128 quadYOffsetV1 =
            _mm_mul_ps(_mm_cvtepi32_ps(quadYOffset), _mm_set1_ps(f1yT[j]));
        __m128 quadXOffsetV2 =
            _mm_mul_ps(_mm_cvtepi32_ps(quadXOffset), _mm_set1_ps(f2xT[j]));
        __m128 quadYOffsetV2 =
            _mm_mul_ps(_mm_cvtepi32_ps(quadYOffset), _mm_set1_ps(f2yT[j]));
        __m128 quadXOffsetV3 =
            _mm_mul_ps(_mm_cvtepi32_ps(quadXOffset), _mm_set1_ps(f3xT[j]));
        __m128 quadYOffsetV3 =
            _mm_mul_ps(_mm_cvtepi32_ps(quadYOffset), _mm_set1_ps(f3yT[j]));
        ...
        __m128 baryVecXs =
            _mm_add_ps(baseBaryVecX, _mm_add_ps(quadXOffsetV1, quadYOffsetV1));
        __m128 baryVecYs =
            _mm_add_ps(baseBaryVecY, _mm_add_ps(quadXOffsetV2, quadYOffsetV2));
        __m128 baryVecZs =
            _mm_add_ps(baseBaryVecZ, _mm_add_ps(quadXOffsetV3, quadYOffsetV3));

        __m128 zCorr =
            _mm_rcp_ps(_mm_add_ps(baryVecXs, _mm_add_ps(baryVecYs, baryVecZs)));
        ...
        desiredBaryR[0] = _mm_fmadd_ps(
            ...
            _mm_fmadd_ps(baryVecYs, _mm_set1_ps(atpBxYT[j]),
                         _mm_mul_ps(baryVecZs, _mm_set1_ps(atpBxZT[j]))));
        desiredBaryR[1] = _mm_fmadd_ps(
            baryVecXs, _mm_set1_ps(atpByXT[j]),
            _mm_fmadd_ps(baryVecYs, _mm_set1_ps(atpByYT[j]),
                         _mm_mul_ps(baryVecZs, _mm_set1_ps(atpByZT[j]))));
        desiredBaryR[2] = _mm_sub_ps(
            _mm_set1_ps(1.0f), _mm_add_ps(desiredBaryR[0], desiredBaryR[1]));
        ...
        for (int k = T * 4; k < T * 4 + 4; k++) {
          if (idxT[k] == idxT[j]) {
            args.colorAttachment0->fillPixelRGBA128ps(
                dx[k], dy[k], _mm_loadu_ps((float *)(colorOutpQuad[k])));
          }
        }
      }
    }
...
void TileRasterWorker::pixelShadingFromTagBuffer(
  ...
  for (int i = 0; i < tagbufferSizeX * tagbufferSizeX; i += loopIncre) {
    ...
    __m256i tagBufferValid = _mm256_castps_si256(
        _mm256_loadu_ps((const float *)ptrValid +
                        i)); //_mm256_i32gather_epi32(ptrValid, dxId256, 4);
    ...
    // If all invalid, skip
    if (_mm256_testz_si256(idxMask2, _mm256_set1_epi32(-1)))
      continue;
    ...
    desiredBaryR[0] = _mm256_fmadd_ps(
        baryVecX, atpBxX,
        _mm256_fmadd_ps(baryVecY, atpBxY, _mm256_mul_ps(baryVecZ, atpBxZ)));
    desiredBaryR[1] = _mm256_fmadd_ps(
        baryVecX, atpByX,
        _mm256_fmadd_ps(baryVecY, atpByY, _mm256_mul_ps(baryVecZ, atpByZ)));
    desiredBaryR[2] = _mm256_sub_ps(
        _mm256_set1_ps(1.0f), _mm256_add_ps(desiredBaryR[0], desiredBaryR[1]));
    ...
    if (dx >= context->frameWidth || dy >= context->frameHeight)
      continue;
    ...
    psEntry->execute(interpolatedVaryings.data(), colorOutput.data(),
                     &interpolatedDepth);
#ifdef IFRIT_USE_SIMD_128
    args.colorAttachment0->fillPixelRGBA128ps(
        dx, dy, _mm_loadu_ps((float *)(&colorOutput[0])));
#else
    args.colorAttachment0->fillPixelRGBA(dx, dy, colorOutput[0].x,
                                         colorOutput[0].y, colorOutput[0].z,
                                         colorOutput[0].w);
#endif
#endif
  }
...
template <bool tpAlphaBlendEnable, IfritCompareOp tpDepthFunc,
          bool tpOnlyTaggingPass>
void TileRasterWorker::pixelShadingSingleQuad(
    const AssembledTriangleProposalShadeStage &atp, int quadMask, const int dx,
  ...
  auto ptrAtpBary = args.tagBuffer->tagBufferBary;
  __m128 posZ[3];
  posZ[0] = _mm_set1_ps(valF1.w);
  posZ[1] = _mm_set1_ps(valF2.w);
  posZ[2] = _mm_set1_ps(valF3.w);
  ...
  __m128 interpolatedDepth128 =
      _mm_fmadd_ps(bary[0], posZ[0], _mm_mul_ps(bary[1], posZ[1]));
  ...
  _mm_storeu_ps(bary32[0], bary[0]);
  _mm_storeu_ps(bary32[1], bary[1]);
  _mm_storeu_ps(bary32[2], bary[2]);
  ...
  for (int i = 0; i < 4; i++) {
    ...
    uint32_t x = udx + (i & 1);
    uint32_t y = udy + (i >> 1);
    auto targetMask = 0xF << (i << 2);
    if (!(quadMask & targetMask))
      continue;
    uint32_t dx1 = ((unsigned)x) % tagbufferSizeX;
    uint32_t dy1 = ((unsigned)y) % tagbufferSizeX;
    ...
    if constexpr (tpDepthFunc == IF_COMPARE_OP_EQUAL) {
      if (interpolatedDepth[i] != depthAttachment2)
        continue;
    }
...
template <bool tpAlphaBlendEnable, IfritCompareOp tpDepthFunc,
          bool tpOnlyTaggingPass>
void TileRasterWorker::pixelShading(
    const AssembledTriangleProposalShadeStage &atp, const int dx, const int dy,
    const PixelShadingFuncArgs &args) IFRIT_AP_NOTHROW {
  ...
  if constexpr (tpOnlyTaggingPass) {
    ...
    if (args.forcedInQuads) {
      args.tagBuffer->atpF1F2[dxId] =
          vfloat4(atp.f1.x, atp.f1.y, atp.f2.x, atp.f2.y);
  ...
  if constexpr (tpAlphaBlendEnable) {
    ...
    auto mxSrcX = (blendParam.s.x + blendParam.s.y * (1 - srcRgba.w) +
                   blendParam.s.z * (1 - dstRgba[3])) *
                  (1 - blendParam.s.w);
    auto mxDstX = (blendParam.d.x + blendParam.d.y * (1 - srcRgba.w) +
                   blendParam.d.z * (1 - dstRgba[3])) *
                  (1 - blendParam.d.w);
...
#ifdef IFRIT_USE_SIMD_128
    args.colorAttachment0->fillPixelRGBA128ps(
        dx, dy, _mm_loadu_ps((float *)(&mixRgba)));
#else
    args.colorAttachment0->fillPixelRGBA(dx, dy, mixRgba.x, mixRgba.y,
                                         mixRgba.z, mixRgba.w);
#endif
  } else {
#ifdef IFRIT_USE_SIMD_128
...
template <bool tpAlphaBlendEnable, IfritCompareOp tpDepthFunc,
          bool tpOnlyTaggingPass>
void TileRasterWorker::pixelShadingSIMD128(
    const AssembledTriangleProposalShadeStage &atp, const int dx, const int dy,
    const PixelShadingFuncArgs &args) IFRIT_AP_NOTHROW {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    // float barytmp[3] = { bary32[0][i] * zCorr32[i],bary32[1][i] *
    // zCorr32[i],bary32[2][i] * zCorr32[i] };
    vfloat3 bary32Vec = vfloat3(bary32[0][i], bary32[1][i], bary32[2][i]);
    if constexpr (tpOnlyTaggingPass) {
      ptrAtpBx[dxId] = atpBxVec;
      ptrAtpBy[dxId] = atpByVec;
    ...
    if constexpr (tpAlphaBlendEnable) {
      ...
      auto mxDstX = (blendParam.d.x + blendParam.d.y * (1 - srcRgba.w) +
                    ...
                    (1 - blendParam.d.w);
      auto mxSrcA =
          (blendParamAlpha.s.x + blendParamAlpha.s.y * (1 - srcRgba.w) +
           blendParamAlpha.s.z * (1 - dstRgba[3])) *
          (1 - blendParamAlpha.s.w);
      auto mxDstA =
          (blendParamAlpha.d.x + blendParamAlpha.d.y * (1 - srcRgba.w) +
           blendParamAlpha.d.z * (1 - dstRgba[3])) *
          (1 - blendParamAlpha.d.w);
      ...
      mixRgba.w = dstRgba[3] * mxDstA + srcRgba.w * mxSrcA;
      args.colorAttachment0->fillPixelRGBA128ps(
          x, y, _mm_loadu_ps((const float *)(&mixRgba)));
    } else {
      args.colorAttachment0->fillPixelRGBA128ps(
          x, y, _mm_loadu_ps((const float *)(&colorOutput[0])));
    }
    // Depth Write
    (*context->frameBuffer->getDepthAttachment())(x, y, 0) =
        interpolatedDepth[i];
    depthCache[dxId] = interpolatedDepth[i];
  }
#endif
...
__m256 depthTestSIMD256(__m256 src, __m256 dst) {
  if constexpr (tpDepthFunc == IF_COMPARE_OP_ALWAYS) {
    return FastUtil::fillAllOne_256();
  }
  if constexpr (tpDepthFunc == IF_COMPARE_OP_EQUAL) {
    return _mm256_cmp_ps(src, dst, _CMP_EQ_OQ);
  }
  if constexpr (tpDepthFunc == IF_COMPARE_OP_GREATER) {
    return _mm256_cmp_ps(src, dst, _CMP_GT_OQ);
  }
  if constexpr (tpDepthFunc == IF_COMPARE_OP_GREATER_OR_EQUAL) {
    return _mm256_cmp_ps(src, dst, _CMP_GE_OQ);
  }
  if constexpr (tpDepthFunc == IF_COMPARE_OP_LESS) {
    return _mm256_cmp_ps(src, dst, _CMP_LT_OQ);
  }
  if constexpr (tpDepthFunc == IF_COMPARE_OP_LESS_OR_EQUAL) {
    return _mm256_cmp_ps(src, dst, _CMP_LE_OQ);
  }
  if constexpr (tpDepthFunc == IF_COMPARE_OP_NEVER) {
...
template <bool tpAlphaBlendEnable, IfritCompareOp tpDepthFunc,
          bool tpOnlyTaggingPass>
void TileRasterWorker::pixelShadingSIMD256Grouped(
    const AssembledTriangleProposalShadeStage &atp, int groupsX, int groupsY,
  ...
  __m256i dxInterOffsetX = _mm256_setr_epi32(0, 1, 0, 1, 2, 3, 2, 3);
  __m256i dxInterOffsetY = _mm256_setr_epi32(0, 0, 1, 1, 0, 0, 1, 1);
  __m256i dxInterIncreX = _mm256_set1_epi32(4);
  __m256i dxInterIncreY = _mm256_set1_epi32(2);
  __m256i dx256i = _mm256_add_epi32(_mm256_set1_epi32(offx), dxInterOffsetX);
  for (int gx = 0; gx < groupsX; gx++) {
    ...
    baryBase[0] = _mm256_fmadd_ps(_mm256_set1_ps(tAtpF1.x), pDx,
                                  _mm256_set1_ps(tAtpF1.z));
    baryBase[1] = _mm256_fmadd_ps(_mm256_set1_ps(tAtpF2.x), pDx,
                                  _mm256_set1_ps(tAtpF2.z));
    baryBase[2] = _mm256_fmadd_ps(_mm256_set1_ps(tAtpF3.x), pDx,
                                  _mm256_set1_ps(tAtpF3.z));

    for (int gy = 0; gy < groupsY; gy++) {
      dy256i = _mm256_add_epi32(dy256i, dxInterIncreY);
      ...
      bary[0] = _mm256_fmadd_ps(_mm256_set1_ps(tAtpF1.y), pDy, baryBase[0]);
      ...
      __m256 baryDivWSum =
          _mm256_add_ps(_mm256_add_ps(bary[0], bary[1]), bary[2]);
      __m256 interpolatedDepth256 = _mm256_fmadd_ps(
          bary[0], posZ[0],
          _mm256_fmadd_ps(bary[1], posZ[1], _mm256_mul_ps(bary[2], posZ[2])));
      ...
      _mm256_storeu_ps(bary32[0], bary[0]);
      _mm256_storeu_ps(bary32[1], bary[1]);
      _mm256_storeu_ps(bary32[2], bary[2]);
      auto depthTestResult =
          depthTestSIMD256<tpDepthFunc>(interpolatedDepth256, depthCache256);
      auto depthTestResultMask = _mm256_movemask_ps(depthTestResult);

      for (int i = 0; i < 8; i++) {
        unsigned dxId = idPacked[i];
        if (!(depthTestResultMask & (1 << i)))
          continue;
        vfloat3 bary32Vec = vfloat3(bary32[0][i], bary32[1][i], bary32[2][i]);
        if constexpr (tpOnlyTaggingPass) {
...
template <bool tpAlphaBlendEnable, IfritCompareOp tpDepthFunc,
          bool tpOnlyTaggingPass>
void TileRasterWorker::pixelShadingSIMD256(
    const AssembledTriangleProposalShadeStage &atp, const int dx, const int dy,
    const PixelShadingFuncArgs &args) IFRIT_AP_NOTHROW {
#ifndef IFRIT_USE_SIMD_256
  ifritError("SIMD 256 (AVX2) not enabled");
  ...
  __m256i dx256i = _mm256_setr_epi32(dx + 0, dx + 1, dx + 0, dx + 1, dx + 2,
                                     dx + 3, dx + 2, dx + 3);
  __m256i dy256i = _mm256_setr_epi32(dy + 0, dy + 0, dy + 1, dy + 1, dy + 0,
                                     dy + 0, dy + 1, dy + 1);
  __m256i dx256Mod16 = _mm256_and_si256(dx256i, _mm256_set1_epi32(0xf));
  __m256i dy256Mod16 = _mm256_and_si256(dy256i, _mm256_set1_epi32(0xf));
  __m256i dxId256 =
  ...
  bary[0] =
      _mm256_fmadd_ps(_mm256_set1_ps(tAtpF1.y), pDy, _mm256_set1_ps(tAtpF1.z));
  bary[0] = _mm256_fmadd_ps(_mm256_set1_ps(tAtpF1.x), pDx, bary[0]);
  bary[1] =
      _mm256_fmadd_ps(_mm256_set1_ps(tAtpF2.y), pDy, _mm256_set1_ps(tAtpF2.z));
  bary[1] = _mm256_fmadd_ps(_mm256_set1_ps(tAtpF2.x), pDx, bary[1]);
  bary[2] =
      _mm256_fmadd_ps(_mm256_set1_ps(tAtpF3.y), pDy, _mm256_set1_ps(tAtpF3.z));
...
#define IF_DECLPS1(tpAlphaBlending, tpDepthFunc, tpOnlyTaggingPass)            \
  template void TileRasterWorker::pixelShadingSingleQuad<                      \
      tpAlphaBlending, tpDepthFunc, tpOnlyTaggingPass>(                        \
      const AssembledTriangleProposalShadeStage &atp, int quadMask,            \
      const int dx, const int dy, const PixelShadingFuncArgs &args)            \
      IFRIT_AP_NOTHROW;                                                        \
  template void TileRasterWorker::pixelShading<tpAlphaBlending, tpDepthFunc,   \
                                               tpOnlyTaggingPass>(             \
      const AssembledTriangleProposalShadeStage &atp, const int dx,            \
      const int dy, const PixelShadingFuncArgs &args) IFRIT_AP_NOTHROW;        \
  template void TileRasterWorker::pixelShadingSIMD128<                         \
      tpAlphaBlending, tpDepthFunc, tpOnlyTaggingPass>(                        \
      const AssembledTriangleProposalShadeStage &atp, const int dx,            \
      const int dy, const PixelShadingFuncArgs &args) IFRIT_AP_NOTHROW;        \
  template void TileRasterWorker::pixelShadingSIMD256<                         \
      tpAlphaBlending, tpDepthFunc, tpOnlyTaggingPass>(                        \
      const AssembledTriangleProposalShadeStage &atp, const int dx,            \
      const int dy, const PixelShadingFuncArgs &args) IFRIT_AP_NOTHROW;        \
  template void TileRasterWorker::pixelShadingSIMD256Grouped<                  \
      tpAlphaBlending, tpDepthFunc, tpOnlyTaggingPass>(                        \
      const AssembledTriangleProposalShadeStage &atp, int groupsX,             \
      int groupsY, const int dx, const int dy,                                 \
      const PixelShadingFuncArgs &args) IFRIT_AP_NOTHROW;

#define IF_DECLPS1_2(tpDepthFunc, tpOnlyTaggingPass)                           \
  IF_DECLPS1(true, tpDepthFunc, tpOnlyTaggingPass);                            \
  IF_DECLPS1(false, tpDepthFunc, tpOnlyTaggingPass);
#define IF_DECLPS1_1(tpDepthFunc)                                              \
  IF_DECLPS1_2(tpDepthFunc, true);                                             \
  IF_DECLPS1_2(tpDepthFunc, false);
