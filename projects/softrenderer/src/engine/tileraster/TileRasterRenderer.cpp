...
namespace Ifrit::Engine::SoftRenderer::TileRaster::Inline {
...
IFRIT_APIDECL void
TileRasterRenderer::bindVertexBuffer(const VertexBuffer &vertexBuffer) {
  this->context->vertexBuffer = &vertexBuffer;
  varyingBufferDirtyFlag = true;
}

IFRIT_APIDECL void
TileRasterRenderer::bindFragmentShader(FragmentShader &fragmentShader) {
  this->context->fragmentShader = &fragmentShader;
  if (!fragmentShader.isThreadSafe) {
    for (int i = 0; i < context->numThreads + 1; i++) {
      context->threadSafeFSOwningSection[i] =
          fragmentShader.getThreadLocalCopy();
...
IFRIT_APIDECL void
TileRasterRenderer::bindUniformBuffer(int binding, int set,
...
IFRIT_APIDECL void
TileRasterRenderer::bindIndexBuffer(BufferManager::IfritBuffer indexBuffer) {
  auto p = indexBuffer.manager.lock();
  p->mapBufferMemory(indexBuffer, (void **)&this->context->indexBuffer);
}

IFRIT_APIDECL void
TileRasterRenderer::bindVertexShader(VertexShader &vertexShader) {
  this->context->owningVaryingDesc = std::make_unique<VaryingDescriptor>(
      std::move(vertexShader.getVaryingDescriptor()));
...
IFRIT_APIDECL void TileRasterRenderer::intializeRenderContext() {
  if (varyingBufferDirtyFlag) {
    context->vertexShaderResult = std::make_unique<VertexShaderResult>(
        context->vertexBuffer->getVertexCount(),
        context->varyingDescriptor->getVaryingCounts());
    shaderBindingDirtyFlag = false;
  }
  if (varyingBufferDirtyFlag) {
    context->vertexShaderResult->allocateVaryings(
        context->varyingDescriptor->getVaryingCounts());
    context->varyingDescriptor->applyVaryingDescriptors(
        context->vertexShaderResult.get());
    context->vertexShaderResult->setVertexCount(
        context->vertexBuffer->getVertexCount());
...
void TileRasterRenderer::statusTransitionBarrier2(TileRasterStage waitOn,
                                                  TileRasterStage proceedTo) {
  while (true) {
    bool allOnBarrier = true;
    for (auto &worker : workers) {
      ...
      allOnBarrier = allOnBarrier &&
                     (worker->status.compare_exchange_weak(
                          expected, proceedTo, std::memory_order::acq_rel) ||
                      (expected >= proceedTo));
    }
    auto expected = waitOn;
    allOnBarrier =
        allOnBarrier && (selfOwningWorker->status.compare_exchange_weak(
                             expected, proceedTo, std::memory_order::acq_rel) ||
                         (expected >= proceedTo));
    if (allOnBarrier)
      break;
...
IFRIT_APIDECL void
TileRasterRenderer::setBlendFunc(IfritColorAttachmentBlendState state) {
  ...
  } else if (bs.srcColorBlendFactor ==
             IfritBlendFactor::IF_BLEND_FACTOR_DST_ALPHA) {
    context->blendColorCoefs.s = {0, 0, 1, 0};
  } else if (bs.srcColorBlendFactor ==
             IfritBlendFactor::IF_BLEND_FACTOR_SRC_ALPHA) {
    context->blendColorCoefs.s = {0, 1, 0, 0};
  } else if (bs.srcColorBlendFactor ==
             IfritBlendFactor::IF_BLEND_FACTOR_ONE_MINUS_DST_ALPHA) {
    context->blendColorCoefs.s = {1, 0, -1, 0};
  } else if (bs.srcColorBlendFactor ==
             IfritBlendFactor::IF_BLEND_FACTOR_ONE_MINUS_SRC_ALPHA) {
  ...
  } else if (bs.srcAlphaBlendFactor ==
             IfritBlendFactor::IF_BLEND_FACTOR_DST_ALPHA) {
  ...
  } else if (bs.dstColorBlendFactor ==
             IfritBlendFactor::IF_BLEND_FACTOR_DST_ALPHA) {
  ...
  } else if (bs.dstAlphaBlendFactor ==
             IfritBlendFactor::IF_BLEND_FACTOR_DST_ALPHA) {
  ...
  for (int i = context->numThreads; i >= 0; i--) {
    for (const auto &x : vsUniforms) {
      if (context->uniformMapping.count(x)) {
        context->threadSafeVS[i]->updateUniformData(x.first, x.second,
                                                    context->uniformMapping[x]);
      }
    }
    for (const auto &x : fsUniforms) {
      if (context->uniformMapping.count(x)) {
        context->threadSafeFS[i]->updateUniformData(x.first, x.second,
                                                    context->uniformMapping[x]);
      }
    }
  }
...
IFRIT_APIDECL void TileRasterRenderer::init() {
  ...
  createWorkers();
  for (auto &worker : workers) {
    worker->status.store(TileRasterStage::COMPLETED,
                         std::memory_order::relaxed);
    worker->threadStart();
  }
  selfOwningWorker->status.store(TileRasterStage::COMPLETED,
                                 std::memory_order::relaxed);
  initialized = true;
}
...
IFRIT_APIDECL void
TileRasterRenderer::drawElements(int vertexCount,
                                 bool clearFramebuffer) IFRIT_AP_NOTHROW {
  intializeRenderContext();
  updateUniformBuffer();
  context->indexBufferSize = vertexCount;
  unresolvedTileRaster.store(context->numTilesX * context->numTilesY,
                             std::memory_order::relaxed);
  auto vertexChunks = Inline::ceilDiv(context->vertexBuffer->getVertexCount(),
                                      context->vsChunkSize);
  auto geometryChunks = Inline::ceilDiv(vertexCount / context->vertexStride,
                                        context->gsChunkSize);
  ...
  statusTransitionBarrier3(TileRasterStage::FRAGMENT_SHADING_SYNC,
