...
namespace Ifrit::Runtime
{
        ...
        GraphicsShaderPassType passType, RenderTargets* renderTargets, const SceneCollectConfig& config)
    {
        ...
        if (true)
        {
            ...
            viewData.m_viewData.m_worldToClip =
    ...
    IFRIT_APIDECL void RendererBase::RecreateGBuffers(PerFrameData& perframeData, RenderTargets* renderTargets)
    {
        ...
        if (needRecreate)
        {
            ...
            perframeData.m_gbuffer.m_emissive = rhi->CreateTexture2D(
                "Render_GEmissive", actualRtWidth, actualRtHeight, targetFomrat, tarGetUsage, true);
    ...
    IFRIT_APIDECL void RendererBase::PrepareDeviceResources(PerFrameData& perframeData, RenderTargets* renderTargets)
    {
        ...
        for (auto& shaderEffectId : perframeData.m_enabledEffects)
        {
            ...
            // preloading meshes
            UnorderedFor<size_t>(
                0, shaderEffect.m_materials.size(), [&](size_t x) { auto mesh = shaderEffect.m_meshes[x]; });
            ...
            for (int i = 0; i < shaderEffect.m_materials.size(); i++)
            {
                ...
                if (meshResource.objectBuffer == nullptr || mesh->m_resourceDirty)
                {
                    ...
                    meshResource.vertexBuffer = rhi->CreateBufferDevice("Mesh_Vertex",
                        SizeCast<u32>(meshDataRef->m_verticesAligned.size() * sizeof(Vector4f)), tmpUsage, true);
                    meshResource.normalBuffer = rhi->CreateBufferDevice("Mesh_Normal",
                    ...
                    if (materialRef->m_data.size() > 0)
                    {
                        ...
                        meshResource.materialDataBuffer = rhi->CreateBufferDevice("MaterialData",
                            SizeCast<u32>(shaderEffect.m_materials[i]->m_data[0].size()), tmpUsage, true);
                    }
                    else
                    {
...
#define enqueueStagedBuffer(name, vecBuffer)                                    \
    ...
    funcEnqueueStagedBuffer(staged##name, meshDataRef->vecBuffer.data(),        \
