...
namespace Ifrit::Runtime::RenderingUtil::CascadeShadowMapping
{
    ...
    IFRIT_APIDECL CSMResult calculateCSMSplits(const PerFrameData::PerViewData& perView, u32 shadowResolution,
        Vector3f lightFront, u32 splitCount, float maxDistance, const Vec<float>& splits, const Vec<float>& borders)
    {
    ...
    IFRIT_APIDECL Vec<PerFrameData::PerViewData> fillCSMViews(const PerFrameData::PerViewData& perView, Light& light,
        u32 shadowResolution, Transform& lightTransform, u32 splitCount, float maxDistance, const Vec<float>& splits,
        const Vec<float>& borders, std::array<float, 4>& splitStart, std::array<float, 4>& splitEnd)
    {
        ...
        auto csmResult =
            calculateCSMSplits(perView, shadowResolution, lightDir3, splitCount, maxDistance, splits, borders);
        ...
        for (auto i = 0u; i < splitCount; i++)
        {
            ...
            view.m_viewData.m_worldToClip  = Math::Transpose(Math::MatMul(
