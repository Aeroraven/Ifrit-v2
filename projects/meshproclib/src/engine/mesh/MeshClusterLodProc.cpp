...
namespace Ifrit::MeshProcLib::MeshProcess
{
    ...
    void MetisValidation(ClusterLodGeneratorContext& ctx)
    {
        ...
        {
            std::unordered_map<u64, Vec<u32>> edgeToMeshletMap{};
            ...
            for (auto i : cluster)
            {
                for (auto j : cluster)
                {
                    if (i >= j)
                        continue;
                    std::unordered_set<u64> edgeSetA;
    ...
    void          BvhCollapse(ClusterGroupBVH& bvh)
    {
        ...
        Fn<void(ClusterGroupBVHNode*, IndirectChildrenSet*, int)> getChild =
        ...
        Fn<DFSRet(ClusterGroupBVHNode*)> dfsSubTreeSize = [&](ClusterGroupBVHNode* node) -> DFSRet {
            if (node->isLeaf)
            {
                DFSRet ret;
                node->subTreeSize = 1;
