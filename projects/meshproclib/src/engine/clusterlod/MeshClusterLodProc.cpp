...
namespace Ifrit::Engine::MeshProcLib::ClusterLod{
  ...
  constexpr int VERTICES_PER_MESHLET = 256;
  ...
  void freeUnusedMemoryInCotenxt(ClusterLodGeneratorContext& ctx){
    ...
    auto maxTriangleCount = ctx.meshletsRaw[maxMeshlets-1].triangle_offset + ctx.meshletsRaw[maxMeshlets-1].triangle_count;
    ctx.meshletsRaw.resize(maxMeshlets);
    ctx.meshletVertices.resize(maxVertexCount);
    ctx.meshletTriangles.resize(maxTriangleCount);

  }  
  void freeUnusedMemoryInCotenxt2(std::vector<meshopt_Meshlet>& meshletsRaw, std::vector<uint32_t>& meshletVertices, std::vector<uint8_t>& meshletTriangles, int totalMeshlets){
    ...
    auto maxTriangleCount = meshletsRaw[maxMeshlets-1].triangle_offset + meshletsRaw[maxMeshlets-1].triangle_count;
  ...
  void initialMeshletGeneration(const MeshDescriptor& mesh, ClusterLodGeneratorContext& ctx){
    constexpr int triangleMinBoundByVertex = VERTICES_PER_MESHLET/3;
    constexpr int vertexMinBoundByTriangle = 3*TRIANGLES_PER_MESHLET;
    constexpr int triangleMinBound = std::min(triangleMinBoundByVertex, TRIANGLES_PER_MESHLET);
    constexpr int vertexMinBound = std::min(vertexMinBoundByTriangle, VERTICES_PER_MESHLET);
    
    int maxMeshletsByVertex = ceilDiv(mesh.vertexCount, vertexMinBound);
    int maxMeshletsByTriangle = ceilDiv(mesh.indexCount/3, triangleMinBound);
    int maxMeshlets = std::max(maxMeshletsByVertex, maxMeshletsByTriangle);
    ctx.meshletsRaw.resize(maxMeshlets);
    ...
    auto meshletCount = meshopt_buildMeshlets(ctx.meshletsRaw.data(),
      ctx.meshletVertices.data(),ctx.meshletTriangles.data(),
  ...
  void meshletAdjacencyGeneration(ClusterLodGeneratorContext& ctx){
    ...
    METIS_PartGraphKway(&nvtxs, &ncon,
      xadjPtr, adjncyPtr, vwgt, vsize, adjwgt,
  ...
  void clusterGroupSimplification(const MeshDescriptor& mesh, ClusterLodGeneratorContext& ctx,ClusterLodGeneratorContext& outCtx){
    ...
    for(auto& [key, meshletsR] : clusterGroupToMeshletMap){
      ...
      meshopt_simplify(simplifiedIndexBuffer.data(), aggregatedIndexBuffer.data(), aggregatedIndexBuffer.size(),
        ...
        aggregatedIndexBuffer.size(), MESH_SIMPLIFICATION_RATE, option, &simplifyError);
      ...
      auto maxMeshlets = meshopt_buildMeshletsBound(simplifiedIndexBuffer.size(), VERTICES_PER_MESHLET, TRIANGLES_PER_MESHLET);
      meshlets.resize(maxMeshlets);
      meshletVertices.resize(maxMeshlets*VERTICES_PER_MESHLET);
      meshletTriangles.resize(maxMeshlets*TRIANGLES_PER_MESHLET*3);
      auto meshletCount = meshopt_buildMeshlets(meshlets.data(),
        meshletVertices.data(), meshletTriangles.data(),
        simplifiedIndexBuffer.data(), simplifiedIndexBuffer.size(),
        reinterpret_cast<float*>(mesh.vertexData+mesh.positionOffset), mesh.vertexCount,
        mesh.vertexStride, VERTICES_PER_MESHLET, TRIANGLES_PER_MESHLET, 0.0f);

      freeUnusedMemoryInCotenxt2(meshlets, meshletVertices, meshletTriangles, meshletCount);
      ...
      for(int i=0;i<meshletCount;i++){
        newMeshletVertexSize += meshlets[i].vertex_count;
        newMeshletTriangleSize += meshlets[i].triangle_count;
      }
      ...
      outCtx.meshletTriangles.resize(newMeshletTriangleOffset + newMeshletTriangleSize*3);
      ...
      for(int i=0;i<meshletCount;i++){
        ...
        outCtx.meshletsRaw[outCtx.totalMeshlets+i].vertex_offset += newMeshletVertexOffset;
        outCtx.meshletsRaw[outCtx.totalMeshlets+i].triangle_offset += newMeshletTriangleOffset;
        outCtx.meshletVertices.insert(outCtx.meshletVertices.begin() + outCtx.meshletsRaw[outCtx.totalMeshlets+i].vertex_offset,
          meshletVertices.begin() + meshlets[i].vertex_offset,
          meshletVertices.begin() + meshlets[i].vertex_offset + meshlets[i].vertex_count);
        outCtx.meshletTriangles.insert(outCtx.meshletTriangles.begin() + outCtx.meshletsRaw[outCtx.totalMeshlets+i].triangle_offset*3,
          meshletTriangles.begin() + meshlets[i].triangle_offset*3,
          meshletTriangles.begin() + meshlets[i].triangle_offset*3 + meshlets[i].triangle_count*3);
      }
    }
  }
  ...
  void combineBuffer(const std::vector<ClusterLodGeneratorContext>& ctx, CombinedClusterLodBuffer& outCtx){
    ...
    for(int i=0;i<ctx.size();i++){
      ...
      for(int j=0;j<ctx[i].totalMeshlets;j++){
        outCtx.meshletsRaw[meshletOffset+j] = ifloat4(ctx[i].meshletsRaw[j].triangle_offset + prevlevelTriangleCount,
          ctx[i].meshletsRaw[j].triangle_count,
          ctx[i].meshletsRaw[j].vertex_offset + prevlevelVertexCount,
          ctx[i].meshletsRaw[j].vertex_count);
        outCtx.parentStart[meshletOffset+j] = ctx[i].parentStart[j] == -1 ? -1 : ctx[i].parentStart[j] + prevlevelMeshletCount;
        outCtx.parentSize[meshletOffset+j] = ctx[i].parentSize[j];
        outCtx.graphPartition[meshletOffset+j] = ctx[i].graphPartition[j]+prevlevelMeshletCount+ctx[i].totalMeshlets;
      }
      outCtx.meshletVertices.insert(outCtx.meshletVertices.end(), ctx[i].meshletVertices.begin(), ctx[i].meshletVertices.end());
      outCtx.meshletTriangles.insert(outCtx.meshletTriangles.end(), ctx[i].meshletTriangles.begin(), ctx[i].meshletTriangles.end());
      //TODO: Compat size
  ...
  int generateClusterLodHierachyFromSimpleMesh(const std::vector<float>& vertices, const std::vector<uint32_t>& indices,
   std::vector<ClusterLodGeneratorContext>& ctx, int maxLod){
    MeshDescriptor mesh;
    mesh.vertexData = reinterpret_cast<char*>(const_cast<float*>(vertices.data()));
    mesh.indexData = reinterpret_cast<char*>(const_cast<uint32_t*>(indices.data()));
