...
namespace Ifrit::Display::Window
{
    ...
    IFRIT_APIDECL bool GLFWWindowProvider::Setup(size_t argWidth, size_t argHeight)
    {
        ...
        glfwSwapInterval(0);
        if (!m_args.vulkanMode)
            displayAssert(gladLoadGLLoader((GLADloadproc)glfwGetProcAddress), "Failed to initialize GLAD");
        this->width  = argWidth;
        this->height = argHeight;
        return true;
    }
    IFRIT_APIDECL void GLFWWindowProvider::SetTitle(const std::string& titleName) { this->title = titleName; }
    ...
    IFRIT_APIDECL const char** GLFWWindowProvider::GetVkRequiredInstanceExtensions(uint32_t* count)
    {
