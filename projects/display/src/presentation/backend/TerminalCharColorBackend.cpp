...
IFRIT_APIDECL TerminalCharColorBackend::TerminalCharColorBackend(int cWid, int cHeight) {
  this->consoleWidth = cWid;
  this->consoleHeight = cHeight;
  resultBuffer = "";
}
IFRIT_APIDECL void TerminalCharColorBackend::updateTexture(const float *image, int channels, int width, int height) {
  std::string res = "";
  for (int i = consoleHeight - 1; i >= 0; i--) {
    for (int j = 0; j < consoleWidth; j++) {
      ...
      std::stringstream ss;
      ss << "\033[38;2;" << (int)(colR * 255) << ";" << (int)(colG * 255) << ";" << (int)(colB * 255) << "m��\033[0m";
      res += ss.str();
    }
    res += "\n";
  }
