...
// Address ops
template <typename T> IFRIT_APIDECL inline T elementAt(CoreVec2<T> &a, int i) {
  return i == 0 ? a.x : a.y;
}
template <typename T> IFRIT_APIDECL inline T elementAt(CoreVec3<T> &a, int i) {
  return i == 0 ? a.x : i == 1 ? a.y : a.z;
}
template <typename T> IFRIT_APIDECL inline T elementAt(CoreVec4<T> &a, int i) {
...
// Length ops
template <typename T> IFRIT_APIDECL inline T length(const CoreVec2<T> &a) {
  return sqrt(a.x * a.x + a.y * a.y);
}
template <typename T> IFRIT_APIDECL inline T length(const CoreVec3<T> &a) {
  return sqrt(a.x * a.x + a.y * a.y + a.z * a.z);
}
template <typename T> IFRIT_APIDECL inline T length(const CoreVec4<T> &a) {
...
// Lerp ops
template <typename T>
IFRIT_APIDECL inline CoreVec2<T> lerp(const CoreVec2<T> &a,
                                      const CoreVec2<T> &b, const T &t) {
  return a * (1 - t) + b * t;
}
template <typename T>
IFRIT_APIDECL inline CoreVec3<T> lerp(const CoreVec3<T> &a,
                                      const CoreVec3<T> &b, const T &t) {
  return a * (1 - t) + b * t;
}
template <typename T>
IFRIT_APIDECL inline CoreVec4<T> lerp(const CoreVec4<T> &a,
                                      const CoreVec4<T> &b, const T &t) {
  return a * (1 - t) + b * t;
}

// Clamp ops
template <typename T>
IFRIT_APIDECL inline CoreVec2<T> clamp(const CoreVec2<T> &a, const T &min,
                                       const T &max) {
  return CoreVec2<T>{std::clamp(a.x, min, max), std::clamp(a.y, min, max)};
}
template <typename T>
IFRIT_APIDECL inline CoreVec3<T> clamp(const CoreVec3<T> &a, const T &min,
                                       const T &max) {
  return CoreVec3<T>{std::clamp(a.x, min, max), std::clamp(a.y, min, max),
                     std::clamp(a.z, min, max)};
}
template <typename T>
IFRIT_APIDECL inline CoreVec4<T> clamp(const CoreVec4<T> &a, const T &min,
                                       const T &max) {
  return CoreVec4<T>{std::clamp(a.x, min, max), std::clamp(a.y, min, max),
                     std::clamp(a.z, min, max), std::clamp(a.w, min, max)};
}

// Cross ops
template <typename T>
IFRIT_APIDECL inline CoreVec3<T> cross(const CoreVec3<T> &a,
                                       const CoreVec3<T> &b) {
  return CoreVec3<T>{a.y * b.z - a.z * b.y, a.z * b.x - a.x * b.z,
                     a.x * b.y - a.y * b.x};
}
...
// Min ops
template <typename T>
IFRIT_APIDECL inline CoreVec2<T> min(const CoreVec2<T> &a,
                                     const CoreVec2<T> &b) {
  return CoreVec2<T>{std::min(a.x, b.x), std::min(a.y, b.y)};
}
template <typename T>
IFRIT_APIDECL inline CoreVec3<T> min(const CoreVec3<T> &a,
                                     const CoreVec3<T> &b) {
  return CoreVec3<T>{std::min(a.x, b.x), std::min(a.y, b.y),
                     std::min(a.z, b.z)};
}
template <typename T>
IFRIT_APIDECL inline CoreVec4<T> min(const CoreVec4<T> &a,
                                     const CoreVec4<T> &b) {
  return CoreVec4<T>{std::min(a.x, b.x), std::min(a.y, b.y), std::min(a.z, b.z),
                     std::min(a.w, b.w)};
}

// Max ops
template <typename T>
IFRIT_APIDECL inline CoreVec2<T> max(const CoreVec2<T> &a,
                                     const CoreVec2<T> &b) {
  return CoreVec2<T>{std::max(a.x, b.x), std::max(a.y, b.y)};
}
template <typename T>
IFRIT_APIDECL inline CoreVec3<T> max(const CoreVec3<T> &a,
                                     const CoreVec3<T> &b) {
  return CoreVec3<T>{std::max(a.x, b.x), std::max(a.y, b.y),
                     std::max(a.z, b.z)};
}
template <typename T>
IFRIT_APIDECL inline CoreVec4<T> max(const CoreVec4<T> &a,
                                     const CoreVec4<T> &b) {
  return CoreVec4<T>{std::max(a.x, b.x), std::max(a.y, b.y), std::max(a.z, b.z),
                     std::max(a.w, b.w)};
}
