...
#include "ifrit/core/math/simd/SimdUtilities.h"
...
namespace Ifrit::Graphics::SoftGraphics::TileRaster
{
    ...
    {
        IF_CONSTEXPR int shift = (sizeof(int) * 8 - TileRasterContext::workerReprBits);
    ...
    void TileRasterWorker::drawCall(bool withClear) IFRIT_AP_NOTHROW
    {
        ...
        if (withClear)
        {
            IF_CONSTEXPR auto depth = getInftyDepthValue();
            ptrCtx->frameBuffer->GetDepthAttachment()->clearImageMultithread(depth, workerId, TOTAL_THREADS);
        ...
        const int primitiveId, SVector4f v1, SVector4f v2, SVector4f v3) IFRIT_AP_NOTHROW
    {
        ...
        u32                  retCnt = 0;
        retSrc[0]                   = { SVector4f(1, 0, 0, 0), v1 };
        retSrc[1]                   = { SVector4f(0, 1, 0, 0), v2 };
        retSrc[2]                   = { SVector4f(0, 0, 1, 0), v3 };
        auto pc                     = retSrc[0];
        auto npc                    = -pc.pos.w;
        for (int j = 0; j < 3; j++)
        {
            ...
            if (npc * npn < 0)
            {
                SVector4f            dir          = pn.pos - pc.pos;
                f32                  t            = (-1 + pc.pos.w) / -dir.w;
                SVector4f            intersection = Fma(dir, t, pc.pos);
                SVector4f            barycenter   = Lerp(pc.barycenter, pn.barycenter, t);
        ...
        auto       smallTriangleCullVecP = SVector4f(context->frameWidth * 1.0f, context->frameHeight * 1.0f,
        ...
        for (int i = 0; i < static_cast<int>(retCnt) - 2; i++)
        {
            ...
            SVector4f                            tv1, tv2, tv3;
            SVector4f                            tb1, tb2, tb3;
            ...
            auto edgeFunctionSimdVec = [](const SVector4f& a, const SVector4f& b, const SVector4f& c) {
                return (c.x - a.x) * (b.y - a.y) - (c.y - a.y) * (b.x - a.x);
            };

            const f32  ar     = 1 / edgeFunctionSimdVec(tv1, tv2, tv3);
            const f32  sV2V1y = tv2.y - tv1.y;
            const f32  sV2V1x = tv1.x - tv2.x;
            const f32  sV3V2y = tv3.y - tv2.y;
            const f32  sV3V2x = tv2.x - tv3.x;
            const f32  sV1V3y = tv1.y - tv3.y;
            const f32  sV1V3x = tv3.x - tv1.x;
            ...
            atriShade.f1 *= ApproxReciprocal(tv1.w);
            atriShade.f2 *= ApproxReciprocal(tv2.w);
            atriShade.f3 *= ApproxReciprocal(tv3.w);
            ...
            SVector4f bboxR = Round(Fma(bbox, smallTriangleCullVecP, SVector4f(-0.5f))); // -0.5f;
            if (bboxR.x == bboxR.z || bboxR.w == bboxR.y)
            {
                continue;
            }
            bbox                   = Fma(bbox, 0.5f, SVector4f(0.5f));
            auto frameBufferWidth  = context->frameWidth;
            auto frameBufferHeight = context->frameHeight;
            f32  fbwDivTw          = 1.0f * frameBufferWidth / context->tileWidth;
            f32  fbwDivTh          = 1.0f * frameBufferHeight / context->tileWidth;
            bbox *= SVector4f(fbwDivTw, fbwDivTh, fbwDivTw, fbwDivTh);
            ...
            executeBinner(SizeCast<int>(xid), atriRaster, bbox);
            xid++;
        }
        return retCnt - 2;
    }
    bool TileRasterWorker::triangleFrustumClip(Ifrit::Math::SIMD::SVector4f v1, Ifrit::Math::SIMD::SVector4f v2,
        Ifrit::Math::SIMD::SVector4f v3, SVector4f& bbox) IFRIT_AP_NOTHROW
    {
    ...
    bool TileRasterWorker::triangleCulling(SVector4f v1, SVector4f v2, SVector4f v3) IFRIT_AP_NOTHROW
    {
        f32 d1 = (v1.x * v2.y);
        f32 d2 = (v2.x * v3.y);
        f32 d3 = (v3.x * v1.y);
        ...
        const int primitiveId, const AssembledTriangleProposalRasterStage& atp, SVector4f bbox) IFRIT_AP_NOTHROW
    {
        ...
        SVector3f              edgeCoefs[3];
        edgeCoefs[0] = atp.e1;
        edgeCoefs[1] = atp.e2;
        edgeCoefs[2] = atp.e3;

        Vector2f tileCoords[4];

        int      chosenCoordTR[3];
        int      chosenCoordTA[3];
        getAcceptRejectCoords(edgeCoefs, chosenCoordTR, chosenCoordTA);

        IF_CONSTEXPR f32 tileSizeX = TileRasterContext::tileWidth;
        IF_CONSTEXPR f32 tileSizeY = TileRasterContext::tileWidth;

        SVector3f        zeroVec = SVector3f(0.0f);
    ...
    void TileRasterWorker::tiledProcessing(TileRasterRenderer* renderer, bool clearDepth) IFRIT_AP_NOTHROW
    {
        ...
        for (int i = SizeCast<int>(interpolatedVaryingsAddr.size()) - 1; i >= 0; i--)
        {
    ...
    void TileRasterWorker::vertexProcessing(TileRasterRenderer* renderer) IFRIT_AP_NOTHROW
    {
        ...
        std::vector<SVector4f*>  outVaryings(varyingCnts);
        std::vector<SVector4f*>  outVaryingsBase(varyingCnts);
        ...
        while ((curChunk = renderer->fetchUnresolvedChunkVertex()) >= 0)
        {
            ...
            for (int j = curChunk * context->vsChunkSize; j < lim; j++)
            {
                ...
                vsEntry->execute(inVertexData, pos, (Vector4f* const*)outVaryingsData);
            }
        }
    ...
    void TileRasterWorker::geometryProcessing(TileRasterRenderer* renderer) IFRIT_AP_NOTHROW
    {
        ...
        SVector4f candV1[8], candV2[8], candV3[8];
        ...
        __m256  aCsFhFw         = _mm256_set1_ps(context->frameHeight * context->frameWidth * 1.0f);
        ...
        __m256  aInvTileWidth   = _mm256_set1_ps(ApproxReciprocal(context->tileWidth * 1.0f));
        ...
        auto    directTriangleSetup = [&]() {
            ...
            Gather32Fp32Stride4_256((f32*)candV1, tv1X, tv1Y, tv1Z, tv1W);
            __m256 tv1WInv = ApproxReciprocalNR256(tv1W);
            tv1X           = _mm256_mul_ps(tv1X, tv1WInv);
            tv1Y           = _mm256_mul_ps(tv1Y, tv1WInv);
            tv1Z           = _mm256_mul_ps(tv1Z, tv1WInv);

            Gather32Fp32Stride4_256((f32*)candV2, tv2X, tv2Y, tv2Z, tv2W);
            __m256 tv2WInv = ApproxReciprocalNR256(tv2W);
            tv2X           = _mm256_mul_ps(tv2X, tv2WInv);
            tv2Y           = _mm256_mul_ps(tv2Y, tv2WInv);
            tv2Z           = _mm256_mul_ps(tv2Z, tv2WInv);

            Gather32Fp32Stride4_256((f32*)candV3, tv3X, tv3Y, tv3Z, tv3W);
            __m256 tv3WInv = ApproxReciprocalNR256(tv3W);
            ...
            __m256 aV3ySubV1yNeg = FlipFp32Sign256(aV3ySubV1y); // _mm256_sub_ps(_mm256_setzero_ps(), aV3ySubV1y);
            __m256 aEdgeCoef     = _mm256_fmadd_ps(aV3ySubV1yNeg, aV2xSubV1xS, aEdgeCoefMul);

            __m256 aAr = ApproxReciprocalNR256(aEdgeCoef); //_mm256_rcp_ps(aEdgeCoef);
            ...
            __m256 rF3Z    = FlipFp32Sign256(_mm256_mul_ps(rF3Z_o, aAr));
            ...
            __m256 rF1Z   = FlipFp32Sign256(_mm256_mul_ps(rF1Z_o, aAr));
            ...
            __m256 rF2Z   = FlipFp32Sign256(_mm256_mul_ps(rF2Z_o, aAr));

            __m256 rE1X   = _mm256_mul_ps(aCsFh, aV2ySubV1y);
            __m256 rE1Y   = _mm256_mul_ps(aCsFw, aV2xSubV1x);
            __m256 rE1Z_o = _mm256_fmadd_ps(tv2X, tv1Y, FlipFp32Sign256(aS3));
            ...
            __m256 rE2Z_o = _mm256_fmadd_ps(tv3X, tv2Y, FlipFp32Sign256(aS1));
            ...
            __m256 rE3Z_o = _mm256_fmadd_ps(tv1X, tv3Y, FlipFp32Sign256(aS2));
            ...
            int    overallMaskT[8];
            f32    f1xT[8], f1yT[8], f1zT[8];
            f32    f2xT[8], f2yT[8], f2zT[8];
            f32    f3xT[8], f3yT[8], f3zT[8];
            f32    e1xT[8], e1yT[8], e1zT[8];
            f32    e2xT[8], e2yT[8], e2zT[8];
            f32    e3xT[8], e3yT[8], e3zT[8];
            f32    bboxMinXT[8], bboxMaxXT[8];
            f32    bboxMinYT[8], bboxMaxYT[8];
            f32    vz1T[8], vz2T[8], vz3T[8];
            ...
            for (int i = 0; i < cands; i++)
            {
                ...
                atriShade.bx                = SVector3f(0, 0, 1);
                atriShade.by                = SVector3f(1, 0, 0);
                atriShade.f1                = SVector4f(f1xT[i], f1yT[i], f1zT[i], vz1T[i]);
                atriShade.f2                = SVector4f(f2xT[i], f2yT[i], f2zT[i], vz2T[i]);
                atriShade.f3                = SVector4f(f3xT[i], f3yT[i], f3zT[i], vz3T[i]);
                atriShade.originalPrimitive = candPrim[i];
                atriRaster.e1               = SVector3f(e1xT[i], e1yT[i], e1zT[i]);
                atriRaster.e2               = SVector3f(e2xT[i], e2yT[i], e2zT[i]);
                atriRaster.e3               = SVector3f(e3xT[i], e3yT[i], e3zT[i]);

                SVector4f bbox = { bboxMinXT[i], bboxMinYT[i], bboxMaxXT[i], bboxMaxYT[i] };
                executeBinner(SizeCast<int>(xid) + vid, atriRaster, bbox);
        ...
        while ((curChunk = renderer->fetchUnresolvedChunkGeometry()) >= 0)
        {
            ...
            for (int j = start; j < lim; j += context->vertexStride)
            {
                ...
                SVector4f  v1 = ToSimdVector(posBuffer[id0]);
                SVector4f  v2 = ToSimdVector(posBuffer[id1]);
                SVector4f  v3 = ToSimdVector(posBuffer[id2]);

                const auto prim = (u32)j / context->vertexStride;
    ...
    void TileRasterWorker::rasterizationSingleTile(TileRasterRenderer* renderer, int tileId) IFRIT_AP_NOTHROW
    {
        ...
        curTile     = tileId;
        int tileIdX = (u32)curTile % (u32)context->numTilesX;
        int tileIdY = (u32)curTile / (u32)context->numTilesX;

        f32 tileMinX = tileIdX * context->tileWidth * 1.0f;
        f32 tileMinY = tileIdY * context->tileWidth * 1.0f;
        f32 tileMaxX = (tileIdX + 1) * context->tileWidth * 1.0f;
        f32 tileMaxY = (tileIdY + 1) * context->tileWidth * 1.0f;
        ...
        for (int T = TOTAL_THREADS - 1; T >= 0; T--)
        {
            ...
            for (int j = SizeCast<int>(proposalT.size()) - 1; j >= 0; j--)
            {
                ...
                IF_CONSTEXPR int leftBlock   = 0;
                IF_CONSTEXPR int rightBlock  = TileRasterContext::numSubtilesPerTileX;
                IF_CONSTEXPR int topBlock    = 0;
                IF_CONSTEXPR int bottomBlock = TileRasterContext::numSubtilesPerTileX;
                ...
                npropPixel128.clipTrianglePacked = cvrsQueuePack(propWorkerId, propPrimId);

                TileBinProposal npropPixel256;
                npropPixel256.level              = TileRasterLevel::PIXEL_PACK4X2;
                npropPixel256.clipTrianglePacked = cvrsQueuePack(propWorkerId, propPrimId);
                ...
                for (int x = leftBlock; x < rightBlock; x += 4)
                {
                    ...
                    for (int y = topBlock; y < bottomBlock; y += 2)
                    {
                        ...
                        __m256  subTileMinY256 = _mm256_fmadd_ps(y256f, xTileWidth256f,
                        ...
                        for (int k = 0; k < 3; k++)
                        {
                            criteriaLocalTR256[k] =
                                _mm256_fmadd_ps(edgeCoefs256Y[k], tileCoordsY256[chosenCoordTR[k]], critLocalTRX[k]);
                            criteriaLocalTA256[k] =
                                _mm256_fmadd_ps(edgeCoefs256Y[k], tileCoordsY256[chosenCoordTA[k]], critLocalTRY[k]);
                            __m256i criteriaTRMask =
                                _mm256_castps_si256(_mm256_cmp_ps(criteriaLocalTR256[k], edgeCoefs256Z[k], _CMP_LT_OS));
                            __m256i criteriaTAMask =
                                _mm256_castps_si256(_mm256_cmp_ps(criteriaLocalTA256[k], edgeCoefs256Z[k], _CMP_LT_OS));
                        ...
                        for (int i = 0; i < 8; i++)
                        {
                            ...
                            if (criteriaTA[i] == -3)
                            {
                                npropBlock.tile = IntegerPack32(dwX[i], dwY[i]);
                ...
                for (int x = leftBlock; x < rightBlock; x += 2)
                {
                    for (int y = topBlock; y < bottomBlock; y += 2)
                    {
                        ...
                        for (int k = 0; k < 3; k++)
                        {
                            criteriaLocalTR128[k] = _mm_fmadd_ps(edgeCoefs128X[k], tileCoordsX128[chosenCoordTR[k]],
                        ...
                        for (int i = 0; i < 4; i++)
                        {
                            ...
                            if (criteriaTA[i] == -3)
                            {
                                npropBlock.tile = IntegerPack32(dwX, dwY);
                                coverQueue.push_back(npropBlock);
                            }
                            else
                            {
                                ...
                                for (int sk = 0; sk < 2; sk++)
                                {
                                    ...
                                    if (accept256Mask8 == -1)
                                    {
                                        npropPixel256.tile = IntegerPack32(dx, dy);
                                ...
                                for (int dx = subTileMinX; dx < subTileMaxX; dx += 2)
                                {
                                    for (int dy = subTileMinY; dy < subTileMaxY; dy += 2)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            {
                                                auto pixelId    = dx + dy * TILE_RASTER_TEMP_CONST_MAXW;
                                                npropPixel.tile = IntegerPack32(pixelId, accept128Mask & 0x0000FFFF);
                                                coverQueue.push_back(npropPixel);
                                            }
                                        }
                                    }
                ...
                IF_CONSTEXPR auto xTileWidthf   = TileRasterContext::subtileBlockWidth;
                for (int x = leftBlock; x < rightBlock; x += 1)
                {
                    for (int y = topBlock; y < bottomBlock; y += 1)
                    {
                        ...
                        int      criteriaTR  = 0;
                        int      criteriaTA  = 0;
                        Vector3f tileCoords[4];
                        tileCoords[VLT] = { (f32)subTileMinX, (f32)subTileMinY, 1.0 };
                        tileCoords[VLB] = { (f32)subTileMinX, (f32)subTileMaxY, 1.0 };
                        tileCoords[VRB] = { (f32)subTileMaxX, (f32)subTileMaxY, 1.0 };
                        ...
                        if (criteriaTA == 3)
                        {
                            npropBlock.tile = IntegerPack32(x, y);
                            coverQueue.push_back(npropBlock);
                        }
                        else
                        {
                            for (int dx = subTileMinX; dx < subTileMaxX; dx += 2)
                            {
                                for (int dy = subTileMinY; dy < subTileMaxY; dy += 2)
                                {
                                    ...
                                    for (int k = 0; k < 4; k++)
                                    {
                                        ...
                                        {
                                            f32 criteria = edgeCoefs[i2].x * px + edgeCoefs[i2].y * py;
                                    ...
                                    {
                                        auto pixelId    = dx + dy * TILE_RASTER_TEMP_CONST_MAXW;
                                        npropPixel.tile = IntegerPack32(pixelId, mask & 0x0000FFFF);
                                        coverQueue.push_back(npropPixel);
                                    }
                                }
                            }
        ...
        TileRasterRenderer* renderer, bool clearedDepth, int tileId) IFRIT_AP_NOTHROW
    {
        ...
        const int curTileX  = (u32)curTile % (u32)context->numTilesX;
        const int curTileY  = (u32)curTile / (u32)context->numTilesX;
        ...
        auto proposalProcessFunc = [&]<bool tpAlphaBlendEnable, IfritCompareOp tpDepthFunc, bool tpOnlyTaggingPass>(
            ...
            const auto& triProposal         = atriShadeQueue[propClipTriWorkerId][propClipTriPrimId];
            if (proposal.level == TileRasterLevel::PIXEL)
            {
                ...
                auto dcx          = propY;
#ifdef IFRIT_USE_SIMD_128
                IF_CONSTEXPR bool hasSimd128 = true;
#else
                IF_CONSTEXPR bool hasSimd128   = false;
#endif
            ...
            else if (proposal.level == TileRasterLevel::BLOCK)
            {
                ...
                auto propX           = IntegerUnpack32From64First(proposalTile);
                auto propY           = IntegerUnpack32From64Second(proposalTile);
                auto subTilePixelX   = (curTileX * subtileXPerTile + propX) * context->subtileBlockWidth;
                auto subTilePixelY   = (curTileY * subtileXPerTile + propY) * context->subtileBlockWidth;
                auto subTilePixelX2  = (curTileX * subtileXPerTile + propX + 1) * context->subtileBlockWidth;
                auto subTilePixelY2  = (curTileY * subtileXPerTile + propY + 1) * context->subtileBlockWidth;
                subTilePixelX2       = std::min(subTilePixelX2, (u32)frameBufferWidth);
                subTilePixelY2       = std::min(subTilePixelY2, (u32)frameBufferHeight);
                ...
                {
                    for (int dx = subTilePixelX; dx < static_cast<int>(subTilePixelX2); dx += 4)
                    {
        ...
        if (context->optForceDeterministic)
        {
            auto iterFunc = [&]<bool tpAlphaBlendEnable, IfritCompareOp tpDepthFunc, bool tpOnlyTaggingPass>() {
                ...
                if (clearedDepth)
                {
                    IF_CONSTEXPR auto inftyDepth = getInftyDepthValue();
                    MemSetDword(depthCache, inftyDepth, tagbufferSizeX * tagbufferSizeX);
                }
                else
                {
...
#define IF_DECLPS_ITERFUNC_0(tpAlphaBlendEnable, tpDepthFunc, tpOnlyTaggingPass) \
    iterFunc.operator()<tpAlphaBlendEnable, tpDepthFunc, tpOnlyTaggingPass>();
#define IF_DECLPS_ITERFUNC_0_BRANCH(tpAlphaBlendEnable, tpDepthFunc, tpOnlyTaggingPass) \
            ...
            auto  iterFunc = [&]<bool tpAlphaBlendEnable, IfritCompareOp tpDepthFunc, bool tpOnlyTaggingPass>() {
                ...
                if IF_CONSTEXPR (tpOnlyTaggingPass)
                {
                    ...
                    if (directReturn && coverQueueLocalSize == 0)
                        return;
                    MemSetDword(tagbuf.valid, -1, tagbufferSizeX * tagbufferSizeX);
                }
                // Cache depth
                if (clearedDepth)
                {
                ...
                for (int i = TOTAL_THREADS - 1; i >= 0; i--)
                {
                    ...
                    for (int j = SizeCast<int>(pq.size()) - 1; j >= 0; j--)
                    {
                        auto                               proposal     = pq[j];
                        auto                               propPrimId   = cvrsQueuePrimitiveId(proposal);
                        auto                               propWorkerId = cvrsQueueWorkerId(proposal);
                ...
                // TileLocal
                for (int j = SizeCast<int>(coverQueueLocalSize) - 1; j >= 0; j--)
                {
...
#undef IF_DECLPS_ITERFUNC
        ...
        const int dxA, const int dyA, const PixelShadingFuncArgs& args) IFRIT_AP_NOTHROW
    {
        ...
        SVector4f* vaPtr[TILE_RASTER_MAX_VARYINGS];
        ...
        SVector4f   interpVaryingsQuad[8][TILE_RASTER_MAX_VARYINGS];
        SVector4f   colorOutpQuad[8][TILE_RASTER_MAX_VARYINGS];
        ...
        for (int i = 0; i < tagbufferSizeX * tagbufferSizeX; i += (8 >> 1))
        {
    ...
    #ifdef _MSC_VER
            __m256i dx256T1 = _mm256_and_epi32(dxId256,
            ...
            __m256  baryVecX = _mm256_i32gather_ps(((f32*)ptrAtpBary), gatherIdx, 4);
            __m256  baryVecY = _mm256_i32gather_ps(((f32*)ptrAtpBary) + 1, gatherIdx, 4);
            __m256  baryVecZ = _mm256_i32gather_ps(((f32*)ptrAtpBary) + 2, gatherIdx, 4);

            __m256  atpBxX = _mm256_i32gather_ps(((f32*)ptrAtpBx), gatherIdx, 4);
            __m256  atpBxY = _mm256_i32gather_ps(((f32*)ptrAtpBx) + 1, gatherIdx, 4);
            __m256  atpBxZ = _mm256_i32gather_ps(((f32*)ptrAtpBx) + 2, gatherIdx, 4);

            __m256  atpByX = _mm256_i32gather_ps((f32*)ptrAtpBy, gatherIdx, 4);
            __m256  atpByY = _mm256_i32gather_ps(((f32*)ptrAtpBy) + 1, gatherIdx, 4);
            ...
            __m256 atpF1X = _mm256_i32gather_ps((f32*)ptrAtpF1F2, gatherIdx, 4);
            __m256 atpF1Y = _mm256_i32gather_ps(((f32*)ptrAtpF1F2) + 1, gatherIdx, 4);
            __m256 atpF2X = _mm256_i32gather_ps(((f32*)ptrAtpF1F2) + 2, gatherIdx, 4);
            __m256 atpF2Y = _mm256_i32gather_ps(((f32*)ptrAtpF1F2) + 3, gatherIdx, 4);

            __m256 atpF3X = _mm256_i32gather_ps((f32*)ptrAtpF3, gatherIdx, 2);
            __m256 atpF3Y = _mm256_i32gather_ps(((f32*)ptrAtpF3) + 1, gatherIdx, 2);

            f32    baryVecXT[8], baryVecYT[8], baryVecZT[8];
            _mm256_storeu_ps(baryVecXT, baryVecX);
            _mm256_storeu_ps(baryVecYT, baryVecY);
            _mm256_storeu_ps(baryVecZT, baryVecZ);
            f32 f1xT[8], f1yT[8], f2xT[8], f2yT[8], f3xT[8], f3yT[8];
                ...
                for (int j = T * 4; j < T * 4 + 4; j++)
                {
                    ...
                    for (int k = T * 4; k < T * 4 + 4; k++)
                    {
                        for (int v = 0; v < varyCnts; v++)
                        {
                            ...
                            SVector4f   destVec = tmp0 * desiredBaryX[k & 0x3];
        ...
        const int dxA, const int dyA, const PixelShadingFuncArgs& args) IFRIT_AP_NOTHROW
    {
        ...
        IF_CONSTEXPR auto LoopIncre = 8;
#else
        IF_CONSTEXPR auto LoopIncre = 1;
#endif
        auto ptrCol0 = args.colorAttachment0;
        for (int i = 0; i < tagbufferSizeX * tagbufferSizeX; i += LoopIncre)
        {
            ...
            desiredBaryR[0] =
                _mm256_fmadd_ps(baryVecX, atpBxX, _mm256_fmadd_ps(baryVecY, atpBxY, _mm256_mul_ps(baryVecZ, atpBxZ)));
            desiredBaryR[1] =
                _mm256_fmadd_ps(baryVecX, atpByX, _mm256_fmadd_ps(baryVecY, atpByY, _mm256_mul_ps(baryVecZ, atpByZ)));
            ...
            f32 desiredBaryX[8], desiredBaryY[8], desiredBaryZ[8];
            ...
            for (int j = 0; j < 8; j++)
            {
                ...
                for (int k = 0; k < varyCnts; k++)
                {
                    ...
                    const auto& tmp1    = (va[addr[1]]);
                    const auto& tmp2    = (va[addr[2]]);
                    SVector4f   destVec = tmp0 * desiredBaryX[j];
            ...
            SVector3f baryVec           = tagBuffer.tagBufferBary[dxId];
            SVector3f atpBx             = tagBuffer.atpBx[dxId];
            SVector3f atpBy             = tagBuffer.atpBy[dxId];
            auto      idx               = tagBuffer.valid[dxId] * context->vertexStride;
            f32       interpolatedDepth = depthCache[i];
            if (idx < 0)
                continue;

            f32 zCorr = FastApproxReciprocal(hsum(baryVec));
            baryVec *= zCorr;
            ...
            for (int j = 0; j < vSize; j++)
            {
                ...
                const auto& tmp1    = (va[addr[1]]);
                const auto& tmp2    = (va[addr[2]]);
                SVector4f   destVec = tmp0 * desiredBary[0];
        ...
        const int dx, const int dy, const PixelShadingFuncArgs& args) IFRIT_AP_NOTHROW
    {
        ...
        __m128     bary[3];
        SVector4f  valF1      = atp.f1;
        SVector4f  valF2      = atp.f2;
        SVector4f  valF3      = atp.f3;
        ...
        SVector3f atpBxVec       = atp.bx;
        SVector3f atpByVec       = atp.by;
        u32       udx            = dx;
        u32       udy            = dy;
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            if (!(quadMask & targetMask))
                continue;
            u32        dx1              = ((u32)x) % tagbufferSizeX;
            u32        dy1              = ((u32)y) % tagbufferSizeX;
            u32        dxId             = dx1 + dy1 * tagbufferSizeX;
            const auto depthAttachment2 = depthCache[dxId];
            if IF_CONSTEXPR (tpDepthFunc == IF_COMPARE_OP_EQUAL)
            {
                if (interpolatedDepth[i] != depthAttachment2)
                    continue;
            }
            if IF_CONSTEXPR (tpDepthFunc == IF_COMPARE_OP_GREATER)
            {
                if (interpolatedDepth[i] <= depthAttachment2)
                    continue;
            }
            if IF_CONSTEXPR (tpDepthFunc == IF_COMPARE_OP_GREATER_OR_EQUAL)
            {
                if (interpolatedDepth[i] < depthAttachment2)
                    continue;
            }
            if IF_CONSTEXPR (tpDepthFunc == IF_COMPARE_OP_LESS)
            {
                if (interpolatedDepth[i] >= depthAttachment2)
                    continue;
            }
            if IF_CONSTEXPR (tpDepthFunc == IF_COMPARE_OP_LESS_OR_EQUAL)
            {
                if (interpolatedDepth[i] > depthAttachment2)
                    continue;
            }
            if IF_CONSTEXPR (tpDepthFunc == IF_COMPARE_OP_NEVER)
            {
                return;
            }
            if IF_CONSTEXPR (tpDepthFunc == IF_COMPARE_OP_NOT_EQUAL)
            {
                if (interpolatedDepth[i] == depthAttachment2)
                    continue;
            }
            SVector3f bary32Vec = SVector3f(bary32[0][i], bary32[1][i], bary32[2][i]);
            if IF_CONSTEXPR (tpOnlyTaggingPass)
            {
                ...
                if (valForcedQuads)
                {
                    ptrAtpF1F2[dxId] = SVector4f(valF1.x, valF1.y, valF2.x, valF2.y);
    ...
    template <bool tpAlphaBlendEnable, IfritCompareOp tpDepthFunc, bool tpOnlyTaggingPass>
    void TileRasterWorker::pixelShading(const AssembledTriangleProposalShadeStage& atp, const int dx, const int dy,
        const PixelShadingFuncArgs& args) IFRIT_AP_NOTHROW
    {
        u32 dx1  = (u32)dx % tagbufferSizeX;
        u32 dy1  = (u32)dy % tagbufferSizeX;
        u32 dxId = dx1 + dy1 * tagbufferSizeX;
        ...
        SVector4f pDxDyVec = SVector4f(dx * 1.0f, dy * 1.0f, 1.0f, 0.0f);
        SVector3f zVec     = SVector3f(atp.f1.w, atp.f2.w, atp.f3.w);

        f32       bary[3];
        f32       interpolatedDepth;
        bary[0]           = Dot(pDxDyVec, atp.f1);
        bary[1]           = Dot(pDxDyVec, atp.f2);
        bary[2]           = Dot(pDxDyVec, atp.f3);
        SVector3f baryVec = SVector3f(bary[0], bary[1], bary[2]);
        ...
        if IF_CONSTEXPR (tpOnlyTaggingPass)
        {
            ...
            {
                args.tagBuffer->atpF1F2[dxId] = SVector4f(atp.f1.x, atp.f1.y, atp.f2.x, atp.f2.y);
        ...
        f32 zCorr = 1.0f / hsum(baryVec);
        baryVec *= zCorr;
        SVector3f atpBx = atp.bx;
        SVector3f atpBy = atp.by;
        f32       desiredBary[3];
        desiredBary[0] = Dot(baryVec, atpBx);
        desiredBary[1] = Dot(baryVec, atpBy);
        desiredBary[2] = 1.0f - desiredBary[0] - desiredBary[1];
        ...
        if IF_CONSTEXPR (tpAlphaBlendEnable)
        {
...
#ifdef IFRIT_USE_SIMD_128
            args.colorAttachment0->fillPixelRGBA128ps(dx, dy, _mm_loadu_ps((f32*)(&mixRgba)));
#else
        ...
        const int dy, const PixelShadingFuncArgs& args) IFRIT_AP_NOTHROW
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            // f32 barytmp[3] = { bary32[0][i] * zCorr32[i],bary32[1][i] *
            ...
            if IF_CONSTEXPR (tpAlphaBlendEnable)
            {
                ...
                mixRgba.z = dstRgba[2] * mxDstX + srcRgba.z * mxSrcX;
                mixRgba.w = dstRgba[3] * mxDstA + srcRgba.w * mxSrcA;
                args.colorAttachment0->fillPixelRGBA128ps(x, y, _mm_loadu_ps((const f32*)(&mixRgba)));
            }
            else
            {
                args.colorAttachment0->fillPixelRGBA128ps(x, y, _mm_loadu_ps((const f32*)(&colorOutput[0])));
            }
            // Depth Write
        ...
        int groupsY, const int dx, const int dy, const PixelShadingFuncArgs& args) IFRIT_AP_NOTHROW
    {
        ...
        u32        offx             = dx - 4;
        u32        offy             = dy - 2;
        SVector4f  tAtpF1 = atp.f1, tAtpF2 = atp.f2, tAtpF3 = atp.f3;
        __m256     posZ[3];
        ...
        for (int gx = 0; gx < groupsX; gx++)
        {
            ...
            baryBase[0] = _mm256_fmadd_ps(_mm256_set1_ps(tAtpF1.x), pDx, _mm256_set1_ps(tAtpF1.z));
            baryBase[1] = _mm256_fmadd_ps(_mm256_set1_ps(tAtpF2.x), pDx, _mm256_set1_ps(tAtpF2.z));
            baryBase[2] = _mm256_fmadd_ps(_mm256_set1_ps(tAtpF3.x), pDx, _mm256_set1_ps(tAtpF3.z));
            ...
            for (int gy = 0; gy < groupsY; gy++)
            {
                ...
                auto depthTestResult     = depthTestSIMD256<tpDepthFunc>(interpolatedDepth256, depthCache256);
                ...
                for (int i = 0; i < 8; i++)
                {
                    ...
                    if IF_CONSTEXPR (tpOnlyTaggingPass)
                    {
                        ...
                        if (valForcedQuads)
                        {
                            ptrAtpF1[dxId] = SVector4f(tAtpF1.x, tAtpF1.y, tAtpF2.x, tAtpF2.y);
...
#define IF_DECLPS1(tpAlphaBlending, tpDepthFunc, tpOnlyTaggingPass)                                                   \
    template void TileRasterWorker::pixelShadingSingleQuad<tpAlphaBlending, tpDepthFunc, tpOnlyTaggingPass>(          \
        const AssembledTriangleProposalShadeStage& atp, int quadMask, const int dx, const int dy,                     \
        const PixelShadingFuncArgs& args) IFRIT_AP_NOTHROW;                                                           \
    template void TileRasterWorker::pixelShading<tpAlphaBlending, tpDepthFunc, tpOnlyTaggingPass>(                    \
        const AssembledTriangleProposalShadeStage& atp, const int dx, const int dy, const PixelShadingFuncArgs& args) \
        IFRIT_AP_NOTHROW;                                                                                             \
    template void TileRasterWorker::pixelShadingSIMD128<tpAlphaBlending, tpDepthFunc, tpOnlyTaggingPass>(             \
        const AssembledTriangleProposalShadeStage& atp, const int dx, const int dy, const PixelShadingFuncArgs& args) \
        IFRIT_AP_NOTHROW;                                                                                             \
    template void TileRasterWorker::pixelShadingSIMD256<tpAlphaBlending, tpDepthFunc, tpOnlyTaggingPass>(             \
        const AssembledTriangleProposalShadeStage& atp, const int dx, const int dy, const PixelShadingFuncArgs& args) \
        IFRIT_AP_NOTHROW;                                                                                             \
    template void TileRasterWorker::pixelShadingSIMD256Grouped<tpAlphaBlending, tpDepthFunc, tpOnlyTaggingPass>(      \
        const AssembledTriangleProposalShadeStage& atp, int groupsX, int groupsY, const int dx, const int dy,         \
        const PixelShadingFuncArgs& args) IFRIT_AP_NOTHROW;
