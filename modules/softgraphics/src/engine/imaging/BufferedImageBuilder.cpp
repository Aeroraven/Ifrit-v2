...
{
    std::shared_ptr<BufferedImage> BufferedImageBuilder::build(const IfritImageCreateInfo& pCI)
    {
        ...
        if (intCI.tiling == BufferedImageTiling::TILING_LINEAR)
        {
            ...
            {
                return MakeRef<BufferedImageLinear<uint8_t>>(intCI);
            }
            else if (intCI.format == BufferedImageFormat::FORMAT_R32G32B32A32_SFLOAT)
            {
                return MakeRef<BufferedImageLinear<float>>(intCI);
            }
        }
        else if (intCI.tiling == BufferedImageTiling::TILING_TILED)
        {
            if (intCI.format == BufferedImageFormat::FORMAT_R8G8B8A8_UINT)
            {
                return MakeRef<BufferedImageTiled<uint8_t>>(intCI);
            }
            else if (intCI.format == BufferedImageFormat::FORMAT_R32G32B32A32_SFLOAT)
            {
                return MakeRef<BufferedImageTiled<float>>(intCI);
            }
        }
        return nullptr;
    }
