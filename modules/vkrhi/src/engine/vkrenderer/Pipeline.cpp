...
#include "ifrit/vkrhi/utility/Logger.h"
...
namespace Ifrit::RHI::VulkanAdapter
{
    ...
    IFRIT_APIDECL void GraphicsPipeline::Init()
    {
        ...
        dynamicStateCI.dynamicStateCount = SizeCast<int>(dynamicStates.size());
        ...
        renderCI.colorAttachmentCount    = SizeCast<int>(m_createInfo.colorAttachmentFormats.size());
        ...
        // Color blending info
        Vec<VkPipelineColorBlendAttachmentState> colorBlendAttachment;
        ...
        colorBlendCI.attachmentCount = SizeCast<int>(colorBlendAttachment.size());
        ...
        pipelineLayoutCI.setLayoutCount         = SizeCast<int>(m_createInfo.descriptorSetLayouts.size());
        ...
        pipelineCI.stageCount          = SizeCast<int>(shaderStages.size());
        ...
        auto            cachePath = cacheDir + "/vkrhi.psographics." + digest + ".cache";

        VkPipelineCache cache;

        bool            cacheExists = false;
        std::ifstream   cacheFile(cachePath, std::ios::binary);
        Vec<uint8_t>    cacheData;
    ...
    IFRIT_APIDECL void ComputePipeline::Init()
    {
        ...
        auto            cacheDir  = m_context->GetCacheDir();
        auto            cachePath = cacheDir + "/vkrhi.psocomp." + digest + ".cache";
