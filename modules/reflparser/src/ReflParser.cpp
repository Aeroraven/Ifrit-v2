...
namespace Ifrit::ReflParser
{
    ...
    void GenerateCode(const std::string& outdir, const ReflectionParserContext& ctx)
    {
        ...
        headerStream << "// ****************************************************************\n";
        headerStream << "// This file is auto-generated by ifrit.reflparser.\n";
        headerStream << "// Do not edit this file manually.\n";
        headerStream << "// ****************************************************************\n\n";
        headerStream << "#pragma once\n";
        headerStream << "#include \"ifrit/core/reflection/Reflection.h\"\n\n";
        headerStream << "namespace Ifrit::Reflection\n{\n";
        headerStream << "    void RegisterReflectionTypes();\n";
        headerStream << "}\n";

        // Generate CPP file content
        std::stringstream cppStream;
        cppStream << "// ****************************************************************\n";
        cppStream << "// This file is auto-generated by ifrit.reflparser.\n";
        cppStream << "// Do not edit this file manually.\n";
        cppStream << "// ****************************************************************\n\n";
        cppStream << "#include \"" << std::filesystem::path(outdir).stem().string() << ".generated.h\"\n\n";
        cppStream << "// Begin Body\n";
        for (const auto& record : ctx.records)
        {
            if (record.type == RecordType::IncludeFiles)
            {
                cppStream << "#include \"" << record.symbolName << "\"\n";
            }
        }
        cppStream << "namespace Ifrit::Reflection\n{\n";
        cppStream << "    void RegisterReflectionTypes()\n    {\n";
        for (const auto& record : ctx.records)
        {
            if (record.type == RecordType::Class)
            {
                cppStream << "\n        // " << record.symbolName << "\n";
                cppStream << "        RegisterType<" << record.symbolName << ">();\n";
            }
            else if (record.type == RecordType::Property)
            {
                cppStream << "        RegisterPropertyField<&" << record.symbolName << ">(\"" << record.propertyAlias
                          << "\");\n";
                if (record.node)
                    PropParse::printNode(*record.node, record.symbolName, cppStream, "");
            }
            else if (record.type == RecordType::Function)
            {
                cppStream << "        RegisterMethodField<&" << record.symbolName << ">(\"" << record.propertyAlias
                          << "\");\n";
            }
            else if (record.type == RecordType::PolymorphicRelation)
            {
                cppStream << "        RegisterPolymorphicRelation<" << record.symbolName << ", " << record.propertyAlias
                          << ">();\n";
            }
        }
        cppStream << "    }\n";
        ...
        // Write header file
        std::string headerFilePath = outdir + ".h";
        if (std::filesystem::exists(headerFilePath))
        {
            std::ifstream     existingFile(headerFilePath);
            std::stringstream existingContentStream;
            existingContentStream << existingFile.rdbuf();
            std::string existingContent = existingContentStream.str();
            existingFile.close();

            if (existingContent == headerStream.str())
            {
                LogInfo("No changes detected in the generated header file: ", headerFilePath);
            }
            else
            {
                std::ofstream outFile(headerFilePath);
                outFile << headerStream.str();
                outFile.close();
                LogInfo("Generated header file written: ", headerFilePath);
            }
        }
        else
        {
        ...
        // Write CPP file
        std::string cppFilePath = outdir + ".cpp";
        if (std::filesystem::exists(cppFilePath))
        {
            std::ifstream     existingFile(cppFilePath);
            std::stringstream existingContentStream;
            existingContentStream << existingFile.rdbuf();
            std::string existingContent = existingContentStream.str();
            existingFile.close();

            if (existingContent == cppStream.str())
            {
                LogInfo("No changes detected in the generated CPP file: ", cppFilePath);
            }
            else
            {
                std::ofstream outFile(cppFilePath);
                outFile << cppStream.str();
                outFile.close();
                LogInfo("Generated CPP file written: ", cppFilePath);
            }
        }
        else
        {
