...
//         IF_LOG_ASSERTION(
//             "GLTFAsset", accessorNormal.componentType == TINYGLTF_COMPONENT_TYPE_FLOAT, "component type not
//             supported");
...
//                 {
//                     auto prefab = MakeRef<GLTFPrefab>(keeper, &m_metadata, this, node.mesh, j, nodeId,
//                     parentTransform); auto meshFilter = prefab->m_prefab->AddComponent<MeshFilter>(); auto mesh =
//                     m_meshes[meshHash[{ SizeCast<u32>(node.mesh), j }]]; meshFilter->SetMesh(mesh);
