...
namespace Ifrit::Runtime
{
    ...
    IFRIT_APIDECL void SyaroRenderer::CreatePostprocessTextures(u32 width, u32 height)
    {
        ...
        for (u32 i = 0; i < 2; i++)
        {
            ...
            auto colorRT = rhi->CreateRenderTarget(
                tex.get(), RHI::CreateRhiClearColorValue(Vector4f(0.0f)), RhiRenderTargetLoadOp::Load, 0, 0);
            auto rts = rhi->CreateRenderTargets();

            m_postprocTex[{ width, height }][i]     = tex;
            m_postprocTexSRV[{ width, height }][i]  = rhi->GetSRVDescriptor(tex.get());
        ...
        PerFrameData& perframeData, RenderTargets* renderTargets, const GPUCmdBuffer* cmd)
    {
        ...
        passAtmosphere.SetExecutionFunction([&](const FrameGraphPassContext& data) {
            ...
            pushConst.m_depthTex = primaryView.m_visibilityDepthIdSRV_Combined;
            m_atmospherePass->SetRecordFunction([&](const RhiRenderPassContext* ctx) {
                ctx->m_cmd->SetPushConst(&pushConst, 0, sizeof(AtmoPushConst));
        ...
        passDeferredShadow.SetExecutionFunction([&](const FrameGraphPassContext& data) {
            ...
            pc.depthTexRef      = primaryView.m_visibilityDepthIdSRV_Combined;
        ...
        passBlurShadowHori.SetExecutionFunction([&](const FrameGraphPassContext& data) {
            ...
            auto deferShadowId = perframeData.m_deferShadowMaskId;
        ...
        passBlurShadowVert.SetExecutionFunction([&](const FrameGraphPassContext& data) {
            ...
            m_gaussianVert->RenderPostFx(data.m_CmdList, perframeData.m_deferShadowMaskRTs.get(), postprocId, 3);
        });
        ...
        passDeferredShading.SetExecutionFunction([&](const FrameGraphPassContext& data) {
            ...
            pc.shadowMapDataRef = perframeData.m_shadowData2.m_allShadowDataId->GetActiveId();
            pc.depthTexRef      = primaryView.m_visibilityDepthIdSRV_Combined;
            pc.shadowTexRef     = perframeData.m_deferShadowMaskId;
            pc.m_GIMode         = 0; // AO
        ...
        passGlobalFog.SetExecutionFunction([&](const FrameGraphPassContext& data) {
            ...
            auto inputId       = m_postprocTexSRV[{ mainRtWidth, mainRtHeight }][0];
            auto inputDepthId  = primaryView.m_visibilityDepthIdSRV_Combined;
            auto primaryViewId = primaryView.m_viewBufferId.get();
            m_globalFogPass->RenderPostFx(
                data.m_CmdList, fogRT.get(), inputId, inputDepthId, primaryViewId->GetActiveId());
        });
        ...
        if (m_config->m_AntiAliasingType == AntiAliasingType::TAA)
        {
            ...
            passConvBloom.SetExecutionFunction([&](const FrameGraphPassContext& data) {
                ...
                auto postprocTex0Id = m_postprocTexSRV[{ width, height }][0];
                auto postprocTex1Id = m_postprocTex[{ width, height }][1]->GetDescId();
                m_fftConv2d->RenderPostFx(commandList, postprocTex0Id, postprocTex1Id, ~0u, width, height, 51, 51, 4);
                commandList->EndScope();
            });
        }
        else
        {
            ...
            passConvBloom.SetExecutionFunction([&](const FrameGraphPassContext& data) {
                ...
                auto width          = mainRtWidth;
                auto height         = mainRtHeight;
                auto postprocTex0Id = perframeData.m_taaHistory[perframeData.m_frameId % 2].m_colorRTIdSRV;
                auto postprocTex1Id = m_postprocTex[{ width, height }][1]->GetDescId();
                m_fftConv2d->RenderPostFx(commandList, postprocTex0Id, postprocTex1Id, ~0u, width, height, 51, 51, 4);
                commandList->EndScope();
            });
        ...
        if (m_config->m_AntiAliasingType == AntiAliasingType::FSR2)
        {
            passFsr2Dispatch.SetExecutionFunction([&](const FrameGraphPassContext& data) {
                ...
                RHI::FSR2::RhiFSR2DispatchArgs args;
        ...
        if (m_config->m_AntiAliasingType == AntiAliasingType::FSR2)
        {
            ...
            passToneMapping.SetExecutionFunction([&](const FrameGraphPassContext& data) {
                m_acesToneMapping->RenderPostFx(
                    data.m_CmdList, renderTargets, perframeData.m_fsr2Data.m_fsr2OutputSRVId);
            });
        }
        else
        {
            passToneMapping.SetExecutionFunction([&](const FrameGraphPassContext& data) {
                m_acesToneMapping->RenderPostFx(
                    data.m_CmdList, renderTargets, m_postprocTexSRV[{ mainRtWidth, mainRtHeight }][0]);
            });
        }
        ...
        PerFrameData& perframeData, RenderTargets* renderTargets)
    {
        ...
        if (needRecreateMat)
        {
            perframeData.m_matClassSupportedNumMaterials = Ifrit::SizeCast<u32>(numMaterials);
            auto CreateSize =
                cMatClassCounterBufferSizeBase + cMatClassCounterBufferSizeMult * Ifrit::SizeCast<u32>(numMaterials);
    ...
    IFRIT_APIDECL void SyaroRenderer::DepthTargetsSetup(PerFrameData& perframeData, RenderTargets* renderTargets)
    {
        ...
        auto& primaryView = GetPrimaryView(perframeData);
        perframeData.m_velocityMaterialDesc->AddSRVImage(primaryView.m_visibilityBuffer_Combined.get(), 1);
        ...
        perframeData.m_gbufferDepthDesc->AddSRVImage(perframeData.m_velocityMaterial.get(), 0);
    }
        ...
        PerFrameData& perframeData, SyaroRenderer::RenderTargets* renderTargets, const GPUCmdBuffer* cmd)
    {
        ...
        m_emitDepthTargetsPass->SetRecordFunction([&](const RhiRenderPassContext* ctx) {
            ...
            ctx->m_cmd->ClearUAVTexture(
                perframeData.m_motionVector.get(), { 0, 0, 1, 1 }, RHI::CreateRhiClearColorValue(Vector4f(0.0f)));
        ...
        RenderTargets* renderTargets, const GPUCmdBuffer* cmd, PerFrameData::ViewType filteredViewType, u32 idx)
    {
        ...
        m_instanceCullingPass->SetRecordFunction([&](const RhiRenderPassContext* ctx) {
            ...
            ctx->m_cmd->AttachUniformRef(3, perView.m_instCullDesc);
            ctx->m_cmd->AttachUniformRef(4, perView.m_spHiZData.m_hizDesc);
        ...
        m_persistentCullingPass->SetRecordFunction([&](const RhiRenderPassContext* ctx) {
            ...
            if (cullPass == CullingPass::First)
            {
                ...
                ctx->m_cmd->SetPushConst(&pcPersistCull, 0, sizeof(PersistCullPushConst));
        ...
        visPassHW->SetRecordFunction([&](const RhiRenderPassContext* ctx) {
            ...
            {
                ctx->m_cmd->SetPushConst(&pcData[1], 0, u32Size);
        ...
        visPassSW->SetRecordFunction([&](const RhiRenderPassContext* ctx) {
            ...
            ctx->m_cmd->SetPushConst(&pcsw, 0, sizeof(SWPushConst));
            if (cullPass == CullingPass::First)
            {
        ...
        combinePass->SetRecordFunction([&](const RhiRenderPassContext* ctx) {
            ...
            pcCombine.hwDepthSRVId = perView.m_visDepthIdSRV_HW;
            ...
            cmd->SetPushConst(&pcCombine, 0, sizeof(pcCombine));
        ...
        PerFrameData& perframeData, RenderTargets* renderTargets, const GPUCmdBuffer* cmd)
    {
        ...
        pc.visBufferSRV = primaryView.m_visibilityBufferIdSRV_Combined;
        ...
        for (u32 k = 0; k < perframeData.m_views.size(); k++)
        {
            ...
            if IF_CONSTEXPR (true)
            {
                ...
                perView.m_visDepthRT_HW    = rhi->CreateRenderTargetDepthStencil(
                    visDepthHW.get(), RHI::CreateRhiClearDepthStencilValue(1.0f, 0), RhiRenderTargetLoadOp::Clear);

                perView.m_visRTs_HW = rhi->CreateRenderTargets();
                if (perView.m_viewType == PerFrameData::ViewType::Primary)
                {
                    perView.m_visColorRT_HW = rhi->CreateRenderTarget(visBufferHW.get(),
                        RHI::CreateRhiClearColorValue(Vector4f(0.0f)), RhiRenderTargetLoadOp::Clear, 0, 0);
                ...
                perView.m_visDepthRT2_HW = rhi->CreateRenderTargetDepthStencil(
                    visDepthHW.get(), RHI::CreateRhiClearDepthStencilValue(1.0f, 0), RhiRenderTargetLoadOp::Load);
                perView.m_visRTs2_HW = rhi->CreateRenderTargets();
                if (perView.m_viewType == PerFrameData::ViewType::Primary)
                {
                    perView.m_visColorRT2_HW = rhi->CreateRenderTarget(visBufferHW.get(),
                        RHI::CreateRhiClearColorValue(Vector4f(0.0f)), RhiRenderTargetLoadOp::Load, 0, 0);
            ...
            if (SYARO_ENABLE_SW_RASTERIZER && perView.m_viewType == PerFrameData::ViewType::Primary)
            {
                ...
                perView.m_visibilityBufferIdSRV_Combined =
                    rhi->GetSRVDescriptor(perView.m_visibilityBuffer_Combined.get());
                perView.m_visibilityDepthIdSRV_Combined =
                    rhi->GetSRVDescriptor(perView.m_visibilityDepth_Combined.get());
            }
            else
            {
        ...
        PerFrameData& perframeData, RenderTargets* renderTargets, const GPUCmdBuffer* cmd)
    {
        ...
        m_defaultEmitGBufferPass->SetRecordFunction([&](const RhiRenderPassContext* ctx) {
            ...
            ctx->m_cmd->ClearUAVTexture(perframeData.m_gbuffer.m_albedo_materialFlags.get(), { 0, 0, 1, 1 },
                RHI::CreateRhiClearColorValue(Vector4f(0.0f)));
            ctx->m_cmd->ClearUAVTexture(perframeData.m_gbuffer.m_normal_smoothness.get(), { 0, 0, 1, 1 },
                RHI::CreateRhiClearColorValue(Vector4f(0.0f)));
            ctx->m_cmd->ClearUAVTexture(
                perframeData.m_gbuffer.m_emissive.get(), { 0, 0, 1, 1 }, RHI::CreateRhiClearColorValue(Vector4f(0.0f)));
        ...
        PerFrameData& perframeData, RenderTargets* renderTargets, const GPUCmdBuffer* cmd)
    {
        ...
        auto aoBlurFunc = [&]() {
            ...
            m_gaussianHori->RenderPostFx(cmd, rt1.get(), perframeData.m_gbuffer.m_specular_occlusion_sampId, 5);
            m_gaussianVert->RenderPostFx(
                cmd, rt2.get(), perframeData.m_gbuffer.m_specular_occlusion_intermediate_sampId, 5);
        };
        ...
        {
            m_aoPass->RenderHBAO(cmd, width, height, depthSamp, normalSamp, ao, perframe->GetActiveId());
        ...
        else if (m_config->m_IndirectLightingType == IndirectLightingType::SSGI)
        {
            ...
            cmd->GlobalMemoryBarrier();
            m_aoPass->RenderSSGI(cmd, width, height, primaryView.m_viewBufferId->GetActiveId(),
                primaryView.m_spHiZDataMin.m_hizRefBuffer->GetDescId(),
                primaryView.m_spHiZData.m_hizRefBuffer->GetDescId(), normalSamp, aoIntermediate, srv,
                primaryView.m_spHiZDataMin.m_hizWidth, primaryView.m_spHiZDataMin.m_hizHeight,
                primaryView.m_spHiZDataMin.m_hizIters, m_immRes.m_blueNoiseSRV, albedoSamp);

            cmd->GlobalMemoryBarrier();
            m_jointBilateralFilter->RenderPostFx(cmd, aoRT.get(), aoIntermediateSRV, normalSamp, depthSamp, 0);
        }
        cmd->GlobalMemoryBarrier();
        cmd->EndScope();
    }
    ...
    IFRIT_APIDECL void SyaroRenderer::PrepareAggregatedShadowData(PerFrameData& perframeData)
    {
        ...
        for (auto d = 0; auto& x : shadowData.m_shadowViews)
        {
            for (auto i = 0u; i < x.m_csmSplits; i++)
            {
                ...
                x.m_texRef[i]  = perframeData.m_views[idx].m_visibilityDepthIdSRV_Combined;
        ...
        if (perframeData.m_deferShadowMask == nullptr)
        {
            ...
            perframeData.m_deferShadowMaskId = rhi->GetSRVDescriptor(perframeData.m_deferShadowMask.get());
        }
    }
    ...
    IFRIT_APIDECL void SyaroRenderer::Fsr2Setup(PerFrameData& perframeData, RenderTargets* renderTargets)
    {
        ...
        perframeData.m_fsr2Data.m_fsr2OutputSRVId = rhi->GetSRVDescriptor(perframeData.m_fsr2Data.m_fsr2Output.get());

        if (m_config->m_AntiAliasingType == AntiAliasingType::FSR2)
        {
            RHI::FSR2::RhiFSR2InitialzeArgs args;
    ...
    IFRIT_APIDECL void SyaroRenderer::TaaHistorySetup(PerFrameData& perframeData, RenderTargets* renderTargets)
    {
        ...
        perframeData.m_taaHistoryDesc->AddSRVImage(perframeData.m_taaUnresolved.get(), 0);
        for (int i = 0; i < 2; i++)
        {
            // TODO: choose formats
            ...
            perframeData.m_taaHistoryDesc->AddSRVImage(perframeData.m_taaHistory[i].m_colorRT.get(), i + 1);
        }
    }
