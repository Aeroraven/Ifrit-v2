...
{
    IFRIT_APIDECL      Square2D::Square2D(f32 m_Width, f32 m_Height) : m_Width(m_Width), m_Height(m_Height) {}

    IFRIT_APIDECL void Square2D::BuildMesh()
    {
        ...
        // Square
        m_SelfData->m_vertices[0] = Vector3f(-m_Width / 2.0f, -m_Height / 2.0f, 0.0f);
        m_SelfData->m_vertices[1] = Vector3f(m_Width / 2.0f, -m_Height / 2.0f, 0.0f);
        m_SelfData->m_vertices[2] = Vector3f(m_Width / 2.0f, m_Height / 2.0f, 0.0f);
        m_SelfData->m_vertices[3] = Vector3f(-m_Width / 2.0f, m_Height / 2.0f, 0.0f);
    ...
    IFRIT_APIDECL Ref<MeshData> Square2D::LoadMesh()
    {
        if (!m_Loaded)
        {
            BuildMesh();
    ...
    IFRIT_APIDECL MeshData* Square2D::LoadMeshUnsafe()
    {
        if (!m_Loaded)
        {
            BuildMesh();
    ...
    IFRIT_APIDECL u32 Square2D::GetNumIndices() { return 6; }
    IFRIT_APIDECL u32 Square2D::GetNumVertices() { return 4; }
