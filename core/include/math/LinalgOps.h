...
namespace Ifrit::Math {
	...
	IFRIT_APIDECL inline float4x4 axisAngleRotation(const ifloat3& axis, float angle) {
	...
	IFRIT_APIDECL inline float4x4 lookAt(ifloat3 eye, ifloat3 center, ifloat3 up) {
		...
		ifloat3 u = cross(s, f);
		float4x4 result;
		result[0][0] = s.x; result[0][1] = s.y; result[0][2] = s.z; result[0][3] = 0;
		result[1][0] = u.x; result[1][1] = u.y; result[1][2] = u.z; result[1][3] = 0;
		result[2][0] = f.x; result[2][1] = f.y; result[2][2] = f.z; result[2][3] = 0;
		result[3][0] = 0;	result[3][1] = 0; result[3][2] = 0; float4x4 trans;
		for (int i = 0; i <= 3; i++) {
			trans[i][0] = 0;trans[i][1] = 0;trans[i][2] = 0;trans[i][3] = 0;
		}
		trans[0][3] = -eye.x;
		trans[1][3] = -eye.y;
		trans[2][3] = -eye.z;
