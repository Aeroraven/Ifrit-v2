...
namespace Ifrit::Engine::Imaging{
	...
	enum class BufferedImageTiling {
		TILING_LINEAR,
		TILING_TILED
	};
	...
	public:
		...
		virtual void setPixel3D(uint32_t x, uint32_t y, uint32_t z, const void* pixel) = 0;
		virtual void setPixel2D(uint32_t x, uint32_t y, const void* pixel) = 0;
		virtual void getPixel3D(uint32_t x, uint32_t y, uint32_t z, void* pixel) const = 0;
	...
	public:
		...
		inline uint32_t getPixelOffset3DLinear(uint32_t x, uint32_t y, uint32_t z) const {
		...
		inline uint32_t getPixelOffset3DTiled(uint32_t x, uint32_t y, uint32_t z) const {
			...
			const uint32_t tileOffset = (tileZ * (width / BufferedImageConfig::TILE_SIZE) * (height / BufferedImageConfig::TILE_SIZE) + tileY * (width / BufferedImageConfig::TILE_SIZE) + tileX) * (BufferedImageConfig::TILE_SIZE * BufferedImageConfig::TILE_SIZE * BufferedImageConfig::TILE_SIZE) * channels;
		...
		inline uint32_t getPixelOffset2DTiled(uint32_t x, uint32_t y) const {
			const uint32_t tileX = x / BufferedImageConfig::TILE_SIZE;
			const uint32_t tileY = y / BufferedImageConfig::TILE_SIZE;
			const uint32_t tileOffset = (tileY * (width / BufferedImageConfig::TILE_SIZE) + tileX) * (BufferedImageConfig::TILE_SIZE * BufferedImageConfig::TILE_SIZE) * channels;
			const uint32_t localX = x % BufferedImageConfig::TILE_SIZE;
			const uint32_t localY = y % BufferedImageConfig::TILE_SIZE;
		...
		inline uint32_t getRequiredBufferSizeTiled() const {
			const uint32_t numTilesX = (width + BufferedImageConfig::TILE_SIZE - 1) / BufferedImageConfig::TILE_SIZE;
			const uint32_t numTilesY = (height + BufferedImageConfig::TILE_SIZE - 1) / BufferedImageConfig::TILE_SIZE;
			const uint32_t numTilesZ = (depth + BufferedImageConfig::TILE_SIZE - 1) / BufferedImageConfig::TILE_SIZE;
	...
	public:
		...
		~BufferedImageImpl(){
			allocator.deallocate(data, getRequiredBufferSize());
		}
	...
	public:
		...
		inline void setPixel3D(uint32_t x, uint32_t y, uint32_t z, const void* pixel) {
		...
		inline void getPixel3D(uint32_t x, uint32_t y, uint32_t z, void* pixel) const {
			const uint32_t offset = this->getPixelOffset3DLinear(x, y, z);
