...
namespace  Ifrit::Engine::Imaging {
	inline void processWarpMode2D(IfritSamplerAddressMode mode, float& u, float& v) {
		switch (mode) {
		...
		case IfritSamplerAddressMode::IF_SAMPLER_ADDRESS_MODE_MIRRORED_REPEAT:
			u = u - floor(u);
			v = v - floor(v);
			if (int(floor(u)) % 2 == 1) u = 1.0f - u;
			if (int(floor(v)) % 2 == 1) v = 1.0f - v;
			break;
	...
	void BufferedImageSampler::sample2DDirect(float u, float v, int lod, iint2 offset, const LayeredBufferedImage& image, void* pixel) const {
		float wpU = u, wpV = v;
	...
	void BufferedImageSampler::sample3DDirect(float u, float v, float w, int lod, iint3 offset, const LayeredBufferedImage& image, void* pixel) const{
		...
		image.getLayer(lod).getPixel3D(x + offset.x, y + offset.y, z + offset.z, pixel);
	}
	void BufferedImageSampler::sample2DLod(float u, float v, float lod, iint2 offset, const LayeredBufferedImage& image, void* pixel) const{
		auto minLod = int(floor(lod));
		auto maxLod = int(ceil(lod));
		sample2DDirect(u, v, minLod, offset, image, pixel);
	}
	void BufferedImageSampler::sample3DLod(float u, float v, float w, float lod, iint3 offset, const LayeredBufferedImage& image, void* pixel) const{
		auto minLod = int(floor(lod));
		auto maxLod = int(ceil(lod));
