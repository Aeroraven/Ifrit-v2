...
#define DEF_PASS(x) void defPass_##x(int pc, std::vector<uint32_t> params, SpVcVMGeneratorContext* ctx, SpVcQuadGroupedIRGenerator* irg)
#define CONV_PASS(x) void convPass_##x(int pc, std::vector<uint32_t> params, SpVcVMGeneratorContext* ctx, SpVcQuadGroupedIRGenerator* irg)
...
#define GET_CONV_PASS(x) ConversionPass::convPass_##x
	...
	namespace DefinitionPass {
		...
		DEF_PASS(OpLoopMerge) {
			...
			contTgt->blockBelong->blockType.push_back({SPVC_BLOCK_LOOP_CONTINUE,startPc });
			auto act = irg->getActiveBlock();
		...
		DEF_PASS(OpBranchConditional) {
			...
			if (ctx->cfgInd == SPVC_STRUCTCFG_SELECTION_MERGE) {
				trueJmp->blockBelong->blockType.push_back({ SPVC_BLOCK_SELECTION_BODY_FIRST,startPc });
				falseJmp->blockBelong->blockType.push_back({ SPVC_BLOCK_SELECTION_BODY_SECOND,startPc });
			}
				...
				auto trueFind = std::find(trueJmp->blockBelong->blockType.begin(),trueJmp->blockBelong->blockType.end(),rec);
				if (trueFind == trueJmp->blockBelong->blockType.end())
					trueJmp->blockBelong->blockType.push_back({ SPVC_BLOCK_LOOP_BODY,startPc });
				auto falseFind = std::find(falseJmp->blockBelong->blockType.begin(),falseJmp->blockBelong->blockType.end(),rec);
				if (falseFind == falseJmp->blockBelong->blockType.end())
					falseJmp->blockBelong->blockType.push_back({ SPVC_BLOCK_LOOP_BODY,startPc });
			}
		...
		DEF_PASS(OpBranch) {
			...
			if (ctx->cfgInd == SPVC_STRUCTCFG_SELECTION_MERGE) {
				...
				SpVcBlockTypeRecord rec = { SPVC_BLOCK_LOOP_MERGE,startPc };
				auto jmpFind = std::find(jmp->blockBelong->blockType.begin(), jmp->blockBelong->blockType.end(), rec);
				...
				for(int i=0;i<jmp->blockBelong->blockType.size();i++) {
					if(jmp->blockBelong->blockType[i].blockType == SPVC_BLOCK_LOOP_MERGE) {
						auto act = irg->getActiveBlock();
						act->blockType.push_back({ SPVC_BLOCK_LOOP_BREAK, jmp->blockBelong->blockType[i].progCounter });
						break;
					}
				}
		...
		DEF_PASS(OpSwitch) {
			...
			for (int i = 1; i < params.size(); i++) {
				...
				target->blockBelong->blockType.push_back({ SPVC_BLOCK_SELECTION_BODY_SWITCH, startPc });
		...
		DEF_PASS(OpTypeFloat) {
			...
			if (bits == 64) tgt->tp->type = SpVcVMTypeEnum::SPVC_TYPE_FLOAT64;
			if (bits == 32) tgt->tp->type = SpVcVMTypeEnum::SPVC_TYPE_FLOAT32;
			if (bits == 16) tgt->tp->type = SpVcVMTypeEnum::SPVC_TYPE_FLOAT16;
			if (bits == 8) tgt->tp->type = SpVcVMTypeEnum::SPVC_TYPE_FLOAT8;
		}
		DEF_PASS(OpTypeInt) {
			auto tgt = GET_PARAM(0);
			auto bits = GET_PARAM_SCALAR(1);
			...
			tgt->flag |= SPVC_VARIABLE_TYPE;
			if (sign == 0) {
				if (bits == 64) tgt->tp->type = SpVcVMTypeEnum::SPVC_TYPE_UNSIGNED64;
				if (bits == 32) tgt->tp->type = SpVcVMTypeEnum::SPVC_TYPE_UNSIGNED32;
				if (bits == 16) tgt->tp->type = SpVcVMTypeEnum::SPVC_TYPE_UNSIGNED16;
				if (bits == 8) tgt->tp->type = SpVcVMTypeEnum::SPVC_TYPE_UNSIGNED8;
			}
			else {
				if (bits == 64) tgt->tp->type = SpVcVMTypeEnum::SPVC_TYPE_INT64;
				if (bits == 32) tgt->tp->type = SpVcVMTypeEnum::SPVC_TYPE_INT32;
				if (bits == 16) tgt->tp->type = SpVcVMTypeEnum::SPVC_TYPE_INT16;
				if (bits == 8) tgt->tp->type = SpVcVMTypeEnum::SPVC_TYPE_INT8;
			}
		}
		...
		// Mode Setting
		DEF_PASS(OpCapability) {
			ctx->capabilities.push_back(GET_PARAM_SCALAR(0));
		}
		...
		DEF_PASS(OpFunctionEnd) {
			ctx->activeFuncEnv = nullptr;
		}
		...
		DEF_PASS(OpMemberName) {
			EMIT_WARN("Not implemented: OpMemberName");
		}
		...
		DEF_PASS(OpVariable) {
			...
			if (tgt->storageClass == spv::StorageClass::StorageClassUniform || tgt->storageClass == spv::StorageClass::StorageClassUniformConstant) {
				tgt->isAllQuad = true;
			}
		...
		DEF_PASS(OpMemberDecorate) {
			EMIT_WARN("Not implemented: OpMemberDecorate");
		}
	...
	namespace ConversionPass {
		...
		LLVM::SpVcLLVMType* spirvGetImmediateType(int id, int invo, SpVcQuadGroupedIRGenerator* irg){
			return irg->getVariableSafe(id)->tpRef->tp->llvmType;
		}
		LLVM::SpVcLLVMArgument* spirvImmediateLoad(int id,int invo, SpVcQuadGroupedIRGenerator* irg) {
			auto var = irg->getVariableSafe(id);
			if (var->flag & (SPVC_VARIABLE_VAR | SPVC_VARIABLE_TEMP)) {
				if (var->isAllQuad) invo = 0;
				...
				auto cVarLoad = irg->addIrB(std::make_unique<LLVM::SpVcLLVMIns_LoadForcedPtr>(cVarTypeArg, cVar), irg->getActiveBlock());
				return cVarTypeArg;
			}
		...
		void spirvImmediateMaskedStore(int id, int invo, LLVM::SpVcLLVMArgument* val, SpVcQuadGroupedIRGenerator* irg) {
			auto var = irg->getVariableSafe(id);
			auto cVar = var->llvmVarName[invo].arg;
			...
			auto sel = irg->addIrB(std::make_unique<LLVM::SpVcLLVMIns_Select>(selRes, cmpReg, val, cVarArg), irg->getActiveBlock());
			auto cVarStore = irg->addIrB(std::make_unique<LLVM::SpVcLLVMIns_StoreForcedPtr>(cVar, selRes), irg->getActiveBlock());
		}
		...
		CONV_PASS(OpLabel) {
			auto CONTAIN_TAG = [&](int reqType) {
				...
				for (auto& s : p->blockBelong->blockType) {
					if (s.blockType == reqType) return true;
				}
				return false;
			};
			...
			auto MAKE_LABEL = [&]() {
				auto q = irg->addIrB(std::make_unique<LLVM::SpVcLLVMLabelName>(irg->allocateLlvmVarName()), irg->getActiveBlock());
				return q;
				};
			...
			auto FIND_STRUCT_PRED = [&](SpVcVMGenBlock* b) {
				...
				for (int i = 0; i < b->cfgPredecessor.size(); i++) {
					...
					if (std::find(b->cfgPredecessor[i]->blockType.begin(), b->cfgPredecessor[i]->blockType.end(), rp) != b->cfgPredecessor[i]->blockType.end()) {
						continue;
					}
					if (found) {
...
#define MASK_CONT(v,x) v.continueMask->llvmVarName[(x)].arg
			...
			if (CONTAIN_TAG(SPVC_BLOCK_SELECTION_HEADER)) {
				...
				FORALL_CHAN([&](int x) {
					...
					MAKE_MASK_AND(MASK_ACT_LAST(sp->masks, x), MASK_ACT_LAST(sp->masks, x), MASK_ACT_LAST_PREV(sp->masks, x));
		...
		CONV_PASS(OpSwitch) {
			EMIT_ERROR("not supported opswitch");
		}
		CONV_PASS(OpBranch) {
			...
			auto MAKE_MASK_AND = [&](LLVM::SpVcLLVMArgument* dest, LLVM::SpVcLLVMArgument* a, LLVM::SpVcLLVMArgument* b) {
				...
				auto rA = MAKE_MASK();
				auto rB = MAKE_MASK();
				//auto p = irg->addIrB(std::make_unique<LLVM::SpVcLLVMIns_Load>(rDest, dest), irg->getActiveBlock());
				auto q = irg->addIrB(std::make_unique<LLVM::SpVcLLVMIns_LoadForcedPtr>(rA, a), irg->getActiveBlock());
				auto s = irg->addIrB(std::make_unique<LLVM::SpVcLLVMIns_LoadForcedPtr>(rB, b), irg->getActiveBlock());
				auto t = irg->addIrB(std::make_unique<LLVM::SpVcLLVMIns_MathBinary>(rDest, rA, rB, "and"), irg->getActiveBlock());
				auto u = irg->addIrB(std::make_unique<LLVM::SpVcLLVMIns_StoreForcedPtr>(dest, rDest), irg->getActiveBlock());
				};
			auto MAKE_MASK_AND_TO_REG = [&](LLVM::SpVcLLVMArgument* dest, LLVM::SpVcLLVMArgument* a, LLVM::SpVcLLVMArgument* b) {
				auto rA = MAKE_MASK();
				auto rB = MAKE_MASK();
				auto q = irg->addIrB(std::make_unique<LLVM::SpVcLLVMIns_LoadForcedPtr>(rA, a), irg->getActiveBlock());
				auto s = irg->addIrB(std::make_unique<LLVM::SpVcLLVMIns_LoadForcedPtr>(rB, b), irg->getActiveBlock());
				auto t = irg->addIrB(std::make_unique<LLVM::SpVcLLVMIns_MathBinary>(dest, rA, rB, "and"), irg->getActiveBlock());
				};
			auto MAKE_MASK_OR_REG = [&](LLVM::SpVcLLVMArgument* dest, LLVM::SpVcLLVMArgument* a, LLVM::SpVcLLVMArgument* b) {
				auto rB = MAKE_MASK();
				auto s = irg->addIrB(std::make_unique<LLVM::SpVcLLVMIns_LoadForcedPtr>(rB, b), irg->getActiveBlock());
				auto t = irg->addIrB(std::make_unique<LLVM::SpVcLLVMIns_MathBinary>(dest, a, rB, "or"), irg->getActiveBlock());
				};
			auto target = GET_PARAM(0);
			...
			if (CONTAIN_TAG(SPVC_BLOCK_LOOP_BREAK)) {
				...
				FORALL_CHAN([&](int x) {
					...
					auto p = irg->addIrB(std::make_unique<LLVM::SpVcLLVMIns_LoadForcedPtr>(brMaskReg, brMask), irg->getActiveBlock());
					auto p1 = irg->addIrB(std::make_unique<LLVM::SpVcLLVMIns_LoadForcedPtr>(breakMaskReg, breakMask), irg->getActiveBlock());

					auto immOne = irg->addIr(std::make_unique<LLVM::SpVcLLVMConstantValueInt>(1));
					auto tp = irg->addIr(std::make_unique<LLVM::SpVcLLVMTypeInt32>());
					auto sq = irg->addIr(std::make_unique<LLVM::SpVcLLVMArgument>(tp, immOne));
					auto ps = irg->addIrB(std::make_unique<LLVM::SpVcLLVMIns_MathBinary>(brMaskRegNeg,sq, brMaskReg,"isub"), irg->getActiveBlock());
					auto ps1 = irg->addIrB(std::make_unique<LLVM::SpVcLLVMIns_MathBinary>(breakMaskReg, breakMaskReg, brMaskRegNeg, "and"), irg->getActiveBlock());

					//Store
					auto p2 = irg->addIrB(std::make_unique<LLVM::SpVcLLVMIns_StoreForcedPtr>(breakMask, breakMaskReg), irg->getActiveBlock());
				...
				LLVM::SpVcLLVMArgument* maskOrToken[4] = { MAKE_MASK(),MAKE_MASK(),MAKE_MASK(),MAKE_MASK() };
				FORALL_CHAN([&](int x) {
					if (x == 0) {
						MAKE_MASK_AND_TO_REG(maskOrToken[x], MASK_ACT_LAST(sp->masks, x), MASK_ACT_LAST(sp->masks, x));
					}
					else {
				...
				auto cmpRetName = irg->addIr(std::make_unique<LLVM::SpVcLLVMLocalVariableName>(irg->allocateLlvmVarName()));
				auto cmpRet = irg->addIr(std::make_unique<LLVM::SpVcLLVMArgument>(cmpRetTp, cmpRetName));
				auto cmp = irg->addIrB(std::make_unique<LLVM::SpVcLLVMIns_MathBinary>(cmpRet, maskOrToken[3], immZeroArg, "icmp eq"), irg->getActiveBlock());

				auto skipLabel = irg->addIr(std::make_unique<LLVM::SpVcLLVMLabelName>(irg->allocateLlvmVarName()));
				auto br = irg->addIrB(std::make_unique<LLVM::SpVcLLVMIns_BrCond>(cmpRet, mergeLabel, skipLabel), irg->getActiveBlock());
		...
		CONV_PASS(OpBranchConditional) {
			...
			if (CONTAIN_TAG(SPVC_BLOCK_LOOP_HEADER)) { // Indicates a loop break section
				...
				FORALL_CHAN([&](int x) {
					...
					auto p = irg->addIrF(std::make_unique<LLVM::SpVcLLVMIns_StoreForcedPtr>(mask, condArg), irg->getActiveBlock());
				});
				...
				irg->addIrB(std::make_unique<LLVM::SpVcLLVMIns_BrCond>(cmpRet, falseBlock->llvmLabel, trueBlock->llvmLabel), irg->getActiveBlock());
		...
		void convPassMathBinaryGeneral(int pc, std::vector<uint32_t> params, SpVcVMGeneratorContext* ctx, SpVcQuadGroupedIRGenerator* irg,std::string op) {
			...
			foreachInvo([&](int c) {
				...
				auto res = irg->addIrB(std::make_unique<LLVM::SpVcLLVMIns_MathBinary>(destImmRes, opAImm, opBImm, op), irg->getActiveBlock());
				spirvImmediateMaskedStore(dest, c, destImmRes, irg);

				}, irg);
		}
		void convPassMathUnaryGeneral(int pc, std::vector<uint32_t> params, SpVcVMGeneratorContext* ctx, SpVcQuadGroupedIRGenerator* irg, std::string op) {
			auto dest = GET_PARAM_SCALAR(1);
			auto opA = GET_PARAM_SCALAR(2);
			foreachInvo([&](int c) {
				auto destImmRes = spirvCreateVarWithSameType(dest, c, irg);
				auto opAImm = spirvImmediateLoad(opA, c, irg);
				auto res = irg->addIrB(std::make_unique<LLVM::SpVcLLVMIns_MathUnary>(destImmRes, opAImm, op), irg->getActiveBlock());
				spirvImmediateMaskedStore(dest, c, destImmRes, irg);

				}, irg);
		}

		void convPassMathBinaryInteger(int pc, std::vector<uint32_t> params, SpVcVMGeneratorContext* ctx, SpVcQuadGroupedIRGenerator* irg, std::string op) {
			auto dest = GET_PARAM_SCALAR(1);
			auto opA = GET_PARAM_SCALAR(2);
			auto opB = GET_PARAM_SCALAR(3);
		...
		CONV_PASS(OpFOrdLessThan) { convPassMathBinaryGeneral(pc, params, ctx, irg,"fcmp olt"); }
		CONV_PASS(OpFOrdGreaterThan) { convPassMathBinaryGeneral(pc, params, ctx, irg, "fcmp ogt"); }
		CONV_PASS(OpFOrdLessThanEqual) { convPassMathBinaryGeneral(pc, params, ctx, irg, "fcmp ole"); }
		CONV_PASS(OpFOrdGreaterThanEqual) { convPassMathBinaryGeneral(pc, params, ctx, irg, "fcmp oge"); }
		CONV_PASS(OpFOrdEqual) { convPassMathBinaryGeneral(pc, params, ctx, irg, "fcmp oeq"); }
		CONV_PASS(OpFOrdNotEqual) { convPassMathBinaryGeneral(pc, params, ctx, irg, "fcmp one"); }
		CONV_PASS(OpFUnordLessThan) { convPassMathBinaryGeneral(pc, params, ctx, irg, "fcmp ult"); }
		CONV_PASS(OpFUnordGreaterThan) { convPassMathBinaryGeneral(pc, params, ctx, irg, "fcmp ugt"); }
		CONV_PASS(OpFUnordLessThanEqual) { convPassMathBinaryGeneral(pc, params, ctx, irg, "fcmp ule"); }
		CONV_PASS(OpFUnordGreaterThanEqual) { convPassMathBinaryGeneral(pc, params, ctx, irg, "fcmp uge"); }
		CONV_PASS(OpFUnordEqual) { convPassMathBinaryGeneral(pc, params, ctx, irg, "fcmp ueq"); }
		CONV_PASS(OpFUnordNotEqual) { convPassMathBinaryGeneral(pc, params, ctx, irg, "fcmp une"); }

		CONV_PASS(OpSLessThan) { convPassMathBinaryGeneral(pc, params, ctx, irg, "icmp slt"); }
		CONV_PASS(OpSGreaterThan) { convPassMathBinaryGeneral(pc, params, ctx, irg, "icmp sgt"); }
		CONV_PASS(OpSLessThanEqual) { convPassMathBinaryGeneral(pc, params, ctx, irg, "icmp sle"); }
		CONV_PASS(OpSGreaterThanEqual) { convPassMathBinaryGeneral(pc, params, ctx, irg, "icmp sge"); }
		CONV_PASS(OpULessThan) { convPassMathBinaryGeneral(pc, params, ctx, irg, "icmp ult"); }
		CONV_PASS(OpUGreaterThan) { convPassMathBinaryGeneral(pc, params, ctx, irg, "icmp ugt"); }
		CONV_PASS(OpULessThanEqual) { convPassMathBinaryGeneral(pc, params, ctx, irg, "icmp ule"); }
		CONV_PASS(OpUGreaterThanEqual) { convPassMathBinaryGeneral(pc, params, ctx, irg, "icmp uge"); }
		CONV_PASS(OpIEqual) { convPassMathBinaryGeneral(pc, params, ctx, irg, "icmp eq"); }
		CONV_PASS(OpINotEqual) { convPassMathBinaryGeneral(pc, params, ctx, irg, "icmp ne"); }
		...
		CONV_PASS(OpVariable) {
			...
			if (tgt->isAllQuad) {
				...
				if (irg->getActiveBlock() == nullptr) {
					...
					auto allocaIns = irg->addIrFx(std::make_unique<LLVM::SpVcLLVMIns_Alloca>(arg, tpLlvm), tgt->blockBelong->funcBelong);
				}
		...
		LLVM::SpVcLLVMArgument* spirvCompositeConstruct(int invo, int& p, const std::vector<uint32_t>& params, SpVcVMTypeDescriptor* tp,
			...
			if (tp->type == SpVcVMTypeEnum::SPVC_TYPE_VECTOR) {
				...
				for (int i = 0; i < tp->size; i++) {
					...
					if (i == 0) {
						irg->addIrB(std::make_unique<LLVM::SpVcLLVMIns_InsertElementWithConstantIndexUndefInit>(rp, loadIns, i), b);
					}
					else {
						irg->addIrB(std::make_unique<LLVM::SpVcLLVMIns_InsertElementWithConstantIndex>(rp, last, loadIns, i), b);
					}
					last = rp;
				}
			}
				...
				for (int i = 0; i < tp->size; i++) {
					...
					auto vecBaseTp = irg->getVariableSafe(params[p + i])->tpRef->tp->children[0]->llvmType;
					...
					for (int j = 0; j < vecSize; j++) {
						...
						irg->addIrB(std::make_unique<LLVM::SpVcLLVMIns_ExtractElementWithConstantIndex>(regArgs[j], loadIns, j), b);
					}
					for(int j=0;j<vecSize;j++){
						auto rp = spirvCreateVarWithType(tp, irg);
						if (cnt == 0) {
							irg->addIrB(std::make_unique<LLVM::SpVcLLVMIns_InsertElementWithConstantIndexUndefInit>(rp, regArgs[j], cnt), b);
						}
						else {
							irg->addIrB(std::make_unique<LLVM::SpVcLLVMIns_InsertElementWithConstantIndex>(rp, last, regArgs[j], cnt), b);
						}
						cnt++;
						last = rp;
					}
		...
		LLVM::SpVcLLVMArgument* spirvCompositeExtract(int invo, int& p, const std::vector<uint32_t>& params, SpVcVMTypeDescriptor* tp,
			int tgtId, SpVcQuadGroupedIRGenerator* irg) {
			...
			if (tgtType->type == SpVcVMTypeEnum::SPVC_TYPE_VECTOR) {
				...
				irg->addIrB(std::make_unique<LLVM::SpVcLLVMIns_ExtractElementWithConstantIndex>(regArg, vecReg, idx), irg->getActiveBlock());
				return regArg;
			}
		...
		CONV_PASS(OpCompositeExtract) {
			...
			auto p = 3;
			foreachInvo([&](int x) {
				auto arg = spirvCompositeExtract(x, p, params, tp, GET_PARAM(2)->id, irg);
				spirvImmediateMaskedStore(tgt->id, x, arg, irg);
				}, irg);
		}
		...
		CONV_PASS(OpDot) {
			...
			foreachInvo([&](int x) {
				...
				auto elType = irg->getVariableSafe(src1->id)->tpRef->tp->children[0]->llvmType;
				...
				for (int i = 0; i < irg->getVariableSafe(src1->id)->tpRef->tp->size; i++) {
					...
					irg->addIrB(std::make_unique<LLVM::SpVcLLVMIns_ExtractElementWithConstantIndex>(args[i], reg1, i), irg->getActiveBlock());
				}
				//Sum up
				std::vector<LLVM::SpVcLLVMArgument*> largs;
				for (int i = 0; i < irg->getVariableSafe(src1->id)->tpRef->tp->size - 1; i++) {
					if (i == 0) {
						auto regName = irg->addIr(std::make_unique<LLVM::SpVcLLVMLocalVariableName>(irg->allocateLlvmVarName()));
						auto regArg = irg->addIr(std::make_unique<LLVM::SpVcLLVMArgument>(elType, regName));
						irg->addIrB(std::make_unique<LLVM::SpVcLLVMIns_MathBinary>(regArg, args[i], args[i + 1], "fmul"), irg->getActiveBlock());
						...
						auto regName = irg->addIr(std::make_unique<LLVM::SpVcLLVMLocalVariableName>(irg->allocateLlvmVarName()));
						auto regArg = irg->addIr(std::make_unique<LLVM::SpVcLLVMArgument>(elType, regName));
						irg->addIrB(std::make_unique<LLVM::SpVcLLVMIns_MathBinary>(regArg2, args[i], args[i + 1], "fmul"), irg->getActiveBlock());
						irg->addIrB(std::make_unique<LLVM::SpVcLLVMIns_MathBinary>(regArg, largs.back(), regArg2, "fadd"), irg->getActiveBlock());
						largs.push_back(regArg);
					}
				}
		...
		CONV_PASS(OpVectorShuffle) {
			...
			foreachInvo([&](int x) {
				...
				for (int i = 4; i < params.size(); i++) {
					...
					irg->addIrB(std::make_unique<LLVM::SpVcLLVMIns_ExtractElementWithConstantIndex>(regArg, procVec, procVecIdx), irg->getActiveBlock());
					...
					if (i == 4) {
						...
						irg->addIrB(std::make_unique<LLVM::SpVcLLVMIns_InsertElementWithConstantIndex>(retArg, ret[i-5], regArg, i-4), irg->getActiveBlock());
					}
				}}
			, irg);
	...
	void SpVcQuadGroupedIRGenerator::popNewStack(){
		mCtx->structStack.pop_back();
	}
	...
	std::string SpVcQuadGroupedIRGenerator::getParsingProgress(){
		return "[Pass " + std::to_string(curStage) + ", Line " + std::to_string(curPc) + "]";
	}
	void SpVcQuadGroupedIRGenerator::setCurrentProgCounter(int pc) {
		curPc = pc;
	}
	...
	void SpVcQuadGroupedIRGenerator::bindBytecode(SpVcSpirBytecode* bytecode, SpVcVMGeneratorContext* context){
		mRaw = bytecode;
		mCtx = context;
	}
	...
	void SpVcQuadGroupedIRGenerator::performDefinitionPassRegister(){
		...
		mDefinitionPassHandlers[spv::Op::OpExtInstImport] = GET_DEF_PASS(OpExtInstImport);
		...
		mDefinitionPassHandlers[spv::Op::OpExecutionMode] = GET_DEF_PASS(OpExecutionMode);
		mDefinitionPassHandlers[spv::Op::OpSource] = GET_DEF_PASS(OpSource);
		mDefinitionPassHandlers[spv::Op::OpSourceExtension] = GET_DEF_PASS(OpSourceExtension);
		...
		mDefinitionPassHandlers[spv::Op::OpTypeFunction] = GET_DEF_PASS(OpTypeFunction);
		...
		mDefinitionPassHandlers[spv::Op::OpTypeSampledImage] = GET_DEF_PASS(OpTypeSampledImage);
		...
		mDefinitionPassHandlers[spv::Op::OpSelectionMerge] = GET_DEF_PASS(OpSelectionMerge);
		mDefinitionPassHandlers[spv::Op::OpLoopMerge] = GET_DEF_PASS(OpLoopMerge);
		mDefinitionPassHandlers[spv::Op::OpBranchConditional] = GET_DEF_PASS(OpBranchConditional);
	...
	void SpVcQuadGroupedIRGenerator::performDataflowResolutionPassRegister() {
		...
		// Otherwise: -1 -> all
		auto DATA_DEP = [](int ret, std::vector<int> dep, int vaDep)->SpVcDataflowDependency {
			return { ret,std::vector<int>(dep),vaDep };
			};
		auto DATA_DEP_SPECIAL  = [](SpVcDataflowDependencySpecial x)->SpVcDataflowDependency {
			return { -1,{},-1,x };
			};
		...
		mArgumentDependency[spv::Op::OpImageSampleImplicitLod] = DATA_DEP(1, { 0,2,3 }, 5);
		mArgumentDependency[spv::Op::OpImageSampleExplicitLod] = DATA_DEP(1, { 0,2,3,5 }, 6);
		mArgumentDependency[spv::Op::OpImageSampleDrefImplicitLod] = DATA_DEP(1, { 0,2,3,4 }, 6);
		mArgumentDependency[spv::Op::OpImageSampleDrefExplicitLod] = DATA_DEP(1, { 0,2,3,4,6 }, 7);
		mArgumentDependency[spv::Op::OpImageSampleProjImplicitLod] = DATA_DEP(1, { 0,2,3 }, 5);
		mArgumentDependency[spv::Op::OpImageSampleProjExplicitLod] = DATA_DEP(1, { 0,2,3,5 }, 6);
		mArgumentDependency[spv::Op::OpImageSampleProjDrefImplicitLod] = DATA_DEP(1, { 0,2,3,4 }, 6);
		mArgumentDependency[spv::Op::OpImageSampleProjDrefExplicitLod] = DATA_DEP(1, { 0,2,3,4,6 }, 7);
		mArgumentDependency[spv::Op::OpImageFetch] = DATA_DEP(1, { 0,2,3 }, 5);
		mArgumentDependency[spv::Op::OpImageGather] = DATA_DEP(1, { 0,2,3,4 }, 6);
		mArgumentDependency[spv::Op::OpImageDrefGather] = DATA_DEP(1, { 0,2,3,4,5 }, 7);
		mArgumentDependency[spv::Op::OpImageQuerySizeLod] = DATA_DEP(1, { 0,2,3 }, -1);
		...
		mArgumentDependency[spv::Op::OpImageSparseSampleImplicitLod] = DATA_DEP(1, { 0,2,3,4 }, 6);
		mArgumentDependency[spv::Op::OpImageSparseSampleExplicitLod] = DATA_DEP(1, { 0,2,3,4,6 }, 7);
		mArgumentDependency[spv::Op::OpImageSparseSampleDrefImplicitLod] = DATA_DEP(1, { 0,2,3,4,5 }, 7);
		mArgumentDependency[spv::Op::OpImageSparseSampleDrefExplicitLod] = DATA_DEP(1, { 0,2,3,4,5,7 }, 8);
		mArgumentDependency[spv::Op::OpImageSparseSampleProjImplicitLod] = DATA_DEP(1, { 0,2,3 }, 5);
		mArgumentDependency[spv::Op::OpImageSparseSampleProjExplicitLod] = DATA_DEP(1, { 0,2,3,5 }, 6);
		mArgumentDependency[spv::Op::OpImageSparseSampleProjDrefImplicitLod] = DATA_DEP(1, { 0,2,3,4 }, 6);
		mArgumentDependency[spv::Op::OpImageSparseSampleProjDrefExplicitLod] = DATA_DEP(1, { 0,2,3,4,6 }, 7);
		mArgumentDependency[spv::Op::OpImageSparseFetch] = DATA_DEP(1, { 0,2,3 }, 5);
		mArgumentDependency[spv::Op::OpImageSparseGather] = DATA_DEP(1, { 0,2,3,4 }, 6);
		mArgumentDependency[spv::Op::OpImageSparseDrefGather] = DATA_DEP(1, { 0,2,3,4 }, 6);
		mArgumentDependency[spv::Op::OpImageSparseTexelsResident] = DATA_DEP(1, { 0,2 }, -1);
		...
		mArgumentDependency[spv::Op::OpFOrdGreaterThanEqual] = DATA_DEP(1, { 0,2 }, -1);
		mArgumentDependency[spv::Op::OpFUnordGreaterThanEqual] = DATA_DEP(1, { 0,2 }, -1);
		...
		mArgumentDependency[spv::Op::OpShiftRightArithmetic] = DATA_DEP(1, { 0,2 }, -1);
		...
		mArgumentDependency[spv::Op::OpVectorTimesScalar] = DATA_DEP(1, { 0,2,3 }, -1);
		mArgumentDependency[spv::Op::OpVectorTimesMatrix] = DATA_DEP(1, { 0,2,3 }, -1);
		mArgumentDependency[spv::Op::OpMatrixTimesVector] = DATA_DEP(1, { 0,2,3 }, -1);
		mArgumentDependency[spv::Op::OpMatrixTimesScalar] = DATA_DEP(1, { 0,2,3 }, -1);
	...
	void SpVcQuadGroupedIRGenerator::performConversionPassRegister(){
		...
		mConvPassHandlers[spv::Op::OpBranchConditional] = GET_CONV_PASS(OpBranchConditional);
		...
		mConvPassHandlers[spv::Op::OpFOrdGreaterThan] = GET_CONV_PASS(OpFOrdGreaterThan);
		mConvPassHandlers[spv::Op::OpFOrdLessThanEqual] = GET_CONV_PASS(OpFOrdLessThanEqual);
		mConvPassHandlers[spv::Op::OpFOrdGreaterThanEqual] = GET_CONV_PASS(OpFOrdGreaterThanEqual);
		...
		mConvPassHandlers[spv::Op::OpFUnordLessThan] = GET_CONV_PASS(OpFUnordLessThan);
		mConvPassHandlers[spv::Op::OpFUnordGreaterThan] = GET_CONV_PASS(OpFUnordGreaterThan);
		mConvPassHandlers[spv::Op::OpFUnordLessThanEqual] = GET_CONV_PASS(OpFUnordLessThanEqual);
		mConvPassHandlers[spv::Op::OpFUnordGreaterThanEqual] = GET_CONV_PASS(OpFUnordGreaterThanEqual);
		mConvPassHandlers[spv::Op::OpFUnordEqual] = GET_CONV_PASS(OpFUnordEqual);
		mConvPassHandlers[spv::Op::OpFUnordNotEqual] = GET_CONV_PASS(OpFUnordNotEqual);
		...
		mConvPassHandlers[spv::Op::OpSLessThanEqual] = GET_CONV_PASS(OpSLessThanEqual);
		mConvPassHandlers[spv::Op::OpSGreaterThanEqual] = GET_CONV_PASS(OpSGreaterThanEqual);
		...
		mConvPassHandlers[spv::Op::OpULessThanEqual] = GET_CONV_PASS(OpULessThanEqual);
		mConvPassHandlers[spv::Op::OpUGreaterThanEqual] = GET_CONV_PASS(OpUGreaterThanEqual);
...
#define VERBOSE_TPGEN(x) //EMIT_VERBOSE_CORE("Type generated: ",x->emitIR())
		for (auto k:mCtx->vO) {
			...
			if (v.flag == SpVcBlockTypeVariableTypeFlag::SPVC_VARIABLE_TYPE) {
				if (v.tp->type == SpVcVMTypeEnum::SPVC_TYPE_INT32) {
					...
					EMIT_WARN_CORE("Unknown type encountered:", (int)v.tp->type, ", At variable %",v.id);
				}
			}
		}
		...
		for (auto& block : mCtx->blocks) {
			...
			//EMIT_VERBOSE_CORE("Allocated llvmir for block entry label: ", varName->emitIR());
		}
		...
		for (int i = 0; i < mRaw->instructions.size(); i++) {
			...
			if (ins.opCode == spv::Op::OpPhi) {
				for (int j = 2; j < ins.opParams.size(); j++) {
					...
					if (var->flag & (SPVC_VARIABLE_VAR | SPVC_VARIABLE_TEMP)) {
						if (var->blockBelong != getVariableSafe(ins.opParams[j + 1])->blockBelong) {
	...
	void SpVcQuadGroupedIRGenerator::performBlockPass() {
		...
		for (int i = 0; i < mRaw->instructions.size(); i++) {
			...
			if (inst.opCode == spv::Op::OpLabel) {
				...
				getVariableSafe(inst.opParams[0])->blockBelong = mCtx->blocks.back().get();
			}
	...
	void SpVcQuadGroupedIRGenerator::verbose(){
		...
		for (int i = 0; i < mCtx->blocks.size(); i++) {
			...
			for (int j = 0; j < mCtx->blocks[i]->blockType.size(); j++) {
				...
				case SPVC_BLOCK_SELECTION_HEADER:
					printf("<Selection Header %d> ", mCtx->blocks[i]->blockType[j].progCounter);
					break;
				case SPVC_BLOCK_SELECTION_MERGE:
					printf("<Selection Merge %d> ", mCtx->blocks[i]->blockType[j].progCounter);
					break;
				case SPVC_BLOCK_SELECTION_BODY_FIRST:
					printf("<Selection Body <True> %d> ", mCtx->blocks[i]->blockType[j].progCounter);
					break;
				case SPVC_BLOCK_SELECTION_BODY_SECOND:
					printf("<Selection Body <False> %d> ", mCtx->blocks[i]->blockType[j].progCounter);
					break;
				case SPVC_BLOCK_SELECTION_BODY_SWITCH:
					printf("<Selection Body <Switch> %d> ", mCtx->blocks[i]->blockType[j].progCounter);
					break;
				...
				case SPVC_BLOCK_LOOP_CONTINUE:
					printf("<Loop Continue %d> ", mCtx->blocks[i]->blockType[j].progCounter);
					break;
		...
		printf("====== Registers ====== \n");
		for (auto& [k, v] : mCtx->v) {
			printf("[%d] In Block %d\n",v.id, (v.blockBelong) ? v.blockBelong->startingPc : -1);
			printf("  Rely On: ");
	...
	void SpVcQuadGroupedIRGenerator::performDefinitionPass(){
		...
		for (int i = 0; i < ins.size(); i++) {
			...
			if(mDefinitionPassHandlers.count(ins[i].opCode)>0) {
				...
				EMIT_ERROR_CORE("Unknown opcode: ", ins[i].opCode, spv::OpToString((spv::Op)ins[i].opCode));
			}
		}
	}
	...
	void SpVcQuadGroupedIRGenerator::performConversionPass() {
		...
		for (int i = 0; i < ins.size(); i++) {
			...
			if (mConvPassHandlers.count(ins[i].opCode) > 0) {
				...
				if (hasRes && hasRtype) {
					...
					for (auto& phiDep : var->phiDeps) {
						...
						for (int k = 0; k < getQuads(); k++) {
							...
							ConversionPass::spirvImmediateMaskedStore(phiDep->id, k,immReg, this);
						}
					}
				}
				if (ins[i].opCode == spv::Op::OpLabel) {
					...
					for (auto& phiDep : var->phiDepsEx) {
						...
						for (int k = 0; k < getQuads(); k++) {
							...
							ConversionPass::spirvImmediateMaskedStore(phiDep.second->id, k, immReg, this);
						}
					}
				}
				...
				EMIT_WARN_CORE("Unknown opcode: ", ins[i].opCode, spv::OpToString((spv::Op)ins[i].opCode));
			}
		}
	}
			...
			if (mArgumentDependency.count(ins[i].opCode) > 0) {
				...
				if (dep.special == SPVC_DATADEPS_ORDINARY) {
					...
					if (dep.retArg != -1) {
						...
						//EMIT_VERBOSE_CORE("No return value for ", ins[i].opCode, " at ", i);
					}
