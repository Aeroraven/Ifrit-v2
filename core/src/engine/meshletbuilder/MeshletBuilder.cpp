...
namespace Ifrit::Engine::MeshletBuilder::Impl {
    struct MbTriangle {
        iint3 ind;
        ifloat3 normal;
        ifloat3 centroid;
    ...
    void initializeContext(MbContext* ctx, const VertexBuffer& vbuf, const std::vector<int>& ibuf, int posAttrId) {
        ...
        auto cross = [](ifloat3 a,ifloat3 b){
            return ifloat3{ a.y * b.z - a.z * b.y, a.z * b.x - a.x * b.z, a.x * b.y - a.y * b.x };
        };
        auto centroid = [](ifloat4 a, ifloat4 b, ifloat4 c){
            return ifloat3{(a.x+b.x+c.x)/3,(a.y+b.y+c.y)/3,(a.z+b.z+c.z)/3};
        };
    ...
    int meshletExpansion(const MbContext& ctx, const MbCurrentMeshlet& meshlet) {
        // Algo Process Reference: https://github.com/zeux/meshoptimizer/blob/master/src/clusterizer.cpp
        ifloat3 clusterNormal = {meshlet.sumNormal.x*meshlet.invSumNormal, meshlet.sumNormal.y*meshlet.invSumNormal,meshlet.sumNormal.z*meshlet.invSumNormal};
        ...
        if (meshlet.triangleContained.size() == 0) {
            for (int i = 0; i < ctx.triangleEmitted.size(); i++) {
                if (ctx.triangleEmitted[i] == 0)return i;
            }
        }
        for (int i = 0; i < meshlet.usedVertices.size(); i++) {
            ...
            for (int j = 0; j < ctx.adjMaps[v].size(); j++) {
                ...
                auto la = ctx.remainActiveAdj[tri.ind.x], lb = ctx.remainActiveAdj[tri.ind.y], lc = ctx.remainActiveAdj[tri.ind.z];
                auto prior = ua + ub + uc;
                if (prior != 0) {
                    if (la == 1 && lb == 1 && lc == 1)prior = 0;
                    prior++;
                }
    ...
    void addTriangleToMeshlet(int triId, MbContext* ctx, MbCurrentMeshlet* meshlet) {
        ...
        for (int i = 0; i < ctx->adjMaps[tri.ind.x].size(); i++) {
            if (ctx->adjMaps[tri.ind.x][i] == triId)ctx->adjMaps[tri.ind.x][i] = -1;
        }
        for (int i = 0; i < ctx->adjMaps[tri.ind.y].size(); i++) {
            if (ctx->adjMaps[tri.ind.y][i] == triId)ctx->adjMaps[tri.ind.y][i] = -1;
        }
        for (int i = 0; i < ctx->adjMaps[tri.ind.z].size(); i++) {
            if (ctx->adjMaps[tri.ind.z][i] == triId)ctx->adjMaps[tri.ind.z][i] = -1;
        }
    ...
    void initializeCurrentMeshlet(const MbContext& ctx, MbCurrentMeshlet* meshlet) {
        ...
        for (int i = 0; i < ctx.vertices.size(); i++) meshlet->isvertexUsed[i] = 0;
    }
    ...
    void writeGenratedMeshlet(MbContext* ctx, const MbCurrentMeshlet& meshlet) {
        ...
        for (int i = 0; i < totalVerts; i++) {
            emitMeshlet->vbufs.setValue<ifloat4>(i, 0, ctx->vertices[meshlet.usedVertices[i]]);
    ...
    IFRIT_APIDECL void TrivialMeshletBuilder::mergeMeshlet(const std::vector<std::unique_ptr<Meshlet>>& meshlets,
        ...
        for(auto& m:meshlets){
            for(int i=0;i<m->vbufs.getVertexCount();i++){
                outData.vbufs.setValue(accuOffset+i,0,m->vbufs.getValue<ifloat4>(i,0));
