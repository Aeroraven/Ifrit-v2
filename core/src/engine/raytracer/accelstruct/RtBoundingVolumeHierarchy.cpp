...
namespace Ifrit::Engine::Raytracer::Impl {
	...
	int procFindSplit(int start, int end, int axis, float mid, std::vector<vfloat3>& centers,
		std::vector<int>& belonging) {
		using namespace Ifrit::Math;
		int l = start, r = end;

		while (l < r) {
			while (l < r && elementAt(centers[belonging[l]], axis) < mid) l++;
			while (l < r && elementAt(centers[belonging[r]], axis) >= mid) r--;
			if (l < r) {
	...
	void procBuildBvhNode(int size, BVHNode* root, const std::vector<BoundingBox>& bboxes, std::vector<vfloat3>& centers,
	...
	vfloat3 getElementCenterTLAS(int index, const std::vector<BoundingVolumeHierarchyBottomLevelAS*>& data)  {
		using namespace Ifrit::Math;
	...
	void procBuildBvhTLAS(std::unique_ptr<BVHNode>& root, int size, std::vector<BoundingBox>& bboxes, std::vector<int>& indices,
		std::vector<vfloat3>& centers, std::vector<int>& belonging, const std::vector<BoundingVolumeHierarchyBottomLevelAS*>& data) {
	...
	inline RayHit procQueryRayIntersectionTLAS(const RayInternal& ray, float tmin, float tmax, 
		const std::vector<int>& belonging,	bool doRootBoxIgnore, BVHNode* root, const std::vector<BoundingVolumeHierarchyBottomLevelAS*>& data) IFRIT_AP_NOTHROW {
		
	}
}

namespace Ifrit::Engine::Raytracer {
	void BoundingVolumeHierarchyBottomLevelAS::bufferData(const std::vector<ifloat3>& vecData) {
	...
	RayHit BoundingVolumeHierarchyBottomLevelAS::queryIntersection(const RayInternal& ray, float tmin, float tmax) const {
		...
		while (qPos) {
			...
			if (nLeft == nullptr || nRight == nullptr) {
				...
				else if (leftIntersect > 0) q[qPos++] = { nLeft,leftIntersect };
