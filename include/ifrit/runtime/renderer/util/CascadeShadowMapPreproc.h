...
namespace Ifrit::Runtime::RenderingUtil::CascadeShadowMapping
{
    ...
    IFRIT_APIDECL CSMResult calculateCSMSplits(const Ifrit::Runtime::PerFrameData::PerViewData& perView,
        u32 shadowResolution, Vector3f lightFront, u32 splitCount, f32 maxDistance, const Vec<f32>& splits,
        const Vec<f32>& borders);

    IFRIT_APIDECL Vec<PerFrameData::PerViewData> fillCSMViews(const Ifrit::Runtime::PerFrameData::PerViewData& perView,
        Light& light, u32 shadowResolution, Transform& lightTransform, u32 splitCount, f32 maxDistance,
        const Vec<f32>& splits, const Vec<f32>& borders, Array<f32, 4>& splitStart, Array<f32, 4>& splitEnd);
