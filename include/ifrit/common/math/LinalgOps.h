...
namespace Ifrit::Math
{
    ...
    IF_FORCEINLINE Matrix4x4f Inverse4(const Matrix4x4f& p)
    {
        ...
        auto       det    = p[0][0] * (p[1][1] * a2323 - p[1][2] * a1323 + p[1][3] * a1223) - p[0][1] * (p[1][0] * a2323 - p[1][2] * a0323 + p[1][3] * a0223) + p[0][2] * (p[1][0] * a1323 - p[1][1] * a0323 + p[1][3] * a0123) - p[0][3] * (p[1][0] * a1223 - p[1][1] * a0223 + p[1][2] * a0123);
        auto       invdet = 1 / det;
        Matrix4x4f inv;
    ...
    IF_FORCEINLINE Matrix4x4f GetTransformMatrix(const Vector3f& scale, const Vector3f& translation,
