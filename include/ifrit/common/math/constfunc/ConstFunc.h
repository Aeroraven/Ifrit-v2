...
namespace Ifrit::Math
{
    ...
    IF_CONSTEXPR Array<U, N> ConvertArray(const Array<T, N>& arr)
    {
        Array<U, N> result;
        for (u32 i = 0; i < N; ++i)
        {
    ...
    IF_CONSTEXPR Array<T, N> UniformSampleInclusive(T min, T max)
    {
        Array<T, N> result;
        for (u32 i = 0; i < N; ++i)
        {
    ...
    template <typename T, u32 N>
    IF_CONSTEXPR T BinLerp(const Array<T, N> bins, const Array<T, N> values, T x)
    {
        if (x <= bins[0])
        {
            return values[0];
    ...
    IF_CONSTEXPR auto DivRoundUp(auto a, auto b)
    {
        return (a + b - 1) / b;
    }

    // Upper align
    IF_CONSTEXPR auto alignUp(auto csize, auto alignSize)
    {
