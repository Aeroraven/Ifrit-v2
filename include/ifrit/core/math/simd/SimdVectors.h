...
// Crosss
    ...
    // FMA
    template <typename T, typename S, int V>
    inline SimdVector<T, S, V> Fma(
        const SimdVector<T, S, V>& a, const SimdVector<T, S, V>& b, const SimdVector<T, S, V>& c)
    {
        SimdVector<T, S, V> r;
#ifdef IFRIT_USE_SIMD_128
        if IF_CONSTEXPR (std::is_same_v<S, __m128>)
    ...
    // Length
    template <typename T, typename S, int V> inline T Length(const SimdVector<T, S, V>& a) { return sqrt(Dot(a, a)); }
